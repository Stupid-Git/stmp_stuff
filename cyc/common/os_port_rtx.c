   ...
   const OsTaskParameters *params)
{
   ...
   {
      //Create a new task
      taskId = os_tsk_create_user_ex(taskCode, params->priority, params->stack,
         params->stackSize * sizeof(uint32_t), arg);
   }
   else
   {
      //Create a new task
      taskId = os_tsk_create_ex(taskCode, params->priority, arg);
   }
