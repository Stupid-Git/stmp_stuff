...
#pragma location = XMC4400_ETH_RAM_SECTION
static uint8_t txBuffer[XMC4400_ETH_TX_BUFFER_COUNT][XMC4400_ETH_TX_BUFFER_SIZE];
//Receive buffer
#pragma data_alignment = 4
#pragma location = XMC4400_ETH_RAM_SECTION
static uint8_t rxBuffer[XMC4400_ETH_RX_BUFFER_COUNT][XMC4400_ETH_RX_BUFFER_SIZE];
//Transmit DMA descriptors
#pragma data_alignment = 4
#pragma location = XMC4400_ETH_RAM_SECTION
static Xmc4400TxDmaDesc txDmaDesc[XMC4400_ETH_TX_BUFFER_COUNT];
//Receive DMA descriptors
#pragma data_alignment = 4
#pragma location = XMC4400_ETH_RAM_SECTION
static Xmc4400RxDmaDesc rxDmaDesc[XMC4400_ETH_RX_BUFFER_COUNT];
...
//Transmit buffer
static uint8_t txBuffer[XMC4400_ETH_TX_BUFFER_COUNT][XMC4400_ETH_TX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(XMC4400_ETH_RAM_SECTION)));
//Receive buffer
static uint8_t rxBuffer[XMC4400_ETH_RX_BUFFER_COUNT][XMC4400_ETH_RX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(XMC4400_ETH_RAM_SECTION)));
//Transmit DMA descriptors
...
static Xmc4400TxDmaDesc *txCurDmaDesc;
//Pointer to the current RX DMA descriptor
static Xmc4400RxDmaDesc *rxCurDmaDesc;


/**
 * @brief XMC4400 Ethernet MAC driver
 **/
...
const NicDriver xmc4400EthDriver =
{
   ...
   xmc4400EthTick,
   xmc4400EthEnableIrq,
   xmc4400EthDisableIrq,
   xmc4400EthEventHandler,
   xmc4400EthSendPacket,
   xmc4400EthUpdateMacAddrFilter,
   xmc4400EthUpdateMacConfig,
   xmc4400EthWritePhyReg,
   xmc4400EthReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
   FALSE
...
/**
 * @brief XMC4400 Ethernet MAC initialization
...
void xmc4400EthInitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < XMC4400_ETH_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < XMC4400_ETH_RX_BUFFER_COUNT; i++)
   {
      //The descriptor is initially owned by the DMA
      rxDmaDesc[i].rdes0 = ETH_RDES0_OWN;
      //Use chain structure rather than ring structure
      rxDmaDesc[i].rdes1 = ETH_RDES1_RCH | (XMC4400_ETH_RX_BUFFER_SIZE & ETH_RDES1_RBS1);
...
/**
 * @brief XMC4400 Ethernet MAC timer handler
 *
...
/**
 * @brief XMC4400 Ethernet MAC interrupt service routine
 **/
...
/**
 * @brief XMC4400 Ethernet MAC event handler
 * @param[in] interface Underlying network interface
 **/
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > XMC4400_ETH_TX_BUFFER_SIZE)
   {
   ...
   txCurDmaDesc = (Xmc4400TxDmaDesc *) txCurDmaDesc->tdes3;
   ...
   if((rxCurDmaDesc->rdes0 & ETH_RDES0_OWN) == 0)
   {
         ...
         (rxCurDmaDesc->rdes0 & ETH_RDES0_LS) != 0)
      {
         ...
         if((rxCurDmaDesc->rdes0 & ETH_RDES0_ES) == 0)
         {
            ...
            n = MIN(n, XMC4400_ETH_RX_BUFFER_SIZE);
      ...
      //Point to the next descriptor in the list
      rxCurDmaDesc = (Xmc4400RxDmaDesc *) rxCurDmaDesc->rdes3;
   }
   else
   {
      //No more data in the receive buffer
   ...
   for(i = 0, j = 0; i < MAC_ADDR_FILTER_SIZE; i++)
   {
      ...
      if(entry->refCount > 0)
      {
         ...
         if(macIsMulticastAddr(&entry->addr))
         {
            ...
            crc = xmc4400EthCalcCrc(&entry->addr, sizeof(MacAddr));
