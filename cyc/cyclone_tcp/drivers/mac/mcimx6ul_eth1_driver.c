 ...
 * @brief NXP i.MX6UL Ethernet MAC driver (ENET1 instance)
 *
 * @section License
 *
...
#pragma location = MCIMX6UL_ETH1_RAM_SECTION
static uint8_t txBuffer[MCIMX6UL_ETH1_TX_BUFFER_COUNT][MCIMX6UL_ETH1_TX_BUFFER_SIZE];
//RX buffer
#pragma data_alignment = 64
#pragma location = MCIMX6UL_ETH1_RAM_SECTION
static uint8_t rxBuffer[MCIMX6UL_ETH1_RX_BUFFER_COUNT][MCIMX6UL_ETH1_RX_BUFFER_SIZE];
//TX buffer descriptors
#pragma data_alignment = 64
#pragma location = MCIMX6UL_ETH1_RAM_SECTION
static uint32_t txBufferDesc[MCIMX6UL_ETH1_TX_BUFFER_COUNT][8];
//RX buffer descriptors
#pragma data_alignment = 64
#pragma location = MCIMX6UL_ETH1_RAM_SECTION
static uint32_t rxBufferDesc[MCIMX6UL_ETH1_RX_BUFFER_COUNT][8];
...
//TX buffer
static uint8_t txBuffer[MCIMX6UL_ETH1_TX_BUFFER_COUNT][MCIMX6UL_ETH1_TX_BUFFER_SIZE]
   __attribute__((aligned(64), __section__(MCIMX6UL_ETH1_RAM_SECTION)));
//RX buffer
static uint8_t rxBuffer[MCIMX6UL_ETH1_RX_BUFFER_COUNT][MCIMX6UL_ETH1_RX_BUFFER_SIZE]
   __attribute__((aligned(64), __section__(MCIMX6UL_ETH1_RAM_SECTION)));
//TX buffer descriptors
...
/**
 * @brief i.MX6UL Ethernet MAC driver (ENET1 instance)
 **/
...
const NicDriver mcimx6ulEth1Driver =
{
   ...
   mcimx6ulEth1Init,
   mcimx6ulEth1Tick,
   mcimx6ulEth1EnableIrq,
   mcimx6ulEth1DisableIrq,
   mcimx6ulEth1EventHandler,
   mcimx6ulEth1SendPacket,
   mcimx6ulEth1UpdateMacAddrFilter,
   mcimx6ulEth1UpdateMacConfig,
   mcimx6ulEth1WritePhyReg,
   mcimx6ulEth1ReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
   FALSE
...
error_t mcimx6ulEth1Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing i.MX6UL Ethernet MAC (ENET1)...\r\n");
   ...
   //GPIO configuration
   mcimx6ulEth1InitGpio(interface);

   //Reset ENET1 module
   ENET1->ECR = ENET_ECR_RESET_MASK;
   //Wait for the reset to complete
   while((ENET1->ECR & ENET_ECR_RESET_MASK) != 0)
   {
   }

   //Receive control register
   ENET1->RCR = ENET_RCR_MAX_FL(MCIMX6UL_ETH1_RX_BUFFER_SIZE) |
      ENET_RCR_RMII_MODE_MASK | ENET_RCR_MII_MODE_MASK;

   //Transmit control register
   ENET1->TCR = 0;
   //Configure MDC clock frequency
   ENET1->MSCR = ENET_MSCR_HOLDTIME(10) | ENET_MSCR_MII_SPEED(120);
   ...
   ENET1->PAUR = ENET_PAUR_PADDR2(value) | ENET_PAUR_TYPE(0x8808);
   ...
   ENET1->PALR = ENET_PALR_PADDR1(value);
   ...
   ENET1->ECR = ENET_ECR_DBSWP_MASK | ENET_ECR_EN1588_MASK;

   //Reset statistics counters
   ENET1->MIBC = ENET_MIBC_MIB_CLEAR_MASK;
   ENET1->MIBC = 0;

   //Initialize buffer descriptors
   mcimx6ulEth1InitBufferDesc(interface);

   //Clear any pending interrupts
   ENET1->EIR = 0xFFFFFFFF;
   //Enable desired interrupts
   ENET1->EIMR = ENET_EIMR_TXF_MASK | ENET_EIMR_RXF_MASK | ENET_EIMR_EBERR_MASK;

   //Configure ENET1 interrupt priority
   GIC_SetPriority(ENET1_IRQn, MCIMX6UL_ETH1_IRQ_PRIORITY);

   //Enable Ethernet MAC
   ENET1->ECR |= ENET_ECR_ETHEREN_MASK;
   //Instruct the DMA to poll the receive descriptor list
   ENET1->RDAR = ENET_RDAR_RDAR_MASK;
...
//MCIMX6UL-EVKB or MCIMX6ULL-EVK evaluation board?
   ...
   //Configure ENET1_TX_CLK pin as ENET1_REF_CLK1
   IOMUXC_SetPinMux(IOMUXC_ENET1_TX_CLK_ENET1_REF_CLK1, 1);

   //Set ENET1_TX_CLK pad properties
   ...
   //Configure ENET1_TX_EN pin as ENET1_TX_EN
   IOMUXC_SetPinMux(IOMUXC_ENET1_TX_EN_ENET1_TX_EN, 0);

   //Set ENET1_TX_EN pad properties
   ...
   //Configure ENET1_TX_DATA0 pin as ENET1_TDATA00
   IOMUXC_SetPinMux(IOMUXC_ENET1_TX_DATA0_ENET1_TDATA00, 0);

   //Set ENET1_TX_DATA0 pad properties
   ...
   //Configure ENET1_TX_DATA1 pin as ENET1_TDATA01
   IOMUXC_SetPinMux(IOMUXC_ENET1_TX_DATA1_ENET1_TDATA01, 0);

   //Set ENET1_TX_DATA1 pad properties
   ...
   //Configure ENET1_RX_EN pin as ENET1_RX_EN
   IOMUXC_SetPinMux(IOMUXC_ENET1_RX_EN_ENET1_RX_EN, 0);

   //Set ENET1_RX_EN pad properties
   ...
   //Configure ENET1_RX_ER pin as ENET1_RX_ER
   IOMUXC_SetPinMux(IOMUXC_ENET1_RX_ER_ENET1_RX_ER, 0);

   //Set ENET1_RX_ER pad properties
   ...
   //Configure ENET1_RX_DATA0 pin as ENET1_RDATA00
   IOMUXC_SetPinMux(IOMUXC_ENET1_RX_DATA0_ENET1_RDATA00, 0);

   //Set ENET1_RX_DATA0 pad properties
   ...
   //Configure ENET1_RX_DATA1 pin as ENET1_RDATA01
   IOMUXC_SetPinMux(IOMUXC_ENET1_RX_DATA1_ENET1_RDATA01, 0);

   //Set ENET1_RX_DATA1 pad properties
   ...
   //Configure GPIO1_IO06 pin as ENET1_MDIO
   IOMUXC_SetPinMux(IOMUXC_GPIO1_IO06_ENET1_MDIO, 0);

   //Set GPIO1_IO06 pad properties
   ...
   //Configure GPIO1_IO07 pin as ENET1_MDC
   IOMUXC_SetPinMux(IOMUXC_GPIO1_IO07_ENET1_MDC, 0);

   //Set GPIO1_IO07 pad properties
   ...
   //Configure SNVS_TAMPER5 pin as GPIO5_IO05
   IOMUXC_SetPinMux(IOMUXC_SNVS_TAMPER5_GPIO5_IO05, 0);

   //Set SNVS_TAMPER5 pad properties
   IOMUXC_SetPinConfig(IOMUXC_SNVS_TAMPER5_GPIO5_IO05,
      IOMUXC_SW_PAD_CTL_PAD_HYS(0) |
      IOMUXC_SW_PAD_CTL_PAD_PUS(2) |
      IOMUXC_SW_PAD_CTL_PAD_PUE(1) |
      IOMUXC_SW_PAD_CTL_PAD_PKE(1) |
   ...
   //Configure SNVS_TAMPER5 pin as GPIO5_IO05
   IOMUXC_SetPinMux(IOMUXC_SNVS_SNVS_TAMPER5_GPIO5_IO05, 0);

   //Set SNVS_TAMPER5 pad properties
   IOMUXC_SetPinConfig(IOMUXC_SNVS_SNVS_TAMPER5_GPIO5_IO05,
      IOMUXC_SW_PAD_CTL_PAD_HYS(0) |
      IOMUXC_SW_PAD_CTL_PAD_PUS(2) |
      IOMUXC_SW_PAD_CTL_PAD_PUE(1) |
      IOMUXC_SW_PAD_CTL_PAD_PKE(1) |
   ...
   //Configure ENET1_INT as an input
...
void mcimx6ulEth1InitBufferDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < MCIMX6UL_ETH1_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < MCIMX6UL_ETH1_RX_BUFFER_COUNT; i++)
   {
   ...
   ENET1->TDSR = (uint32_t) txBufferDesc;
   //Start location of the RX descriptor list
   ENET1->RDSR = (uint32_t) rxBufferDesc;
   //Maximum receive buffer size
   ENET1->MRBR = MCIMX6UL_ETH1_RX_BUFFER_SIZE;
}
...
/**
 * @brief Ethernet MAC interrupt (ENET1 instance)
...
void ENET1_DriverIRQHandler(uint32_t giccIar, void *userParam)
{
   ...
   if((events & ENET_EIR_TXF_MASK) != 0)
   {
      ...
      ENET1->TDAR = ENET_TDAR_TDAR_MASK;
   }
   ...
   if((events & ENET_EIR_EBERR_MASK) != 0)
   {
      ...
      ENET1->EIMR &= ~ENET_EIMR_EBERR_MASK;
...
void mcimx6ulEth1EventHandler(NetInterface *interface)
{
   ...
   if((status & ENET_EIR_RXF_MASK) != 0)
   {
      ...
      ENET1->EIR = ENET_EIR_RXF_MASK;

      //Process all pending packets
      do
      {
         //Read incoming packet
         error = mcimx6ulEth1ReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > MCIMX6UL_ETH1_TX_BUFFER_SIZE)
   {
   ...
   //Check current index
   if(txBufferIndex < (MCIMX6UL_ETH1_TX_BUFFER_COUNT - 1))
   {
   ...
   if((rxBufferDesc[rxBufferIndex][0] & ENET_RBD0_E) == 0)
   {
      ...
      if((rxBufferDesc[rxBufferIndex][0] & ENET_RBD0_L) != 0)
      {
            ...
            ENET_RBD0_CR | ENET_RBD0_OV | ENET_RBD0_TR)) == 0)
         {
            ...
            n = MIN(n, MCIMX6UL_ETH1_RX_BUFFER_SIZE);
      ...
      //Check current index
      if(rxBufferIndex < (MCIMX6UL_ETH1_RX_BUFFER_COUNT - 1))
      {
...
error_t mcimx6ulEth1UpdateMacAddrFilter(NetInterface *interface)
{
   ...
   for(i = 0; i < MAC_ADDR_FILTER_SIZE; i++)
   {
      ...
      if(entry->refCount > 0)
      {
         ...
         crc = mcimx6ulEth1CalcCrc(&entry->addr, sizeof(MacAddr));
   ...
   ENET1->IALR = unicastHashTable[0];
   ENET1->IAUR = unicastHashTable[1];

   //Write the hash table (multicast address filtering)
   ENET1->GALR = multicastHashTable[0];
   ENET1->GAUR = multicastHashTable[1];
...
error_t mcimx6ulEth1UpdateMacConfig(NetInterface *interface)
{
   ...
   {
      //100 Mbps operation
      ENET1->RCR &= ~ENET_RCR_RMII_10T_MASK;
   }
   else
   {
      //10 Mbps operation
   ...
   {
      //Full-duplex mode
      ENET1->TCR |= ENET_TCR_FDEN_MASK;
      //Receive path operates independently of transmit
      ENET1->RCR &= ~ENET_RCR_DRT_MASK;
   }
   else
   {
      //Half-duplex mode
...
void mcimx6ulEth1WritePhyReg(uint8_t opcode, uint8_t phyAddr,
...
uint16_t mcimx6ulEth1ReadPhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr)
{
   uint16_t data;
   uint32_t temp;
