...
#include "core/net.h"
#include "drivers/mac/rm57_eth_driver.h"
#include "debug.h"
...
#pragma location = RM57_ETH_RAM_SECTION
static uint8_t txBuffer[RM57_ETH_TX_BUFFER_COUNT][RM57_ETH_TX_BUFFER_SIZE];
//Receive buffer
#pragma data_alignment = 4
#pragma location = RM57_ETH_RAM_SECTION
static uint8_t rxBuffer[RM57_ETH_RX_BUFFER_COUNT][RM57_ETH_RX_BUFFER_SIZE];
//Transmit buffer descriptors
#pragma data_alignment = 4
#pragma location = RM57_ETH_RAM_CPPI_SECTION
static Rm57TxBufferDesc txBufferDesc[RM57_ETH_TX_BUFFER_COUNT];
//Receive buffer descriptors
#pragma data_alignment = 4
#pragma location = RM57_ETH_RAM_CPPI_SECTION
static Rm57RxBufferDesc rxBufferDesc[RM57_ETH_RX_BUFFER_COUNT];
...
//Transmit buffer
static uint8_t txBuffer[RM57_ETH_TX_BUFFER_COUNT][RM57_ETH_TX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(RM57_ETH_RAM_SECTION)));
//Receive buffer
static uint8_t rxBuffer[RM57_ETH_RX_BUFFER_COUNT][RM57_ETH_RX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(RM57_ETH_RAM_SECTION)));
//Transmit buffer descriptors
static Rm57TxBufferDesc txBufferDesc[RM57_ETH_TX_BUFFER_COUNT]
   __attribute__((aligned(4), __section__(RM57_ETH_RAM_CPPI_SECTION)));
//Receive buffer descriptors
static Rm57RxBufferDesc rxBufferDesc[RM57_ETH_RX_BUFFER_COUNT]
   __attribute__((aligned(4), __section__(RM57_ETH_RAM_CPPI_SECTION)));
...
static Rm57TxBufferDesc *txCurBufferDesc;
//Pointer to the current RX buffer descriptor
static Rm57RxBufferDesc *rxCurBufferDesc;
...
/**
 * @brief RM57 Ethernet MAC initialization
...
error_t rm57EthInit(NetInterface *interface)
{
   ...
   vimChannelMap(RM57_ETH_TX_IRQ_CHANNEL, RM57_ETH_TX_IRQ_CHANNEL,
      (t_isrFuncPTR) rm57EthTxIrqHandler);

   //Register RX interrupt handler
   vimChannelMap(RM57_ETH_RX_IRQ_CHANNEL, RM57_ETH_RX_IRQ_CHANNEL,
      (t_isrFuncPTR) rm57EthRxIrqHandler);
   ...
   rm57EthInitBufferDesc(interface);
...
void rm57EthInitBufferDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < RM57_ETH_TX_BUFFER_COUNT; i++)
   {
      //Index of the next buffer
      nextIndex = (i + 1) % RM57_ETH_TX_BUFFER_COUNT;
      //Index of the previous buffer
      prevIndex = (i + RM57_ETH_TX_BUFFER_COUNT - 1) % RM57_ETH_TX_BUFFER_COUNT;
   ...
   for(i = 0; i < RM57_ETH_RX_BUFFER_COUNT; i++)
   {
      //Index of the next buffer
      nextIndex = (i + 1) % RM57_ETH_RX_BUFFER_COUNT;
      //Index of the previous buffer
      prevIndex = (i + RM57_ETH_RX_BUFFER_COUNT - 1) % RM57_ETH_RX_BUFFER_COUNT;
      ...
      //Buffer offset and buffer length
      rxBufferDesc[i].word2 = RM57_ETH_RX_BUFFER_SIZE;
      //Status flags and packet length
...
/**
 * @brief RM57 Ethernet MAC timer handler
 *
...
void rm57EthEnableIrq(NetInterface *interface)
{
   ...
   vimEnableInterrupt(RM57_ETH_TX_IRQ_CHANNEL, SYS_IRQ);
   vimEnableInterrupt(RM57_ETH_RX_IRQ_CHANNEL, SYS_IRQ);
...
void rm57EthDisableIrq(NetInterface *interface)
{
   ...
   vimDisableInterrupt(RM57_ETH_TX_IRQ_CHANNEL);
   vimDisableInterrupt(RM57_ETH_RX_IRQ_CHANNEL);
 ...
 **/
   ...
   __irq __arm
#else
   #pragma CODE_STATE(rm57EthTxIrqHandler, 32)
   #pragma INTERRUPT(rm57EthTxIrqHandler, IRQ)
#endif
...
void rm57EthTxIrqHandler(void)
{
   ...
   if(status & (1 << EMAC_CH0))
   {
      ...
      p = (Rm57TxBufferDesc *) EMAC_TXCP_R(EMAC_CH0);
 ...
 **/
   ...
   __irq __arm
#else
   #pragma CODE_STATE(rm57EthRxIrqHandler, 32)
   #pragma INTERRUPT(rm57EthRxIrqHandler, IRQ)
#endif
...
/**
 * @brief RM57 Ethernet MAC event handler
 * @param[in] interface Underlying network interface
 **/
...
void rm57EthEventHandler(NetInterface *interface)
{
   ...
   do
   {
      ...
      error = rm57EthReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > RM57_ETH_TX_BUFFER_SIZE)
   {
   ...
   netBufferRead((uint8_t *) txCurBufferDesc->word1, buffer,
      offset, length);
...
{
   static uint32_t buffer[RM57_ETH_RX_BUFFER_SIZE / 4];
   error_t error;
   size_t n;
   uint32_t temp;
   ...
   if((rxCurBufferDesc->word3 & EMAC_RX_WORD3_OWNER) == 0)
   {
         ...
         (rxCurBufferDesc->word3 & EMAC_RX_WORD3_EOP) != 0)
      {
         ...
         if((rxCurBufferDesc->word3 & EMAC_RX_WORD3_ERROR_MASK) == 0)
         {
            ...
            n = MIN(n, RM57_ETH_RX_BUFFER_SIZE);
      ...
      rxCurBufferDesc->word2 = RM57_ETH_RX_BUFFER_SIZE;
