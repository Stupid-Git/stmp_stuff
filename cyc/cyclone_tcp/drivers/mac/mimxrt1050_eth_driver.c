...
#pragma location = MIMXRT1050_ETH_RAM_SECTION
static uint8_t txBuffer[MIMXRT1050_ETH_TX_BUFFER_COUNT][MIMXRT1050_ETH_TX_BUFFER_SIZE];
//RX buffer
#pragma data_alignment = 64
#pragma location = MIMXRT1050_ETH_RAM_SECTION
static uint8_t rxBuffer[MIMXRT1050_ETH_RX_BUFFER_COUNT][MIMXRT1050_ETH_RX_BUFFER_SIZE];
//TX buffer descriptors
#pragma data_alignment = 64
#pragma location = MIMXRT1050_ETH_RAM_SECTION
static uint32_t txBufferDesc[MIMXRT1050_ETH_TX_BUFFER_COUNT][8];
//RX buffer descriptors
#pragma data_alignment = 64
#pragma location = MIMXRT1050_ETH_RAM_SECTION
static uint32_t rxBufferDesc[MIMXRT1050_ETH_RX_BUFFER_COUNT][8];
...
//TX buffer
static uint8_t txBuffer[MIMXRT1050_ETH_TX_BUFFER_COUNT][MIMXRT1050_ETH_TX_BUFFER_SIZE]
   __attribute__((aligned(64), __section__(MIMXRT1050_ETH_RAM_SECTION)));
//RX buffer
static uint8_t rxBuffer[MIMXRT1050_ETH_RX_BUFFER_COUNT][MIMXRT1050_ETH_RX_BUFFER_SIZE]
   __attribute__((aligned(64), __section__(MIMXRT1050_ETH_RAM_SECTION)));
//TX buffer descriptors
...
/**
 * @brief i.MX RT1050 Ethernet MAC driver
 **/
...
const NicDriver mimxrt1050EthDriver =
{
   ...
   mimxrt1050EthInit,
   mimxrt1050EthTick,
   mimxrt1050EthEnableIrq,
   mimxrt1050EthDisableIrq,
   mimxrt1050EthEventHandler,
   mimxrt1050EthSendPacket,
   mimxrt1050EthUpdateMacAddrFilter,
   mimxrt1050EthUpdateMacConfig,
   mimxrt1050EthWritePhyReg,
   mimxrt1050EthReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
   FALSE
...
/**
 * @brief i.MX RT1050 Ethernet MAC initialization
...
error_t mimxrt1050EthInit(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing i.MX RT1050 Ethernet MAC...\r\n");
   ...
   //GPIO configuration
   mimxrt1050EthInitGpio(interface);
   ...
   ENET->RCR = ENET_RCR_MAX_FL(MIMXRT1050_ETH_RX_BUFFER_SIZE) |
   ...
   mimxrt1050EthInitBufferDesc(interface);
   ...
   NVIC_SetPriorityGrouping(MIMXRT1050_ETH_IRQ_PRIORITY_GROUPING);

   //Configure ENET interrupt priority
   NVIC_SetPriority(ENET_IRQn, NVIC_EncodePriority(MIMXRT1050_ETH_IRQ_PRIORITY_GROUPING,
      MIMXRT1050_ETH_IRQ_GROUP_PRIORITY, MIMXRT1050_ETH_IRQ_SUB_PRIORITY));
...
{
//MIMXRT1050-EVK or MIMXRT1050-EVKB evaluation board?
#if defined(USE_MIMXRT1050_EVK) || defined(USE_MIMXRT1050_EVKB)
...
void mimxrt1050EthInitBufferDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < MIMXRT1050_ETH_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < MIMXRT1050_ETH_RX_BUFFER_COUNT; i++)
   {
   ...
   ENET->MRBR = MIMXRT1050_ETH_RX_BUFFER_SIZE;
}


/**
 * @brief i.MX RT1050 Ethernet MAC timer handler
 *
...
/**
 * @brief i.MX RT1050 Ethernet MAC event handler
 * @param[in] interface Underlying network interface
 **/
...
void mimxrt1050EthEventHandler(NetInterface *interface)
{
   ...
   if((status & ENET_EIR_RXF_MASK) != 0)
   {
      ...
      do
      {
         ...
         error = mimxrt1050EthReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > MIMXRT1050_ETH_TX_BUFFER_SIZE)
   {
   ...
   //Check current index
   if(txBufferIndex < (MIMXRT1050_ETH_TX_BUFFER_COUNT - 1))
   {
   ...
   if((rxBufferDesc[rxBufferIndex][0] & ENET_RBD0_E) == 0)
   {
      ...
      if((rxBufferDesc[rxBufferIndex][0] & ENET_RBD0_L) != 0)
      {
            ...
            ENET_RBD0_CR | ENET_RBD0_OV | ENET_RBD0_TR)) == 0)
         {
            ...
            n = MIN(n, MIMXRT1050_ETH_RX_BUFFER_SIZE);
      ...
      //Check current index
      if(rxBufferIndex < (MIMXRT1050_ETH_RX_BUFFER_COUNT - 1))
      {
...
error_t mimxrt1050EthUpdateMacAddrFilter(NetInterface *interface)
{
   ...
   for(i = 0; i < MAC_ADDR_FILTER_SIZE; i++)
   {
      ...
      if(entry->refCount > 0)
      {
         ...
         crc = mimxrt1050EthCalcCrc(&entry->addr, sizeof(MacAddr));
