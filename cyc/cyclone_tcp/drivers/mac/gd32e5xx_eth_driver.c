...
#include "gd32e50x.h"
#include "core/net.h"
#include "drivers/mac/gd32e5xx_eth_driver.h"
#include "debug.h"
...
static uint8_t txBuffer[GD32E5XX_ETH_TX_BUFFER_COUNT][GD32E5XX_ETH_TX_BUFFER_SIZE];
//Receive buffer
#pragma data_alignment = 4
static uint8_t rxBuffer[GD32E5XX_ETH_RX_BUFFER_COUNT][GD32E5XX_ETH_RX_BUFFER_SIZE];
//Transmit DMA descriptors
#pragma data_alignment = 4
static Gd32e5xxTxDmaDesc txDmaDesc[GD32E5XX_ETH_TX_BUFFER_COUNT];
//Receive DMA descriptors
#pragma data_alignment = 4
static Gd32e5xxRxDmaDesc rxDmaDesc[GD32E5XX_ETH_RX_BUFFER_COUNT];
...
static Gd32e5xxTxDmaDesc *txCurDmaDesc;
//Pointer to the current RX DMA descriptor
static Gd32e5xxRxDmaDesc *rxCurDmaDesc;
...
const NicDriver gd32e5xxEthDriver =
{
   ...
   gd32e5xxEthInit,
   gd32e5xxEthTick,
   gd32e5xxEthEnableIrq,
   gd32e5xxEthDisableIrq,
   gd32e5xxEthEventHandler,
   gd32e5xxEthSendPacket,
   gd32e5xxEthUpdateMacAddrFilter,
   gd32e5xxEthUpdateMacConfig,
   gd32e5xxEthWritePhyReg,
   gd32e5xxEthReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
   FALSE
...
error_t gd32e5xxEthInit(NetInterface *interface)
{
   ...
   NVIC_SetPriorityGrouping(GD32E5XX_ETH_IRQ_PRIORITY_GROUPING);

   //Configure Ethernet interrupt priority
   NVIC_SetPriority(ENET_IRQn, NVIC_EncodePriority(GD32E5XX_ETH_IRQ_PRIORITY_GROUPING,
      GD32E5XX_ETH_IRQ_GROUP_PRIORITY, GD32E5XX_ETH_IRQ_SUB_PRIORITY));
...
{
//GD32E507Z-EVAL evaluation board?
#if defined(USE_GD32E507Z_EVAL)
   //Enable SYSCFG clock
...
void gd32e5xxEthInitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < GD32E5XX_ETH_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < GD32E5XX_ETH_RX_BUFFER_COUNT; i++)
   {
      //The descriptor is initially owned by the DMA
      rxDmaDesc[i].rdes0 = ENET_RDES0_DAV;
      //Use chain structure rather than ring structure
      rxDmaDesc[i].rdes1 = ENET_RDES1_RCHM | (GD32E5XX_ETH_RX_BUFFER_SIZE & ENET_RDES1_RB1S);
...
void gd32e5xxEthEventHandler(NetInterface *interface)
{
   ...
   do
   {
      ...
      error = gd32e5xxEthReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > GD32E5XX_ETH_TX_BUFFER_SIZE)
   {
   ...
   txCurDmaDesc = (Gd32e5xxTxDmaDesc *) txCurDmaDesc->tdes3;
   ...
   if((rxCurDmaDesc->rdes0 & ENET_RDES0_DAV) == 0)
   {
         ...
         (rxCurDmaDesc->rdes0 & ENET_RDES0_LDES) != 0)
      {
         ...
         if((rxCurDmaDesc->rdes0 & ENET_RDES0_ERRS) == 0)
         {
            ...
            n = MIN(n, GD32E5XX_ETH_RX_BUFFER_SIZE);
      ...
      //Point to the next descriptor in the list
      rxCurDmaDesc = (Gd32e5xxRxDmaDesc *) rxCurDmaDesc->rdes3;
   }
   else
   {
      //No more data in the receive buffer
   ...
   for(i = 0, j = 0; i < MAC_ADDR_FILTER_SIZE; i++)
   {
      ...
      if(entry->refCount > 0)
      {
         ...
         if(macIsMulticastAddr(&entry->addr))
         {
            ...
            crc = gd32e5xxEthCalcCrc(&entry->addr, sizeof(MacAddr));
