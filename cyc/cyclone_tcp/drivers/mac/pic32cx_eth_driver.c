...
static uint8_t txBuffer[PIC32CX_ETH_TX_BUFFER_COUNT][PIC32CX_ETH_TX_BUFFER_SIZE];
//RX buffer
#pragma data_alignment = 8
static uint8_t rxBuffer[PIC32CX_ETH_RX_BUFFER_COUNT][PIC32CX_ETH_RX_BUFFER_SIZE];
//TX buffer descriptors
#pragma data_alignment = 4
static Pic32cxTxBufferDesc txBufferDesc[PIC32CX_ETH_TX_BUFFER_COUNT];
//RX buffer descriptors
#pragma data_alignment = 4
static Pic32cxRxBufferDesc rxBufferDesc[PIC32CX_ETH_RX_BUFFER_COUNT];
...
const NicDriver pic32cxEthDriver =
{
   ...
   pic32cxEthTick,
   pic32cxEthEnableIrq,
   pic32cxEthDisableIrq,
   pic32cxEthEventHandler,
   pic32cxEthSendPacket,
   pic32cxEthUpdateMacAddrFilter,
   pic32cxEthUpdateMacConfig,
   pic32cxEthWritePhyReg,
   pic32cxEthReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
   FALSE
...
/**
 * @brief PIC32CX Ethernet MAC initialization
...
error_t pic32cxEthInit(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing PIC32CX Ethernet MAC...\r\n");
   ...
   pic32cxEthInitBufferDesc(interface);
   ...
   NVIC_SetPriority(GMAC_IRQn, NVIC_EncodePriority(PIC32CX_ETH_IRQ_PRIORITY_GROUPING,
      PIC32CX_ETH_IRQ_GROUP_PRIORITY, PIC32CX_ETH_IRQ_SUB_PRIORITY));
...
void pic32cxEthInitBufferDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < PIC32CX_ETH_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < PIC32CX_ETH_RX_BUFFER_COUNT; i++)
   {
...
/**
 * @brief PIC32CX Ethernet MAC timer handler
 *
...
/**
 * @brief PIC32CX Ethernet MAC interrupt service routine
 **/
...
/**
 * @brief PIC32CX Ethernet MAC event handler
 * @param[in] interface Underlying network interface
 **/
...
void pic32cxEthEventHandler(NetInterface *interface)
{
      ...
      GMAC_RSR_BNA_Msk)) != 0)
   {
      ...
      do
      {
         ...
         error = pic32cxEthReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > PIC32CX_ETH_TX_BUFFER_SIZE)
   {
   ...
   if(txBufferIndex < (PIC32CX_ETH_TX_BUFFER_COUNT - 1))
   {
...
error_t pic32cxEthReceivePacket(NetInterface *interface)
{
   ...
   for(i = 0; i < PIC32CX_ETH_RX_BUFFER_COUNT; i++)
   {
      ...
      if(j >= PIC32CX_ETH_RX_BUFFER_COUNT)
      {
         j -= PIC32CX_ETH_RX_BUFFER_COUNT;
      }
   ...
   for(i = 0; i < j; i++)
   {
      ...
      if(eofIndex != UINT_MAX && i >= sofIndex && i <= eofIndex)
      {
         ...
         n = MIN(size, PIC32CX_ETH_RX_BUFFER_SIZE);
      ...
      //Wrap around to the beginning of the buffer if necessary
      if(rxBufferIndex >= PIC32CX_ETH_RX_BUFFER_COUNT)
      {
         rxBufferIndex = 0;
      }
   }
