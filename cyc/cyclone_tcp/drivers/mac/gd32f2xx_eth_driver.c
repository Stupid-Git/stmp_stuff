...
#include "gd32f20x.h"
#include "core/net.h"
#include "drivers/mac/gd32f2xx_eth_driver.h"
#include "debug.h"
...
static uint8_t txBuffer[GD32F2XX_ETH_TX_BUFFER_COUNT][GD32F2XX_ETH_TX_BUFFER_SIZE];
//Receive buffer
#pragma data_alignment = 4
static uint8_t rxBuffer[GD32F2XX_ETH_RX_BUFFER_COUNT][GD32F2XX_ETH_RX_BUFFER_SIZE];
//Transmit DMA descriptors
#pragma data_alignment = 4
static Gd32f2xxTxDmaDesc txDmaDesc[GD32F2XX_ETH_TX_BUFFER_COUNT];
//Receive DMA descriptors
#pragma data_alignment = 4
static Gd32f2xxRxDmaDesc rxDmaDesc[GD32F2XX_ETH_RX_BUFFER_COUNT];
...
static Gd32f2xxTxDmaDesc *txCurDmaDesc;
//Pointer to the current RX DMA descriptor
static Gd32f2xxRxDmaDesc *rxCurDmaDesc;


/**
 * @brief GD32F2XX Ethernet MAC driver
 **/
...
const NicDriver gd32f2xxEthDriver =
{
   ...
   gd32f2xxEthInit,
   gd32f2xxEthTick,
   gd32f2xxEthEnableIrq,
   gd32f2xxEthDisableIrq,
   gd32f2xxEthEventHandler,
   gd32f2xxEthSendPacket,
   gd32f2xxEthUpdateMacAddrFilter,
   gd32f2xxEthUpdateMacConfig,
   gd32f2xxEthWritePhyReg,
   gd32f2xxEthReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
   FALSE
...
/**
 * @brief GD32F2XX Ethernet MAC initialization
...
error_t gd32f2xxEthInit(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing GD32F2XX Ethernet MAC...\r\n");

   //Save underlying network interface
   nicDriverInterface = interface;

   //GPIO configuration
   gd32f2xxEthInitGpio(interface);
   ...
   ENET_DMA_BCTL = ENET_DMA_BCTL_AA | ENET_DMA_BCTL_UIP | ENET_RXDP_32BEAT |
      ENET_ARBITRATION_RXTX_1_1 | ENET_PGBL_32BEAT;

   //Initialize DMA descriptor lists
   gd32f2xxEthInitDmaDesc(interface);
   ...
   NVIC_SetPriorityGrouping(GD32F2XX_ETH_IRQ_PRIORITY_GROUPING);

   //Configure Ethernet interrupt priority
   NVIC_SetPriority(ENET_IRQn, NVIC_EncodePriority(GD32F2XX_ETH_IRQ_PRIORITY_GROUPING,
      GD32F2XX_ETH_IRQ_GROUP_PRIORITY, GD32F2XX_ETH_IRQ_SUB_PRIORITY));
...
{
//GD32207C-EVAL or GD32207I-EVAL evaluation board?
#if defined(USE_GD32207C_EVAL) || defined(USE_GD32207I_EVAL)
   //Enable SYSCFG clock
...
void gd32f2xxEthInitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < GD32F2XX_ETH_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < GD32F2XX_ETH_RX_BUFFER_COUNT; i++)
   {
      //The descriptor is initially owned by the DMA
      rxDmaDesc[i].rdes0 = ENET_RDES0_DAV;
      //Use chain structure rather than ring structure
      rxDmaDesc[i].rdes1 = ENET_RDES1_RCHM | (GD32F2XX_ETH_RX_BUFFER_SIZE & ENET_RDES1_RB1S);
...
/**
 * @brief GD32F2XX Ethernet MAC timer handler
 *
...
/**
 * @brief GD32F2XX Ethernet MAC interrupt service routine
 **/
...
/**
 * @brief GD32F2XX Ethernet MAC event handler
 * @param[in] interface Underlying network interface
 **/
...
void gd32f2xxEthEventHandler(NetInterface *interface)
{
   ...
   do
   {
      ...
      error = gd32f2xxEthReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > GD32F2XX_ETH_TX_BUFFER_SIZE)
   {
   ...
   txCurDmaDesc = (Gd32f2xxTxDmaDesc *) txCurDmaDesc->tdes3;
   ...
   if((rxCurDmaDesc->rdes0 & ENET_RDES0_DAV) == 0)
   {
         ...
         (rxCurDmaDesc->rdes0 & ENET_RDES0_LDES) != 0)
      {
         ...
         if((rxCurDmaDesc->rdes0 & ENET_RDES0_ERRS) == 0)
         {
            ...
            n = MIN(n, GD32F2XX_ETH_RX_BUFFER_SIZE);
      ...
      //Point to the next descriptor in the list
      rxCurDmaDesc = (Gd32f2xxRxDmaDesc *) rxCurDmaDesc->rdes3;
   }
   else
   {
      //No more data in the receive buffer
   ...
   for(i = 0, j = 0; i < MAC_ADDR_FILTER_SIZE; i++)
   {
      ...
      if(entry->refCount > 0)
      {
         ...
         if(macIsMulticastAddr(&entry->addr))
         {
            ...
            crc = gd32f2xxEthCalcCrc(&entry->addr, sizeof(MacAddr));
