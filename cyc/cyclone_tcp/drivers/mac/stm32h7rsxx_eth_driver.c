/**
 * @file stm32h7rsxx_eth_driver.c
 * @brief STM32H7Rx/Sx Ethernet MAC driver
 *
 * @section License
 *
...
#pragma location = STM32H7RSXX_ETH_RAM_SECTION
static uint8_t txBuffer[STM32H7RSXX_ETH_TX_BUFFER_COUNT][STM32H7RSXX_ETH_TX_BUFFER_SIZE];
//Receive buffer
#pragma data_alignment = 4
#pragma location = STM32H7RSXX_ETH_RAM_SECTION
static uint8_t rxBuffer[STM32H7RSXX_ETH_RX_BUFFER_COUNT][STM32H7RSXX_ETH_RX_BUFFER_SIZE];
//Transmit DMA descriptors
#pragma data_alignment = 4
#pragma location = STM32H7RSXX_ETH_RAM_SECTION
static Stm32h7rsxxTxDmaDesc txDmaDesc[STM32H7RSXX_ETH_TX_BUFFER_COUNT];
//Receive DMA descriptors
#pragma data_alignment = 4
#pragma location = STM32H7RSXX_ETH_RAM_SECTION
static Stm32h7rsxxRxDmaDesc rxDmaDesc[STM32H7RSXX_ETH_RX_BUFFER_COUNT];
...
//Transmit buffer
static uint8_t txBuffer[STM32H7RSXX_ETH_TX_BUFFER_COUNT][STM32H7RSXX_ETH_TX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(STM32H7RSXX_ETH_RAM_SECTION)));
//Receive buffer
static uint8_t rxBuffer[STM32H7RSXX_ETH_RX_BUFFER_COUNT][STM32H7RSXX_ETH_RX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(STM32H7RSXX_ETH_RAM_SECTION)));
//Transmit DMA descriptors
...
/**
 * @brief STM32H7Rx/Sx Ethernet MAC initialization
...
error_t stm32h7rsxxEthInit(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing STM32H7Rx/Sx Ethernet MAC...\r\n");

   //Save underlying network interface
   nicDriverInterface = interface;

   //GPIO configuration
   stm32h7rsxxEthInitGpio(interface);
   ...
   ETH->MACECR = temp | STM32H7RSXX_ETH_RX_BUFFER_SIZE;

   //Configure MAC address filtering
   stm32h7rsxxEthUpdateMacAddrFilter(interface);
   ...
   ETH->DMACRCR |= (STM32H7RSXX_ETH_RX_BUFFER_SIZE << 1) & ETH_DMACRCR_RBSZ;
   ...
   stm32h7rsxxEthInitDmaDesc(interface);
   ...
   NVIC_SetPriorityGrouping(STM32H7RSXX_ETH_IRQ_PRIORITY_GROUPING);

   //Configure Ethernet interrupt priority
   NVIC_SetPriority(ETH_IRQn, NVIC_EncodePriority(STM32H7RSXX_ETH_IRQ_PRIORITY_GROUPING,
      STM32H7RSXX_ETH_IRQ_GROUP_PRIORITY, STM32H7RSXX_ETH_IRQ_SUB_PRIORITY));
...
{
//Nucleo-H7S3L8 evaluation board?
   ...
   //Configure ETH_MDIO (PA2) and ETH_RMII_CRS_DV (PA7)
   GPIO_InitStructure.Pin = GPIO_PIN_2 | GPIO_PIN_7;
   HAL_GPIO_Init(GPIOA, &GPIO_InitStructure);

   //Configure ETH_RMII_REF_CLK (PB6)
   GPIO_InitStructure.Pin = GPIO_PIN_6;
   HAL_GPIO_Init(GPIOB, &GPIO_InitStructure);

   //Configure ETH_RMII_RXD0 (PG4), ETH_RMII_RXD1 (PG5), ETH_MDC (PG6),
   //RMII_TX_EN (PG11), ETH_RMII_TXD1 (PG12) and ETH_RMII_TXD0 (PG13)
   GPIO_InitStructure.Pin = GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_6 |
      GPIO_PIN_11 | GPIO_PIN_12 | GPIO_PIN_13;
   HAL_GPIO_Init(GPIOG, &GPIO_InitStructure);

//STM32H7S78-DK evaluation board?
#elif defined(USE_STM32H7S78_DK)
   GPIO_InitTypeDef GPIO_InitStructure;
...
void stm32h7rsxxEthInitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < STM32H7RSXX_ETH_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < STM32H7RSXX_ETH_RX_BUFFER_COUNT; i++)
   {
   ...
   ETH->DMACTDRLR = STM32H7RSXX_ETH_TX_BUFFER_COUNT - 1;

   //Start location of the RX descriptor list
   ETH->DMACRDLAR = (uint32_t) &rxDmaDesc[0];
   //Length of the receive descriptor ring
   ETH->DMACRDRLR = STM32H7RSXX_ETH_RX_BUFFER_COUNT - 1;
}


/**
 * @brief STM32H7Rx/Sx Ethernet MAC timer handler
 *
...
/**
 * @brief STM32H7Rx/Sx Ethernet MAC interrupt service routine
 **/
...
/**
 * @brief STM32H7Rx/Sx Ethernet MAC event handler
 * @param[in] interface Underlying network interface
 **/
...
void stm32h7rsxxEthEventHandler(NetInterface *interface)
{
   ...
   do
   {
      ...
      error = stm32h7rsxxEthReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > STM32H7RSXX_ETH_TX_BUFFER_SIZE)
   {
   ...
   if(++txIndex >= STM32H7RSXX_ETH_TX_BUFFER_COUNT)
   {
      txIndex = 0;
   }
...
error_t stm32h7rsxxEthReceivePacket(NetInterface *interface)
{
   ...
   if((rxDmaDesc[rxIndex].rdes3 & ETH_RDES3_OWN) == 0)
   {
         ...
         (rxDmaDesc[rxIndex].rdes3 & ETH_RDES3_LD) != 0)
      {
         ...
         if((SBS->PMCR & SBS_PMCR_ETH_PHYSEL) != SBS_ETHERNET_PHY_GMII_OR_MII)
         {
            status &= ~ETH_RDES3_DE;
         }
         ...
         if(status == 0)
         {
            ...
            n = MIN(n, STM32H7RSXX_ETH_RX_BUFFER_SIZE);
      ...
      //Increment index and wrap around if necessary
      if(++rxIndex >= STM32H7RSXX_ETH_RX_BUFFER_COUNT)
      {
         rxIndex = 0;
      }
   }
      ...
      for(i = 0, j = 0; i < MAC_ADDR_FILTER_SIZE; i++)
      {
         ...
         if(entry->refCount > 0)
         {
            ...
            if(macIsMulticastAddr(&entry->addr))
            {
               ...
               crc = stm32h7rsxxEthCalcCrc(&entry->addr, sizeof(MacAddr));
...
void stm32h7rsxxEthWritePhyReg(uint8_t opcode, uint8_t phyAddr,
...
uint16_t stm32h7rsxxEthReadPhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr)
{
   uint16_t data;
   uint32_t temp;
