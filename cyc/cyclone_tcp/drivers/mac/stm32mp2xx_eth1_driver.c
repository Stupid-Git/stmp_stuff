/**
 * @file stm32mp2xx_eth1_driver.c
 * @brief STM32MP2 Gigabit Ethernet MAC driver (ETH1 instance)
 *
 * @section License
 *
...
#include "core/net.h"
#include "drivers/mac/stm32mp2xx_eth1_driver.h"
#include "debug.h"
...
static uint8_t txBuffer[STM32MP2XX_ETH1_TX_BUFFER_COUNT][STM32MP2XX_ETH1_TX_BUFFER_SIZE];
//Receive buffer
#pragma data_alignment = 4
static uint8_t rxBuffer[STM32MP2XX_ETH1_RX_BUFFER_COUNT][STM32MP2XX_ETH1_RX_BUFFER_SIZE];
//Transmit DMA descriptors
#pragma data_alignment = 8
static Stm32mp2xxEth1TxDmaDesc txDmaDesc[STM32MP2XX_ETH1_TX_BUFFER_COUNT];
//Receive DMA descriptors
#pragma data_alignment = 8
static Stm32mp2xxEth1RxDmaDesc rxDmaDesc[STM32MP2XX_ETH1_RX_BUFFER_COUNT];
...
/**
 * @brief STM32MP2 Ethernet MAC driver (ETH1 instance)
 **/
...
const NicDriver stm32mp2xxEth1Driver =
{
   ...
   stm32mp2xxEth1Init,
   stm32mp2xxEth1Tick,
   stm32mp2xxEth1EnableIrq,
   stm32mp2xxEth1DisableIrq,
   stm32mp2xxEth1EventHandler,
   stm32mp2xxEth1SendPacket,
   stm32mp2xxEth1UpdateMacAddrFilter,
   stm32mp2xxEth1UpdateMacConfig,
   stm32mp2xxEth1WritePhyReg,
   stm32mp2xxEth1ReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
   FALSE
...
error_t stm32mp2xxEth1Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing STM32MP2 Ethernet MAC (ETH1)...\r\n");

   //Save underlying network interface
   nicDriverInterface = interface;

   //GPIO configuration
   stm32mp2xxEth1InitGpio(interface);
   ...
   ETH1->MACMDIOAR = ETH_MACMDIOAR_CR_DIV124;
   ...
   ETH1->MACECR = temp | STM32MP2XX_ETH1_RX_BUFFER_SIZE;

   //Configure MAC address filtering
   stm32mp2xxEth1UpdateMacAddrFilter(interface);
   ...
   ETH1->DMAC0CR = ETH_DMAC0CR_DSL_Val(0);
   //Configure TX features
   ETH1->DMAC0TXCR = ETH_DMAC0TXCR_TXPBL_Val(32);

   //Configure RX features
   ETH1->DMAC0RXCR = ETH_DMAC0RXCR_RXPBL_Val(32) |
      ETH_DMAC0RXCR_RBSZ_Val(STM32MP2XX_ETH1_RX_BUFFER_SIZE);

   //Enable store and forward mode for transmission
   ETH1->MTLTXQ0OMR = ETH_MTLTXQ0OMR_TQS_Val(7) | ETH_MTLTXQ0OMR_TXQEN_Val(2) |
      ETH_MTLTXQ0OMR_TSF;

   //Enable store and forward mode for reception
   ETH1->MTLRXQ0OMR = ETH_MTLRXQ0OMR_RQS_Val(7) | ETH_MTLRXQ0OMR_RSF;

   //Initialize DMA descriptor lists
   stm32mp2xxEth1InitDmaDesc(interface);

   //Prevent interrupts from being generated when the transmit statistic
   //counters reach half their maximum value
   ETH1->MMCTXIMR = ETH_MMCTXIMR_TXLPITRCIM | ETH_MMCTXIMR_TXLPIUSCIM |
   ...
   //counters reach half their maximum value
   ETH1->MMCRXIMR = ETH_MMCRXIMR_RXLPITRCIM | ETH_MMCRXIMR_RXLPIUSCIM |
      ETH_MMCRXIMR_RXUCGPIM | ETH_MMCRXIMR_RXALGNERPIM | ETH_MMCRXIMR_RXCRCERPIM;

   //Disable MAC interrupts
   ETH1->MACIER = 0;
   //Enable the desired DMA interrupts
   ETH1->DMAC0IER = ETH_DMAC0IER_NIE | ETH_DMAC0IER_RIE | ETH_DMAC0IER_TIE;

   //Set priority grouping (4 bits for pre-emption priority, no bits for subpriority)
   NVIC_SetPriorityGrouping(STM32MP2XX_ETH1_IRQ_PRIORITY_GROUPING);

   //Configure Ethernet interrupt priority
   NVIC_SetPriority(ETH1_SBD_IRQn, NVIC_EncodePriority(STM32MP2XX_ETH1_IRQ_PRIORITY_GROUPING,
      STM32MP2XX_ETH1_IRQ_GROUP_PRIORITY, STM32MP2XX_ETH1_IRQ_SUB_PRIORITY));

   //Enable MAC transmission and reception
   ETH1->MACCR |= ETH_MACCR_TE | ETH_MACCR_RE;

   //Enable DMA transmission and reception
   ETH1->DMAC0TXCR |= ETH_DMAC0TXCR_ST;
   ETH1->DMAC0RXCR |= ETH_DMAC0RXCR_SR;
...
void stm32mp2xxEth1InitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < STM32MP2XX_ETH1_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < STM32MP2XX_ETH1_RX_BUFFER_COUNT; i++)
   {
   ...
   ETH1->DMAC0TXDLAR = (uint32_t) &txDmaDesc[0];
   //Length of the transmit descriptor ring
   ETH1->DMAC0TXRLR = STM32MP2XX_ETH1_TX_BUFFER_COUNT - 1;

   //Start location of the RX descriptor list
   ETH1->DMAC0RXDLAR = (uint32_t) &rxDmaDesc[0];
   //Length of the receive descriptor ring
   ETH1->DMAC0RXRLR = STM32MP2XX_ETH1_RX_BUFFER_COUNT - 1;
}
...
void stm32mp2xxEth1EventHandler(NetInterface *interface)
{
   ...
   do
   {
      ...
      error = stm32mp2xxEth1ReceivePacket(interface);
...
error_t stm32mp2xxEth1SendPacket(NetInterface *interface,
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > STM32MP2XX_ETH1_TX_BUFFER_SIZE)
   {
   ...
   ETH1->DMAC0SR = ETH_DMAC0SR_TBU;
   //Instruct the DMA to poll the transmit descriptor list
   ETH1->DMAC0TXDTPR = 0;

   //Increment index and wrap around if necessary
   if(++txIndex >= STM32MP2XX_ETH1_TX_BUFFER_COUNT)
   {
      txIndex = 0;
   }
   ...
   if((rxDmaDesc[rxIndex].rdes3 & ETH_RDES3_OWN) == 0)
   {
         ...
         (rxDmaDesc[rxIndex].rdes3 & ETH_RDES3_LD) != 0)
      {
         ...
         if((rxDmaDesc[rxIndex].rdes3 & ETH_RDES3_ES) == 0)
         {
            ...
            n = MIN(n, STM32MP2XX_ETH1_RX_BUFFER_SIZE);
      ...
      //Increment index and wrap around if necessary
      if(++rxIndex >= STM32MP2XX_ETH1_RX_BUFFER_COUNT)
      {
         rxIndex = 0;
      }
   }
...
error_t stm32mp2xxEth1UpdateMacAddrFilter(NetInterface *interface)
{
   ...
   else
   {
      ...
      for(i = 0, j = 0; i < MAC_ADDR_FILTER_SIZE; i++)
      {
         ...
         if(entry->refCount > 0)
         {
            ...
            if(macIsMulticastAddr(&entry->addr))
            {
               ...
               crc = stm32mp2xxEth1CalcCrc(&entry->addr, sizeof(MacAddr));
      ...
      if(j >= 1)
      {
         ...
         ETH1->MACA1HR = unicastMacAddr[0].w[2] | ETH_MACA1HR_AE;
      }
      else
      {
      ...
      if(j >= 2)
      {
         ...
         ETH1->MACA2HR = unicastMacAddr[1].w[2] | ETH_MACA2HR_AE;
      }
      else
      {
         //When the AE bit is cleared, the entry is ignored
      ...
      if(j >= 3)
      {
         ...
         ETH1->MACA3HR = unicastMacAddr[2].w[2] | ETH_MACA3HR_AE;
      }
      else
      {
         //When the AE bit is cleared, the entry is ignored
...
void stm32mp2xxEth1WritePhyReg(uint8_t opcode, uint8_t phyAddr,
...
uint16_t stm32mp2xxEth1ReadPhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr)
{
   uint16_t data;
   uint32_t temp;
