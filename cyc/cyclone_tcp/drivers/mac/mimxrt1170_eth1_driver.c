...
#pragma location = MIMXRT1170_ETH1_RAM_SECTION
static uint8_t txBuffer[MIMXRT1170_ETH1_TX_BUFFER_COUNT][MIMXRT1170_ETH1_TX_BUFFER_SIZE];
//RX buffer
#pragma data_alignment = 64
#pragma location = MIMXRT1170_ETH1_RAM_SECTION
static uint8_t rxBuffer[MIMXRT1170_ETH1_RX_BUFFER_COUNT][MIMXRT1170_ETH1_RX_BUFFER_SIZE];
//TX buffer descriptors
#pragma data_alignment = 64
#pragma location = MIMXRT1170_ETH1_RAM_SECTION
static uint32_t txBufferDesc[MIMXRT1170_ETH1_TX_BUFFER_COUNT][8];
//RX buffer descriptors
#pragma data_alignment = 64
#pragma location = MIMXRT1170_ETH1_RAM_SECTION
static uint32_t rxBufferDesc[MIMXRT1170_ETH1_RX_BUFFER_COUNT][8];
...
//TX buffer
static uint8_t txBuffer[MIMXRT1170_ETH1_TX_BUFFER_COUNT][MIMXRT1170_ETH1_TX_BUFFER_SIZE]
   __attribute__((aligned(64), __section__(MIMXRT1170_ETH1_RAM_SECTION)));
//RX buffer
static uint8_t rxBuffer[MIMXRT1170_ETH1_RX_BUFFER_COUNT][MIMXRT1170_ETH1_RX_BUFFER_SIZE]
   __attribute__((aligned(64), __section__(MIMXRT1170_ETH1_RAM_SECTION)));
//TX buffer descriptors
...
/**
 * @brief i.MX RT1170 Ethernet MAC driver (ENET instance)
 **/

const NicDriver mimxrt1170Eth1Driver =
{
   NIC_TYPE_ETHERNET,
   ETH_MTU,
   mimxrt1170Eth1Init,
   mimxrt1170Eth1Tick,
   mimxrt1170Eth1EnableIrq,
   mimxrt1170Eth1DisableIrq,
   mimxrt1170Eth1EventHandler,
   mimxrt1170Eth1SendPacket,
   mimxrt1170Eth1UpdateMacAddrFilter,
   mimxrt1170Eth1UpdateMacConfig,
   mimxrt1170Eth1WritePhyReg,
   mimxrt1170Eth1ReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
   FALSE
...
error_t mimxrt1170Eth1Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing i.MX RT1170 Ethernet MAC (ENET)...\r\n");
   ...
   //GPIO configuration
   mimxrt1170Eth1InitGpio(interface);
   ...
   ENET->RCR = ENET_RCR_MAX_FL(MIMXRT1170_ETH1_RX_BUFFER_SIZE) |
   ...
   mimxrt1170Eth1InitBufferDesc(interface);
   ...
   NVIC_SetPriorityGrouping(MIMXRT1170_ETH1_IRQ_PRIORITY_GROUPING);

   //Configure ENET interrupt priority
   NVIC_SetPriority(ENET_IRQn, NVIC_EncodePriority(MIMXRT1170_ETH1_IRQ_PRIORITY_GROUPING,
      MIMXRT1170_ETH1_IRQ_GROUP_PRIORITY, MIMXRT1170_ETH1_IRQ_SUB_PRIORITY));
...
__weak_func void mimxrt1170Eth1InitGpio(NetInterface *interface)
{
...
void mimxrt1170Eth1InitBufferDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < MIMXRT1170_ETH1_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < MIMXRT1170_ETH1_RX_BUFFER_COUNT; i++)
   {
   ...
   ENET->MRBR = MIMXRT1170_ETH1_RX_BUFFER_SIZE;
}
...
void mimxrt1170Eth1EventHandler(NetInterface *interface)
{
   ...
   if((status & ENET_EIR_RXF_MASK) != 0)
   {
      ...
      do
      {
         ...
         error = mimxrt1170Eth1ReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > MIMXRT1170_ETH1_TX_BUFFER_SIZE)
   {
   ...
   //Check current index
   if(txBufferIndex < (MIMXRT1170_ETH1_TX_BUFFER_COUNT - 1))
   {
   ...
   if((rxBufferDesc[rxBufferIndex][0] & ENET_RBD0_E) == 0)
   {
      ...
      if((rxBufferDesc[rxBufferIndex][0] & ENET_RBD0_L) != 0)
      {
            ...
            ENET_RBD0_CR | ENET_RBD0_OV | ENET_RBD0_TR)) == 0)
         {
            ...
            n = MIN(n, MIMXRT1170_ETH1_RX_BUFFER_SIZE);
      ...
      //Check current index
      if(rxBufferIndex < (MIMXRT1170_ETH1_RX_BUFFER_COUNT - 1))
      {
...
error_t mimxrt1170Eth1UpdateMacAddrFilter(NetInterface *interface)
{
   ...
   for(i = 0; i < MAC_ADDR_FILTER_SIZE; i++)
   {
      ...
      if(entry->refCount > 0)
      {
         ...
         crc = mimxrt1170Eth1CalcCrc(&entry->addr, sizeof(MacAddr));
