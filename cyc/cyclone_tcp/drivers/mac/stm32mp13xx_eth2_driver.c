/**
 * @file stm32mp13xx_eth2_driver.c
 * @brief STM32MP13 Gigabit Ethernet MAC driver (ETH2 instance)
 *
 * @section License
 *
...
#pragma location = STM32MP13XX_ETH2_RAM_SECTION
static uint8_t txBuffer[STM32MP13XX_ETH2_TX_BUFFER_COUNT][STM32MP13XX_ETH2_TX_BUFFER_SIZE];
//Receive buffer
#pragma data_alignment = 4
#pragma location = STM32MP13XX_ETH2_RAM_SECTION
static uint8_t rxBuffer[STM32MP13XX_ETH2_RX_BUFFER_COUNT][STM32MP13XX_ETH2_RX_BUFFER_SIZE];
//Transmit DMA descriptors
#pragma data_alignment = 8
#pragma location = STM32MP13XX_ETH2_RAM_SECTION
static Stm32mp13xxEth2TxDmaDesc txDmaDesc[STM32MP13XX_ETH2_TX_BUFFER_COUNT];
//Receive DMA descriptors
#pragma data_alignment = 8
#pragma location = STM32MP13XX_ETH2_RAM_SECTION
static Stm32mp13xxEth2RxDmaDesc rxDmaDesc[STM32MP13XX_ETH2_RX_BUFFER_COUNT];
...
//Transmit buffer
static uint8_t txBuffer[STM32MP13XX_ETH2_TX_BUFFER_COUNT][STM32MP13XX_ETH2_TX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(STM32MP13XX_ETH2_RAM_SECTION)));
//Receive buffer
static uint8_t rxBuffer[STM32MP13XX_ETH2_RX_BUFFER_COUNT][STM32MP13XX_ETH2_RX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(STM32MP13XX_ETH2_RAM_SECTION)));
//Transmit DMA descriptors
static Stm32mp13xxEth2TxDmaDesc txDmaDesc[STM32MP13XX_ETH2_TX_BUFFER_COUNT]
   __attribute__((aligned(8), __section__(STM32MP13XX_ETH2_RAM_SECTION)));
//Receive DMA descriptors
static Stm32mp13xxEth2RxDmaDesc rxDmaDesc[STM32MP13XX_ETH2_RX_BUFFER_COUNT]
   __attribute__((aligned(8), __section__(STM32MP13XX_ETH2_RAM_SECTION)));
...
/**
 * @brief STM32MP13 Ethernet MAC driver (ETH2 instance)
 **/
...
const NicDriver stm32mp13xxEth2Driver =
{
   ...
   stm32mp13xxEth2Init,
   stm32mp13xxEth2Tick,
   stm32mp13xxEth2EnableIrq,
   stm32mp13xxEth2DisableIrq,
   stm32mp13xxEth2EventHandler,
   stm32mp13xxEth2SendPacket,
   stm32mp13xxEth2UpdateMacAddrFilter,
   stm32mp13xxEth2UpdateMacConfig,
   stm32mp13xxEth2WritePhyReg,
   stm32mp13xxEth2ReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
   FALSE
...
error_t stm32mp13xxEth2Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing STM32MP13 Ethernet MAC (ETH2)...\r\n");

   //Save underlying network interface
   nicDriverInterface = interface;

   //GPIO configuration
   stm32mp13xxEth2InitGpio(interface);
   ...
   __HAL_RCC_ETH2MAC_FORCE_RESET();
   __HAL_RCC_ETH2MAC_RELEASE_RESET();
   ...
   ETH2->MACECR = temp | STM32MP13XX_ETH2_RX_BUFFER_SIZE;

   //Configure MAC address filtering
   stm32mp13xxEth2UpdateMacAddrFilter(interface);
   ...
   ETH2->DMAC0RXCR = ETH_DMAC0RXCR_RXPBL_Val(32) |
      ETH_DMAC0RXCR_RBSZ_Val(STM32MP13XX_ETH2_RX_BUFFER_SIZE);
   ...
   stm32mp13xxEth2InitDmaDesc(interface);
   ...
   IRQ_SetPriority(ETH2_IRQn, STM32MP13XX_ETH2_IRQ_PRIORITY);
...
//STM32MP135F-DK evaluation board?
   ...
   //Configure ETH2_CLK (PG8) as a 50MHz clock output
   ...
   //Select the source of the reference clock (RCC)
   SYSCFG->PMCSETR = SYSCFG_PMCSETR_ETH2_REF_CLK_SEL;
   ...
   //Configure ETH2_RMII_CRS_DV (PA12)
   GPIO_InitStructure.Pin = GPIO_PIN_12;
   GPIO_InitStructure.Alternate = GPIO_AF11_ETH;
   HAL_GPIO_Init(GPIOA, &GPIO_InitStructure);

   //Configure ETH2_MDIO (PB2)
   GPIO_InitStructure.Pin = GPIO_PIN_2;
   GPIO_InitStructure.Alternate = GPIO_AF11_ETH;
   HAL_GPIO_Init(GPIOB, &GPIO_InitStructure);

   //Configure ETH2_RMII_RXD1 (PE2)
   GPIO_InitStructure.Pin = GPIO_PIN_2;
   GPIO_InitStructure.Alternate = GPIO_AF10_ETH;
   HAL_GPIO_Init(GPIOE, &GPIO_InitStructure);

   //Configure ETH2_RMII_RXD0 (PF4), ETH2_RMII_TX_EN (PF6) and ETH2_RMII_TXD0 (PF7)
   GPIO_InitStructure.Pin = GPIO_PIN_4 | GPIO_PIN_6 | GPIO_PIN_7;
   GPIO_InitStructure.Alternate = GPIO_AF11_ETH;
   HAL_GPIO_Init(GPIOF, &GPIO_InitStructure);

   //Configure ETH2_MDC (PG5) and ETH2_RMII_TXD1 (PG11)
   GPIO_InitStructure.Pin = GPIO_PIN_5 | GPIO_PIN_11;
   GPIO_InitStructure.Alternate = GPIO_AF10_ETH;
   HAL_GPIO_Init(GPIOG, &GPIO_InitStructure);

   //Configure ETH2_NRST pin
   IO_InitStructure.Pin = MCP23x17_GPIO_PIN_10;
   ...
   BSP_IO_WritePin(0, MCP23x17_GPIO_PIN_10, IO_PIN_RESET);
   sleep(10);
...
void stm32mp13xxEth2InitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < STM32MP13XX_ETH2_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < STM32MP13XX_ETH2_RX_BUFFER_COUNT; i++)
   {
   ...
   ETH2->DMAC0TXRLR = STM32MP13XX_ETH2_TX_BUFFER_COUNT - 1;

   //Start location of the RX descriptor list
   ETH2->DMAC0RXDLAR = (uint32_t) &rxDmaDesc[0];
   //Length of the receive descriptor ring
   ETH2->DMAC0RXRLR = STM32MP13XX_ETH2_RX_BUFFER_COUNT - 1;
}
...
void stm32mp13xxEth2EventHandler(NetInterface *interface)
{
   ...
   do
   {
      ...
      error = stm32mp13xxEth2ReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > STM32MP13XX_ETH2_TX_BUFFER_SIZE)
   {
   ...
   if(++txIndex >= STM32MP13XX_ETH2_TX_BUFFER_COUNT)
   {
      txIndex = 0;
   }
...
error_t stm32mp13xxEth2ReceivePacket(NetInterface *interface)
{
   ...
   if((rxDmaDesc[rxIndex].rdes3 & ETH_RDES3_OWN) == 0)
   {
      ...
      //Increment index and wrap around if necessary
      if(++rxIndex >= STM32MP13XX_ETH2_RX_BUFFER_COUNT)
      {
         rxIndex = 0;
      }
   }
      ...
      for(i = 0, j = 0; i < MAC_ADDR_FILTER_SIZE; i++)
      {
         ...
         if(entry->refCount > 0)
         {
            ...
            if(macIsMulticastAddr(&entry->addr))
            {
               ...
               crc = stm32mp13xxEth2CalcCrc(&entry->addr, sizeof(MacAddr));
...
void stm32mp13xxEth2WritePhyReg(uint8_t opcode, uint8_t phyAddr,
...
uint16_t stm32mp13xxEth2ReadPhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr)
{
   uint16_t data;
   uint32_t temp;
