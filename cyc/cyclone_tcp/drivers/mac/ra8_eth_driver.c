...
#include "bsp_api.h"
#include "core/net.h"
#include "drivers/mac/ra8_eth_driver.h"
#include "debug.h"
...
#pragma location = RA8_ETH_RAM_SECTION
static uint8_t txBuffer[RA8_ETH_TX_BUFFER_COUNT][RA8_ETH_TX_BUFFER_SIZE];
//Receive buffer
#pragma data_alignment = 32
#pragma location = RA8_ETH_RAM_SECTION
static uint8_t rxBuffer[RA8_ETH_RX_BUFFER_COUNT][RA8_ETH_RX_BUFFER_SIZE];
//Transmit DMA descriptors
#pragma data_alignment = 16
#pragma location = RA8_ETH_RAM_SECTION
static Ra8EthTxDmaDesc txDmaDesc[RA8_ETH_TX_BUFFER_COUNT];
//Receive DMA descriptors
#pragma data_alignment = 16
#pragma location = RA8_ETH_RAM_SECTION
static Ra8EthRxDmaDesc rxDmaDesc[RA8_ETH_RX_BUFFER_COUNT];
...
//Transmit buffer
static uint8_t txBuffer[RA8_ETH_TX_BUFFER_COUNT][RA8_ETH_TX_BUFFER_SIZE]
   __attribute__((aligned(32), __section__(RA8_ETH_RAM_SECTION)));
//Receive buffer
static uint8_t rxBuffer[RA8_ETH_RX_BUFFER_COUNT][RA8_ETH_RX_BUFFER_SIZE]
   __attribute__((aligned(32), __section__(RA8_ETH_RAM_SECTION)));
//Transmit DMA descriptors
static Ra8EthTxDmaDesc txDmaDesc[RA8_ETH_TX_BUFFER_COUNT]
   __attribute__((aligned(16), __section__(RA8_ETH_RAM_SECTION)));
//Receive DMA descriptors
static Ra8EthRxDmaDesc rxDmaDesc[RA8_ETH_RX_BUFFER_COUNT]
   __attribute__((aligned(16), __section__(RA8_ETH_RAM_SECTION)));
...
const NicDriver ra8EthDriver =
{
   ...
   ra8EthEventHandler,
   ra8EthSendPacket,
   ra8EthUpdateMacAddrFilter,
   ra8EthUpdateMacConfig,
   ra8EthWritePhyReg,
   ra8EthReadPhyReg,
   TRUE,
   TRUE,
...
/**
 * @brief RA8 Ethernet MAC initialization
...
error_t ra8EthInit(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing RA8 Ethernet MAC...\r\n");
   ...
   ra8EthInitDmaDesc(interface);

   //Maximum frame length that can be accepted
   R_ETHERC0->RFLR = RA8_ETH_RX_BUFFER_SIZE;
   ...
   NVIC_SetPriorityGrouping(RA8_ETH_IRQ_PRIORITY_GROUPING);

   //Configure EDMAC interrupt priority
   NVIC_SetPriority(EDMAC0_EINT_IRQn, NVIC_EncodePriority(RA8_ETH_IRQ_PRIORITY_GROUPING,
      RA8_ETH_IRQ_GROUP_PRIORITY, RA8_ETH_IRQ_SUB_PRIORITY));
...
//EK-RA8D1 evaluation board?
#elif defined(USE_EK_RA8D1)
   //Disable protection
   R_SYSTEM->PRCR = 0xA50B;
   //Disable VBATT channel 0 input (P4_2)
   R_SYSTEM->VBTICTLR &= ~R_SYSTEM_VBTICTLR_VCH0INEN_Msk;
   ...
   //Configure PHY reset pin (P7_6)
   R_PFS->PORT[7].PIN[6].PmnPFS = R_PFS_PORT_PIN_PmnPFS_PDR_Msk;
   ...
   //Reset PHY transceiver
   R_PORT7->PCNTR3 = (1 << 6) << R_PORT0_PCNTR3_PORR_Pos;
   sleep(10);
   R_PORT7->PCNTR3 = (1 << 6) << R_PORT0_PCNTR3_POSR_Pos;
   sleep(10);

//MCK-RA8T1 evaluation board?
#elif defined(USE_MCK_RA8T1)
   //Disable protection
   R_SYSTEM->PRCR = 0xA50B;
   //Disable VBATT channel 0 input (P4_2)
   R_SYSTEM->VBTICTLR &= ~R_SYSTEM_VBTICTLR_VCH0INEN_Msk;
   ...
   //Configure PHY reset pin (PB_1)
   R_PFS->PORT[11].PIN[1].PmnPFS = R_PFS_PORT_PIN_PmnPFS_PDR_Msk;
...
void ra8EthInitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < RA8_ETH_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < RA8_ETH_RX_BUFFER_COUNT; i++)
   {
      //The descriptor is initially owned by the DMA
      rxDmaDesc[i].rd0 = EDMAC_RD0_RACT;
      //Receive buffer length
      rxDmaDesc[i].rd1 = (RA8_ETH_RX_BUFFER_SIZE << 16) & EDMAC_RD1_RBL;
...
/**
 * @brief RA8 Ethernet MAC timer handler
 *
...
/**
 * @brief RA8 Ethernet MAC interrupt service routine
 **/
...
/**
 * @brief RA8 Ethernet MAC event handler
 * @param[in] interface Underlying network interface
 **/
...
void ra8EthEventHandler(NetInterface *interface)
{
   ...
   do
   {
      ...
      error = ra8EthReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > RA8_ETH_TX_BUFFER_SIZE)
   {
   ...
   //Check current index
   if(txIndex < (RA8_ETH_TX_BUFFER_COUNT - 1))
   {
   ...
   if((rxDmaDesc[rxIndex].rd0 & EDMAC_RD0_RACT) == 0)
   {
         ...
         (rxDmaDesc[rxIndex].rd0 & EDMAC_RD0_RFP_EOF) != 0)
      {
         ...
         if((rxDmaDesc[rxIndex].rd0 & (EDMAC_RD0_RFS_MASK & ~EDMAC_RD0_RFS_RMAF)) == 0)
         {
            ...
            n = MIN(n, RA8_ETH_RX_BUFFER_SIZE);
      ...
      //Check current index
      if(rxIndex < (RA8_ETH_RX_BUFFER_COUNT - 1))
      {
   ...
   uint8_t regAddr, uint16_t data)
{
   ...
   ra8EthWriteSmi(SMI_SYNC, 32);
   //Start of frame
   ra8EthWriteSmi(SMI_START, 2);
   //Set up a write operation
   ra8EthWriteSmi(opcode, 2);
   //Write PHY address
   ra8EthWriteSmi(phyAddr, 5);
   //Write register address
   ra8EthWriteSmi(regAddr, 5);
   //Turnaround
   ra8EthWriteSmi(SMI_TA, 2);
   //Write register value
   ra8EthWriteSmi(data, 16);
   //Release MDIO
   ra8EthReadSmi(1);
}
