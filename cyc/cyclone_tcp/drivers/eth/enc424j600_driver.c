/**
 * @file enc424j600_driver.c
 * @brief ENC424J600 Ethernet controller
 *
 * @section License
 *
...
//Dependencies
#include "core/net.h"
#include "drivers/eth/enc424j600_driver.h"
#include "debug.h"


/**
 * @brief ENC424J600 driver
 **/
...
/**
 * @brief ENC424J600 controller initialization
...
error_t enc424j600Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing ENC424J600 Ethernet controller...\r\n");
   ...
   context->nextPacket = ENC424J600_RX_BUFFER_START;

   //Issue a system reset
   enc424j600SoftReset(interface);

   //Disable CLKOUT output
   enc424j600WriteReg(interface, ENC424J600_ECON2, ENC424J600_ECON2_ETHEN |
   ...
   if(macCompAddr(&interface->macAddr, &MAC_UNSPECIFIED_ADDR))
   {
      ...
      temp = enc424j600ReadReg(interface, ENC424J600_MAADR1);
      interface->macAddr.w[0] = letoh16(temp);
      temp = enc424j600ReadReg(interface, ENC424J600_MAADR2);
      interface->macAddr.w[1] = letoh16(temp);
      temp = enc424j600ReadReg(interface, ENC424J600_MAADR3);
   ...
   else
   {
      ...
      enc424j600WriteReg(interface, ENC424J600_MAADR1, temp);
      temp = htole16(interface->macAddr.w[1]);
      enc424j600WriteReg(interface, ENC424J600_MAADR2, temp);
      temp = htole16(interface->macAddr.w[2]);
      enc424j600WriteReg(interface, ENC424J600_MAADR3, temp);
   }

   //Set receive buffer location
   enc424j600WriteReg(interface, ENC424J600_ERXST, ENC424J600_RX_BUFFER_START);
   //Program the tail pointer ERXTAIL to the last even address of the buffer
   enc424j600WriteReg(interface, ENC424J600_ERXTAIL, ENC424J600_RX_BUFFER_STOP);

   //Configure the receive filters
   enc424j600WriteReg(interface, ENC424J600_ERXFCON, ENC424J600_ERXFCON_HTEN |
      ENC424J600_ERXFCON_CRCEN | ENC424J600_ERXFCON_RUNTEN |
      ENC424J600_ERXFCON_UCEN | ENC424J600_ERXFCON_BCEN);

   //Initialize the hash table
   enc424j600WriteReg(interface, ENC424J600_EHT1, 0x0000);
   enc424j600WriteReg(interface, ENC424J600_EHT2, 0x0000);
   enc424j600WriteReg(interface, ENC424J600_EHT3, 0x0000);
   enc424j600WriteReg(interface, ENC424J600_EHT4, 0x0000);

   //All short frames will be zero-padded to 60 bytes and a valid CRC is then
   //appended
   enc424j600WriteReg(interface, ENC424J600_MACON2, ENC424J600_MACON2_DEFER |
      ENC424J600_MACON2_PADCFG_AUTO | ENC424J600_MACON2_TXCRCEN |
      ENC424J600_MACON2_R1_DEFAULT);

   //Program the MAMXFL register with the maximum frame length to be accepted
   enc424j600WriteReg(interface, ENC424J600_MAMXFL,
      ENC424J600_ETH_RX_BUFFER_SIZE);

   //PHY initialization
   enc424j600WritePhyReg(interface, ENC424J600_PHANA, ENC424J600_PHANA_ADPAUS0 |
      ENC424J600_PHANA_AD100FD | ENC424J600_PHANA_AD100 | ENC424J600_PHANA_AD10FD |
      ENC424J600_PHANA_AD10 | ENC424J600_PHANA_ADIEEE_DEFAULT);

   //Clear interrupt flags
   enc424j600WriteReg(interface, ENC424J600_EIR, 0x0000);

   //Configure interrupts as desired
   enc424j600WriteReg(interface, ENC424J600_EIE, ENC424J600_EIE_INTIE |
      ENC424J600_EIE_LINKIE | ENC424J600_EIE_PKTIE | ENC424J600_EIE_TXIE |
      ENC424J600_EIE_TXABTIE);

   //Set RXEN to enable reception
   enc424j600SetBit(interface, ENC424J600_ECON1, ENC424J600_ECON1_RXEN);

   //Perform custom configuration
   enc424j600InitHook(interface);

   //Dump registers for debugging purpose
   enc424j600DumpReg(interface);
   enc424j600DumpPhyReg(interface);
...
/**
 * @brief ENC424J600 custom configuration
 * @param[in] interface Underlying network interface
 **/
...
/**
 * @brief ENC424J600 timer handler
 * @param[in] interface Underlying network interface
 **/
...
/**
 * @brief ENC424J600 interrupt service routine
 * @param[in] interface Underlying network interface
...
bool_t enc424j600IrqHandler(NetInterface *interface)
{
   ...
   enc424j600ClearBit(interface, ENC424J600_EIE, ENC424J600_EIE_INTIE);

   //Read interrupt status register
   status = enc424j600ReadReg(interface, ENC424J600_EIR);

   //Link status change?
   if((status & ENC424J600_EIR_LINKIF) != 0)
   {
      //Disable LINKIE interrupt
      enc424j600ClearBit(interface, ENC424J600_EIE, ENC424J600_EIE_LINKIE);
   ...
   //Packet received?
   if((status & ENC424J600_EIR_PKTIF) != 0)
   {
      //Disable PKTIE interrupt
      enc424j600ClearBit(interface, ENC424J600_EIE, ENC424J600_EIE_PKTIE);
   ...
   if((status & (ENC424J600_EIR_TXIF | ENC424J600_EIR_TXABTIF)) != 0)
   {
      //Clear interrupt flags
      enc424j600ClearBit(interface, ENC424J600_EIR, ENC424J600_EIR_TXIF |
   ...
   enc424j600SetBit(interface, ENC424J600_EIE, ENC424J600_EIE_INTIE);
...
/**
 * @brief ENC424J600 event handler
 * @param[in] interface Underlying network interface
 **/
...
void enc424j600EventHandler(NetInterface *interface)
{
   ...
   if((status & ENC424J600_EIR_LINKIF) != 0)
   {
      ...
      enc424j600ClearBit(interface, ENC424J600_EIR, ENC424J600_EIR_LINKIF);
      //Read Ethernet status register
      value = enc424j600ReadReg(interface, ENC424J600_ESTAT);

      //Check link state
      if((value & ENC424J600_ESTAT_PHYLNK) != 0)
      {
         //Read PHY status register 3
         value = enc424j600ReadPhyReg(interface, ENC424J600_PHSTAT3);

         //Get current speed
         if((value & ENC424J600_PHSTAT3_SPDDPX1) != 0)
         {
         ...
         if((value & ENC424J600_PHSTAT3_SPDDPX2) != 0)
         {
         ...
         enc424j600UpdateMacConfig(interface);
      }
      else
      {
         //Link is down
   ...
   {
      //Clear interrupt flag
      enc424j600ClearBit(interface, ENC424J600_EIR, ENC424J600_EIR_PKTIF);

      //Process all pending packets
      do
      {
         //Read incoming packet
         error = enc424j600ReceivePacket(interface);
   ...
   enc424j600SetBit(interface, ENC424J600_EIE, ENC424J600_EIE_LINKIE |
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(enc424j600ReadReg(interface, ENC424J600_ECON1) & ENC424J600_ECON1_TXRTS)
   {
      return ERROR_FAILURE;
   }

   //Point to the SRAM buffer
   enc424j600WriteReg(interface, ENC424J600_EGPWRPT, ENC424J600_TX_BUFFER_START);
   //Copy the packet to the SRAM buffer
   enc424j600WriteBuffer(interface, ENC424J600_CMD_WGPDATA, buffer, offset);

   //Program ETXST to the start address of the packet
   enc424j600WriteReg(interface, ENC424J600_ETXST, ENC424J600_TX_BUFFER_START);
   //Program ETXLEN with the length of data copied to the memory
   enc424j600WriteReg(interface, ENC424J600_ETXLEN, length);
   ...
   enc424j600SetBit(interface, ENC424J600_ECON1, ENC424J600_ECON1_TXRTS);
...
{
   static uint8_t temp[ENC424J600_ETH_RX_BUFFER_SIZE];
   error_t error;
   uint16_t length;
   uint32_t status;
   uint8_t header[8];
   ...
   if(enc424j600ReadReg(interface, ENC424J600_ESTAT) & ENC424J600_ESTAT_PKTCNT)
   {
      //Point to the next packet
      enc424j600WriteReg(interface, ENC424J600_ERXRDPT, context->nextPacket);

      //The packet is preceded by a 8-byte header
      enc424j600ReadBuffer(interface, ENC424J600_CMD_RRXDATA, header,
         sizeof(header));
      ...
      if((status & ENC424J600_RSV_RECEIVED_OK) != 0)
      {
         //Limit the number of data to read
         length = MIN(length, ENC424J600_ETH_RX_BUFFER_SIZE);
         //Read packet data
         enc424j600ReadBuffer(interface, ENC424J600_CMD_RRXDATA, temp, length);
      ...
      //memory buffer
      if(context->nextPacket == ENC424J600_RX_BUFFER_START)
      {
         enc424j600WriteReg(interface, ENC424J600_ERXTAIL,
      ...
      enc424j600SetBit(interface, ENC424J600_ECON1, ENC424J600_ECON1_PKTDEC);
   }
   else
   {
      //No more data in the receive buffer
...
error_t enc424j600UpdateMacAddrFilter(NetInterface *interface)
{
   ...
   for(i = 0; i < MAC_ADDR_FILTER_SIZE; i++)
   {
      ...
      if(entry->refCount > 0)
      {
         ...
         crc = enc424j600CalcCrc(&entry->addr, sizeof(MacAddr));
   ...
   //Write the hash table to the ENC424J600 controller
   enc424j600WriteReg(interface, ENC424J600_EHT1, hashTable[0]);
   enc424j600WriteReg(interface, ENC424J600_EHT2, hashTable[1]);
   enc424j600WriteReg(interface, ENC424J600_EHT3, hashTable[2]);
   enc424j600WriteReg(interface, ENC424J600_EHT4, hashTable[3]);

   //Debug message
   TRACE_DEBUG("  EHT1 = %04" PRIX16 "\r\n", enc424j600ReadReg(interface, ENC424J600_EHT1));
   TRACE_DEBUG("  EHT2 = %04" PRIX16 "\r\n", enc424j600ReadReg(interface, ENC424J600_EHT2));
   TRACE_DEBUG("  EHT3 = %04" PRIX16 "\r\n", enc424j600ReadReg(interface, ENC424J600_EHT3));
   TRACE_DEBUG("  EHT4 = %04" PRIX16 "\r\n", enc424j600ReadReg(interface, ENC424J600_EHT4));
...
void enc424j600UpdateMacConfig(NetInterface *interface)
{
   ...
   duplexMode = enc424j600ReadReg(interface, ENC424J600_ESTAT) & ENC424J600_ESTAT_PHYDPX;
   ...
   if(duplexMode)
   {
      ...
      enc424j600WriteReg(interface, ENC424J600_MACON2, ENC424J600_MACON2_DEFER |
         ENC424J600_MACON2_PADCFG_AUTO | ENC424J600_MACON2_TXCRCEN |
         ENC424J600_MACON2_R1_DEFAULT | ENC424J600_MACON2_FULDPX);

      //Configure the Back-to-Back Inter-Packet Gap register
      enc424j600WriteReg(interface, ENC424J600_MABBIPG,
...
/**
 * @brief Reset ENC424J600 controller
 * @param[in] interface Underlying network interface
 * @return Error code
 **/
...
error_t enc424j600SoftReset(NetInterface *interface)
{
   ...
   do
   {
      ...
      enc424j600WriteReg(interface, ENC424J600_EUDAST, 0x1234);
      //Read back register and check contents
   } while(enc424j600ReadReg(interface, ENC424J600_EUDAST) != 0x1234);

   //Poll CLKRDY and wait for it to become set
   while((enc424j600ReadReg(interface, ENC424J600_ESTAT) & ENC424J600_ESTAT_CLKRDY) == 0)
   {
   }

   //Issue a system reset command by setting ETHRST
   enc424j600SetBit(interface, ENC424J600_ECON2, ENC424J600_ECON2_ETHRST);
   ...
   if(enc424j600ReadReg(interface, ENC424J600_EUDAST) != 0x0000)
   {
      return ERROR_FAILURE;
   }
...
/**
 * @brief Write ENC424J600 register
 * @param[in] interface Underlying network interface
 * @param[in] address Register address
 * @param[in] data Register value
 **/

void enc424j600WriteReg(NetInterface *interface, uint8_t address,
   uint16_t data)
{
   //Pull the CS pin low
   ...
   //Write opcode
   interface->spiDriver->transfer(ENC424J600_CMD_WCRU);
...
/**
 * @brief Read ENC424J600 register
 * @param[in] interface Underlying network interface
 * @param[in] address Register address
...
uint16_t enc424j600ReadReg(NetInterface *interface, uint8_t address)
{
   ...
   //Write opcode
   interface->spiDriver->transfer(ENC424J600_CMD_RCRU);
   //Write register address
   interface->spiDriver->transfer(address);
...
void enc424j600WritePhyReg(NetInterface *interface, uint8_t address,
   uint16_t data)
{
   uint16_t status;

   //Write the address of the PHY register to write to
   enc424j600WriteReg(interface, ENC424J600_MIREGADR,
      ENC424J600_MIREGADR_R12_8_DEFAULT | address);

   //Write the 16 bits of data into the MIWR register
   enc424j600WriteReg(interface, ENC424J600_MIWR, data);

   //Wait until the PHY register has been written
   do
   {
      //Read MII Management Status register
      status = enc424j600ReadReg(interface, ENC424J600_MISTAT);
      //Check the value of the busy status bit
   } while((status & ENC424J600_MISTAT_BUSY) != 0);
}
...
uint16_t enc424j600ReadPhyReg(NetInterface *interface, uint8_t address)
{
   ...
   enc424j600WriteReg(interface, ENC424J600_MICMD, ENC424J600_MICMD_MIIRD);
   ...
   enc424j600WriteReg(interface, ENC424J600_MICMD, 0x00);

   //Return register contents
   return enc424j600ReadReg(interface, ENC424J600_MIRD);
}
...
void enc424j600SetBit(NetInterface *interface, uint8_t address,
   uint16_t mask)
{
   //Pull the CS pin low
   interface->spiDriver->assertCs();
   ...
   //Write opcode
   interface->spiDriver->transfer(ENC424J600_CMD_BFSU);
   //Write register address
   interface->spiDriver->transfer(address);
   //Write bit mask
...
void enc424j600ClearBit(NetInterface *interface, uint8_t address,
   uint16_t mask)
{
   //Pull the CS pin low
   interface->spiDriver->assertCs();
   ...
   //Write opcode
   interface->spiDriver->transfer(ENC424J600_CMD_BFCU);
   //Write register address
   interface->spiDriver->transfer(address);
   //Write bit mask
   interface->spiDriver->transfer(LSB(mask));
...
void enc424j600DumpReg(NetInterface *interface)
{
   ...
   for(i = 0; i < 32; i += 2)
   {
      ...
      for(bank = 0; bank < 5; bank++)
      {
         ...
         TRACE_DEBUG("0x%04" PRIX16 "  ", enc424j600ReadReg(interface, address));
      }
...
void enc424j600DumpPhyReg(NetInterface *interface)
{
   ...
   for(i = 0; i < 32; i++)
   {
      ...
      TRACE_DEBUG("%02" PRIX8 ": 0x%04" PRIX16 "\r\n", i, enc424j600ReadPhyReg(interface, i));
   }
