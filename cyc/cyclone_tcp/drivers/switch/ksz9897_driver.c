   ...
   if(interface->spiDriver != NULL)
   {
      ...
      for(port = KSZ9897_PORT1; port <= KSZ9897_PORT5; port++)
      {
         ...
         if(interface->port != 0)
         {
            ...
            ksz9897SetPortState(interface, port, SWITCH_PORT_STATE_LISTENING);
         }
         else
#endif
         {
            //Enable transmission, reception and address learning
            ksz9897SetPortState(interface, port, SWITCH_PORT_STATE_FORWARDING);
         }
      }
   ...
   size_t *offset, NetTxAncillary *ancillary)
{
   ...
   {
      //Valid port?
      if(ancillary->port <= KSZ9897_PORT5)
      {
         size_t length;
