...
/**
 * @brief KSZ8775 Ethernet switch driver
 **/
...
/**
 * @brief Tail tag rules (host to KSZ8775)
 **/

const uint8_t ksz8775IngressTailTag[5] =
{
   KSZ8775_TAIL_TAG_NORMAL_ADDR_LOOKUP,
   KSZ8775_TAIL_TAG_PORT_SEL | KSZ8775_TAIL_TAG_DEST_PORT1,
   KSZ8775_TAIL_TAG_PORT_SEL | KSZ8775_TAIL_TAG_DEST_PORT2,
   KSZ8775_TAIL_TAG_PORT_SEL | KSZ8775_TAIL_TAG_DEST_PORT3,
   KSZ8775_TAIL_TAG_PORT_SEL | KSZ8775_TAIL_TAG_DEST_PORT4
};


/**
 * @brief KSZ8775 Ethernet switch initialization
...
error_t ksz8775Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing KSZ8775...\r\n");
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      do
      {
         ...
         temp = ksz8775ReadSwitchReg(interface, KSZ8775_CHIP_ID0);

         //The returned data is invalid until the serial interface is ready
      } while(temp != KSZ8775_CHIP_ID0_FAMILY_ID_DEFAULT);

#if (ETH_PORT_TAGGING_SUPPORT == ENABLED)
      //Enable tail tag feature
      temp = ksz8775ReadSwitchReg(interface, KSZ8775_GLOBAL_CTRL10);
      temp |= KSZ8775_GLOBAL_CTRL10_TAIL_TAG_EN;
      ksz8775WriteSwitchReg(interface, KSZ8775_GLOBAL_CTRL10, temp);

      //Disable packet size check
      temp = ksz8775ReadSwitchReg(interface, KSZ8775_GLOBAL_CTRL2);
      temp |= KSZ8775_GLOBAL_CTRL2_MAX_PKT_SIZE_CHECK_DIS;
      ksz8775WriteSwitchReg(interface, KSZ8775_GLOBAL_CTRL2, temp);
#else
      ...
      for(port = KSZ8775_PORT1; port <= KSZ8775_PORT4; port++)
      {
#if (ETH_PORT_TAGGING_SUPPORT == ENABLED)
         ...
         if(interface->port != 0)
         {
            ...
            ksz8775SetPortState(interface, port, SWITCH_PORT_STATE_LISTENING);
         }
         else
#endif
         {
            //Enable transmission, reception and address learning
            ksz8775SetPortState(interface, port, SWITCH_PORT_STATE_FORWARDING);
         }
      }
      ...
      ksz8775DumpSwitchReg(interface);
   }
   else
   {
      ...
      //Loop through the ports
      for(port = KSZ8775_PORT1; port <= KSZ8775_PORT3; port++)
      {
         //Debug message
         TRACE_DEBUG("Port %u:\r\n", port);
         //Dump PHY registers for debugging purpose
         ksz8775DumpPhyReg(interface, port);
      }
   }

   //Perform custom configuration
   ksz8775InitHook(interface);
...
/**
 * @brief KSZ8775 custom configuration
 * @param[in] interface Underlying network interface
 **/
...
void ksz8775Tick(NetInterface *interface)
{
   ...
   if(interface->port != 0)
   {
      ...
      for(i = 0; i < NET_INTERFACE_COUNT; i++)
      {
            ...
            virtualInterface->parent == interface)
         {
            ...
            linkState = ksz8775GetLinkState(interface, virtualInterface->port);
...
#endif
   {
      ...
      {
         //Retrieve current link state
         if(ksz8775GetLinkState(interface, port))
         {
            linkState = TRUE;
         }
      }
...
void ksz8775EventHandler(NetInterface *interface)
{
   ...
   if(interface->port != 0)
   {
      ...
      for(i = 0; i < NET_INTERFACE_COUNT; i++)
      {
            ...
            virtualInterface->parent == interface)
         {
            ...
            //Valid port?
            if(port >= KSZ8775_PORT1 && port <= KSZ8775_PORT4)
            {
               //Retrieve current link state
               linkState = ksz8775GetLinkState(interface, port);

               //Link up event?
               if(linkState && !virtualInterface->linkState)
               {
                  //Retrieve host interface speed
                  interface->linkSpeed = ksz8775GetLinkSpeed(interface,
                     KSZ8775_PORT5);

                  //Retrieve host interface duplex mode
                  interface->duplexMode = ksz8775GetDuplexMode(interface,
                     KSZ8775_PORT5);
                  ...
                  //Check current speed
                  virtualInterface->linkSpeed = ksz8775GetLinkSpeed(interface,
                     port);

                  //Check current duplex mode
                  virtualInterface->duplexMode = ksz8775GetDuplexMode(interface,
                     port);
...
#endif
   {
      ...
      {
         //Retrieve host interface speed
         interface->linkSpeed = ksz8775GetLinkSpeed(interface, KSZ8775_PORT5);
         //Retrieve host interface duplex mode
         interface->duplexMode = ksz8775GetDuplexMode(interface, KSZ8775_PORT5);
   ...
   size_t *length, NetRxAncillary *ancillary)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      if(*length >= (sizeof(EthHeader) + sizeof(uint8_t)))
      {
         ...
         ancillary->port = (*tailTag & KSZ8775_TAIL_TAG_SRC_PORT) + 1;
...
bool_t ksz8775GetLinkState(NetInterface *interface, uint8_t port)
{
   ...
   //Check port number
   if(port >= KSZ8775_PORT1 && port <= KSZ8775_PORT3)
   {
      //SPI slave mode?
      if(interface->spiDriver != NULL)
      {
         //Read port status 2 register
         status = ksz8775ReadSwitchReg(interface, KSZ8775_PORTn_STAT2(port));

         //Retrieve current link state
         linkState = (status & KSZ8775_PORTn_STAT2_LINK_GOOD) ? TRUE : FALSE;
      }
      else
      {
         //Read status register
         status = ksz8775ReadPhyReg(interface, port, KSZ8775_BMSR);

         //Retrieve current link state
         linkState = (status & KSZ8775_BMSR_LINK_STATUS) ? TRUE : FALSE;
      }
   }
   else if(port == KSZ8775_PORT4)
   {
      //Port 4 is always up
      linkState = TRUE;
   }
...
uint32_t ksz8775GetLinkSpeed(NetInterface *interface, uint8_t port)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      if(port >= KSZ8775_PORT1 && port <= KSZ8775_PORT3)
      {
         ...
         value = ksz8775ReadSwitchReg(interface, KSZ8775_PORTn_STAT1(port));

         //Retrieve current link speed
         if((value & KSZ8775_PORTn_STAT1_OP_SPEED) != 0)
         {
      ...
      {
         //Read port 4 control 9 register
         value = ksz8775ReadSwitchReg(interface, KSZ8775_PORT4_CTRL9);

         //Retrieve current link speed
         if((value & KSZ8775_PORTn_CTRL9_FORCED_SPEED) != 0)
         {
            linkSpeed = NIC_LINK_SPEED_100MBPS;
         }
         else
      ...
      else if(port == KSZ8775_PORT5)
      {
         //Read port 5 interface control 6 register
         value = ksz8775ReadSwitchReg(interface, KSZ8775_PORT5_IF_CTRL6);

         //Retrieve host interface type
         type = value & KSZ8775_PORTn_IF_CTRL6_IF_MODE_SEL;

         //Gigabit interface?
         if(type == KSZ8775_PORTn_IF_CTRL6_IF_MODE_SEL_RGMII &&
            (value & KSZ8775_PORTn_IF_CTRL6_IS_1GBPS) != 0)
         {
            //1000 Mb/s mode
         ...
         else
         {
            ...
            value = ksz8775ReadSwitchReg(interface, KSZ8775_GLOBAL_CTRL4);

            //Retrieve host interface speed
            if((value & KSZ8775_GLOBAL_CTRL4_SW5_SPEED) != 0)
            {
               //10 Mb/s mode
               linkSpeed = NIC_LINK_SPEED_10MBPS;
            }
...
NicDuplexMode ksz8775GetDuplexMode(NetInterface *interface, uint8_t port)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      if(port >= KSZ8775_PORT1 && port <= KSZ8775_PORT3)
      {
         ...
         if((value & KSZ8775_PORTn_STAT1_OP_DUPLEX) != 0)
         {
      ...
      {
         //Read port status 1 register
         value = ksz8775ReadSwitchReg(interface, KSZ8775_PORTn_CTRL9(port));

         //Retrieve current duplex mode
         if((value & KSZ8775_PORTn_CTRL9_FORCED_DUPLEX) != 0)
         {
            duplexMode = NIC_FULL_DUPLEX_MODE;
         }
         else
      ...
      else if(port == KSZ8775_PORT5)
      {
         ...
         //Retrieve host interface duplex mode
         if((value & KSZ8775_GLOBAL_CTRL4_SW5_HALF_DUPLEX_MODE) != 0)
         {
            duplexMode = NIC_HALF_DUPLEX_MODE;
         }
         else
   ...
   SwitchPortState state)
{
   ...
   if(port >= KSZ8775_PORT1 && port <= KSZ8775_PORT4)
   {
      ...
      temp = ksz8775ReadSwitchReg(interface, KSZ8775_PORTn_CTRL2(port));
      ...
      case SWITCH_PORT_STATE_LISTENING:
         temp &= ~KSZ8775_PORTn_CTRL2_TRANSMIT_EN;
         temp |= KSZ8775_PORTn_CTRL2_RECEIVE_EN;
         temp |= KSZ8775_PORTn_CTRL2_LEARNING_DIS;
         break;
      ...
      ksz8775WriteSwitchReg(interface, KSZ8775_PORTn_CTRL2(port), temp);
   }
}
...
SwitchPortState ksz8775GetPortState(NetInterface *interface, uint8_t port)
{
   ...
   if(port >= KSZ8775_PORT1 && port <= KSZ8775_PORT4)
   {
      ...
      //Check port state
      if((temp & KSZ8775_PORTn_CTRL2_TRANSMIT_EN) == 0 &&
         (temp & KSZ8775_PORTn_CTRL2_RECEIVE_EN) == 0 &&
         (temp & KSZ8775_PORTn_CTRL2_LEARNING_DIS) != 0)
      {
         //Disabled state
         state = SWITCH_PORT_STATE_DISABLED;
      }
      else if((temp & KSZ8775_PORTn_CTRL2_TRANSMIT_EN) == 0 &&
         (temp & KSZ8775_PORTn_CTRL2_RECEIVE_EN) != 0 &&
         (temp & KSZ8775_PORTn_CTRL2_LEARNING_DIS) != 0)
      {
         //Listening state
...
void ksz8775EnableIgmpSnooping(NetInterface *interface, bool_t enable)
{
   ...
   temp = ksz8775ReadSwitchReg(interface, KSZ8775_GLOBAL_CTRL3);

   //Enable or disable IGMP snooping
   if(enable)
   {
      temp |= KSZ8775_GLOBAL_CTRL3_SW5_IGMP_SNOOP_EN;
   }
   else
   {
      temp &= ~KSZ8775_GLOBAL_CTRL3_SW5_IGMP_SNOOP_EN;
   ...
   ksz8775WriteSwitchReg(interface, KSZ8775_GLOBAL_CTRL3, temp);
}
...
void ksz8775EnableMldSnooping(NetInterface *interface, bool_t enable)
{
   ...
   temp = ksz8775ReadSwitchReg(interface, KSZ8775_GLOBAL_CTRL21);

   //Enable or disable MLD snooping
   if(enable)
   {
      temp |= KSZ8775_GLOBAL_CTRL21_MLD_SNOOP_EN;
   }
   else
   {
      temp &= ~KSZ8775_GLOBAL_CTRL21_MLD_SNOOP_EN;
   ...
   ksz8775WriteSwitchReg(interface, KSZ8775_GLOBAL_CTRL21, temp);
}
...
void ksz8775EnableRsvdMcastTable(NetInterface *interface, bool_t enable)
{
   ...
   for(i = 0; i <= 15; i++)
   {
      ...
      if(enable)
      {
         ksz8775AddStaticFdbEntry(interface, &entry);
      }
      else
      {
         ksz8775DeleteStaticFdbEntry(interface, &entry);
      }
   }
}
   ...
   const SwitchFdbEntry *entry)
{
   ...
   Ksz8775StaticMacEntryW newEntry;

   //Keep track of the first free entry
   j = KSZ8775_STATIC_MAC_TABLE_SIZE;

   //Loop through the static MAC table
   for(i = 0; i < KSZ8775_STATIC_MAC_TABLE_SIZE; i++)
   {
      //Read current entry
      error = ksz8775GetStaticFdbEntry(interface, i, &currentEntry);
      ...
      {
         //Keep track of the first free entry
         if(j == KSZ8775_STATIC_MAC_TABLE_SIZE)
         {
            j = i;
         }
      }
   ...
   if(j < KSZ8775_STATIC_MAC_TABLE_SIZE)
   {
      ...
      {
         newEntry.forwardPorts = KSZ8775_PORT5_MASK;
      }
      else
      {
         newEntry.forwardPorts = entry->destPorts & KSZ8775_PORT_MASK;
      }

      //Select the static MAC address table
      ksz8775WriteSwitchReg(interface, KSZ8775_INDIRECT_CTRL0,
         KSZ8775_INDIRECT_CTRL0_READ |
         KSZ8775_INDIRECT_CTRL0_TABLE_SEL_STATIC_MAC);

      //Point to the MAC entry
      p = (uint8_t *) &newEntry;

      //Write indirect data registers
      for(i = 0; i < sizeof(Ksz8775StaticMacEntryW); i++)
      {
         ksz8775WriteSwitchReg(interface, KSZ8775_INDIRECT_DATA7 + i, p[i]);
      }

      //Setup a write operation
      ksz8775WriteSwitchReg(interface, KSZ8775_INDIRECT_CTRL0,
         KSZ8775_INDIRECT_CTRL0_WRITE |
         KSZ8775_INDIRECT_CTRL0_TABLE_SEL_STATIC_MAC);

      //Trigger the write operation
      ksz8775WriteSwitchReg(interface, KSZ8775_INDIRECT_CTRL1, j);
   ...
   const SwitchFdbEntry *entry)
{
   ...
   //Loop through the static MAC table
   for(j = 0; j < KSZ8775_STATIC_MAC_TABLE_SIZE; j++)
   {
      //Read current entry
      error = ksz8775GetStaticFdbEntry(interface, j, &currentEntry);
   ...
   if(j < KSZ8775_STATIC_MAC_TABLE_SIZE)
   {
      ...
      for(i = 0; i < sizeof(Ksz8775StaticMacEntryW); i++)
      {
         ksz8775WriteSwitchReg(interface, KSZ8775_INDIRECT_DATA7 + i, 0);
      }
   ...
   SwitchFdbEntry *entry)
{
   ...
   uint_t i;
   uint8_t *p;
   Ksz8775StaticMacEntryR currentEntry;

   //Check index parameter
   if(index < KSZ8775_STATIC_MAC_TABLE_SIZE)
   {
      //Select the static MAC address table
      ksz8775WriteSwitchReg(interface, KSZ8775_INDIRECT_CTRL0,
         KSZ8775_INDIRECT_CTRL0_READ |
      ...
      ksz8775WriteSwitchReg(interface, KSZ8775_INDIRECT_CTRL1, index);

      //Point to the MAC entry
      p = (uint8_t *) &currentEntry;

      //Read indirect data registers
      for(i = 0; i < sizeof(Ksz8775StaticMacEntryR); i++)
      {
         p[i] = ksz8775ReadSwitchReg(interface, KSZ8775_INDIRECT_DATA7 + i);
      }
      ...
      if(currentEntry.valid)
      {
         ...
         entry->srcPort = 0;
         entry->destPorts = currentEntry.forwardPorts & KSZ8775_PORT_MASK;
...
void ksz8775FlushStaticFdbTable(NetInterface *interface)
{
   ...
   //Loop through the ports
   for(i = KSZ8775_PORT1; i <= KSZ8775_PORT5; i++)
   {
      //Save the current state of the port
      state[i - 1] = ksz8775ReadSwitchReg(interface, KSZ8775_PORTn_CTRL2(i));

      //Turn off learning capability
      ksz8775WriteSwitchReg(interface, KSZ8775_PORTn_CTRL2(i),
         state[i - 1] | KSZ8775_PORTn_CTRL2_LEARNING_DIS);
   }

   //All the entries associated with a port that has its learning capability
   //being turned off will be flushed
   temp = ksz8775ReadSwitchReg(interface, KSZ8775_GLOBAL_CTRL0);
   temp |= KSZ8775_GLOBAL_CTRL0_FLUSH_STATIC_MAC_TABLE;
   ksz8775WriteSwitchReg(interface, KSZ8775_GLOBAL_CTRL0, temp);

   //Loop through the ports
   for(i = KSZ8775_PORT1; i <= KSZ8775_PORT5; i++)
   {
      //Restore the original state of the port
      ksz8775WriteSwitchReg(interface, KSZ8775_PORTn_CTRL2(i), state[i - 1]);
   }
}
   ...
   SwitchFdbEntry *entry)
{
   ...
   uint_t n;
   uint8_t *p;
   Ksz8775DynamicMacEntry currentEntry;

   //Check index parameter
   if(index < KSZ8775_DYNAMIC_MAC_TABLE_SIZE)
   {
      ...
      do
      {
         ...
         ksz8775WriteSwitchReg(interface, KSZ8775_INDIRECT_CTRL0,
            KSZ8775_INDIRECT_CTRL0_READ |
            KSZ8775_INDIRECT_CTRL0_TABLE_SEL_DYNAMIC_MAC |
            (MSB(index) & KSZ8775_INDIRECT_CTRL0_ADDR_9_8));

         //Trigger the read operation
         ksz8775WriteSwitchReg(interface, KSZ8775_INDIRECT_CTRL1, LSB(index));

         //Point to the MAC entry
         p = (uint8_t *) &currentEntry;

         //Read indirect data registers
         for(i = 0; i < sizeof(Ksz8775DynamicMacEntry); i++)
         {
            p[i] = ksz8775ReadSwitchReg(interface, KSZ8775_INDIRECT_DATA8 + i);
         }
...
void ksz8775FlushDynamicFdbTable(NetInterface *interface, uint8_t port)
{
   ...
   //being turned off will be flushed
   temp = ksz8775ReadSwitchReg(interface, KSZ8775_GLOBAL_CTRL0);
   temp |= KSZ8775_GLOBAL_CTRL0_FLUSH_DYNAMIC_MAC_TABLE;
   ksz8775WriteSwitchReg(interface, KSZ8775_GLOBAL_CTRL0, temp);
   ...
   bool_t enable, uint32_t forwardPorts)
{
   ...
   temp = ksz8775ReadSwitchReg(interface, KSZ8775_GLOBAL_CTRL16);

   //Clear port map
   temp &= ~KSZ8775_GLOBAL_CTRL16_UNKNOWN_MCAST_FWD_MAP;

   //Enable or disable forwarding of unknown multicast packets
   if(enable)
   {
      //Enable forwarding
      temp |= KSZ8775_GLOBAL_CTRL16_UNKNOWN_MCAST_FWD;

      //Check whether unknown multicast packets should be forwarded to the CPU port
      if((forwardPorts & SWITCH_CPU_PORT_MASK) != 0)
      {
         temp |= KSZ8775_GLOBAL_CTRL16_UNKNOWN_MCAST_FWD_MAP_PORT5;
      }

      //Select the desired forward ports
      temp |= forwardPorts & KSZ8775_GLOBAL_CTRL16_UNKNOWN_MCAST_FWD_MAP_ALL;
   }
   else
   {
      //Disable forwarding
   ...
   ksz8775WriteSwitchReg(interface, KSZ8775_GLOBAL_CTRL16, temp);
}
...
void ksz8775DumpPhyReg(NetInterface *interface, uint8_t port)
{
   ...
   for(i = 0; i < 32; i++)
   {
      ...
      TRACE_DEBUG("%02" PRIu8 ": 0x%04" PRIX16 "\r\n", i,
         ksz8775ReadPhyReg(interface, port, i));
   }
   ...
   uint8_t data)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      command = KSZ8775_SPI_CMD_WRITE;
      //Set register address
      command |= (address << 1) & KSZ8775_SPI_CMD_ADDR;
...
void ksz8775DumpSwitchReg(NetInterface *interface)
{
   ...
   for(i = 0; i < 256; i++)
   {
      ...
      TRACE_DEBUG("0x%02" PRIX16 " (%02" PRIu16 ") : 0x%02" PRIX8 "\r\n",
         i, i, ksz8775ReadSwitchReg(interface, i));
   }
