...
//Dependencies
#include "core/net.h"
#include "drivers/switch/lan9353_driver.h"
#include "debug.h"


/**
 * @brief LAN9353 Ethernet switch driver
 **/
...
/**
 * @brief LAN9353 Ethernet switch initialization
...
error_t lan9353Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing LAN9353...\r\n");
   ...
   do
   {
      ...
      value = lan9353ReadSysReg(interface, LAN9353_BYTE_TEST);
   ...
   do
   {
      ...
      value = lan9353ReadSysReg(interface, LAN9353_HW_CFG);
   ...
   lan9353WriteSwitchReg(interface, LAN9353_SWE_INGRSS_PORT_TYP,
      LAN9353_SWE_INGRSS_PORT_TYP_PORT0);

   //Configure egress VLAN tagging rules
   lan9353WriteSwitchReg(interface, LAN9353_BM_EGRSS_PORT_TYPE,
      LAN9353_BM_EGRSS_PORT_TYPE_PORT0_TYPE_CPU);
#else
   //Disable special VLAN tagging mode
   lan9353WriteSwitchReg(interface, LAN9353_SWE_INGRSS_PORT_TYP, 0);

   //Revert to default configuration
   lan9353WriteSwitchReg(interface, LAN9353_BM_EGRSS_PORT_TYPE, 0);
#endif
   ...
   if(interface->port != 0)
   {
      ...
      lan9353WriteSwitchReg(interface, LAN9353_SWE_PORT_MIRROR,
      ...
      lan9353WriteSwitchReg(interface, LAN9353_SWE_PORT_STATE,
...
#endif
   {
      ...
      lan9353WriteSwitchReg(interface, LAN9353_SWE_PORT_MIRROR, 0);
   ...
   lan9353WriteSwitchReg(interface, LAN9353_MAC_RX_CFG(0),
      LAN9353_MAC_RX_CFG_REJECT_MAC_TYPES | LAN9353_MAC_RX_CFG_RX_EN);

   //Configure port 0 transmit parameters
   lan9353WriteSwitchReg(interface, LAN9353_MAC_TX_CFG(0),
      LAN9353_MAC_TX_CFG_IFG_CONFIG_DEFAULT | LAN9353_MAC_TX_CFG_TX_PAD_EN |
      LAN9353_MAC_TX_CFG_TX_EN);

   //Loop through the ports
   for(port = LAN9353_PORT1; port <= LAN9353_PORT2; port++)
   {
      //Debug message
      TRACE_DEBUG("Port %u:\r\n", port);
      //Dump PHY registers for debugging purpose
      lan9353DumpPhyReg(interface, port);
   }

   //Perform custom configuration
   lan9353InitHook(interface);
...
/**
 * @brief LAN9353 custom configuration
 * @param[in] interface Underlying network interface
 **/
...
void lan9353Tick(NetInterface *interface)
{
   ...
   if(interface->port != 0)
   {
      ...
      for(i = 0; i < NET_INTERFACE_COUNT; i++)
      {
            ...
            virtualInterface->parent == interface)
         {
            ...
            linkState = lan9353GetLinkState(interface, virtualInterface->port);
...
#endif
   {
      ...
      {
         //Retrieve current link state
         if(lan9353GetLinkState(interface, port))
         {
            linkState = TRUE;
         }
      }
   ...
   if(interface->port != 0)
   {
      ...
      for(i = 0; i < NET_INTERFACE_COUNT; i++)
      {
            ...
            virtualInterface->parent == interface)
         {
            ...
            //Valid port?
            if(port >= LAN9353_PORT1 && port <= LAN9353_PORT2)
            {
               //Retrieve current link state
               linkState = lan9353GetLinkState(interface, port);
               ...
               if(linkState && !virtualInterface->linkState)
               {
                  ...
                  status = lan9353ReadPhyReg(interface, port, LAN9353_PSCSR);

                  //Check current operation mode
                  switch(status & LAN9353_PSCSR_SPEED)
                  {
                  //10BASE-T half-duplex
                  case LAN9353_PSCSR_SPEED_10BT_HD:
                     virtualInterface->linkSpeed = NIC_LINK_SPEED_10MBPS;
                     virtualInterface->duplexMode = NIC_HALF_DUPLEX_MODE;
                     break;

                  //10BASE-T full-duplex
                  case LAN9353_PSCSR_SPEED_10BT_FD:
                     virtualInterface->linkSpeed = NIC_LINK_SPEED_10MBPS;
                     virtualInterface->duplexMode = NIC_FULL_DUPLEX_MODE;
                     break;

                  //100BASE-TX half-duplex
                  case LAN9353_PSCSR_SPEED_100BTX_HD:
                     virtualInterface->linkSpeed = NIC_LINK_SPEED_100MBPS;
                     virtualInterface->duplexMode = NIC_HALF_DUPLEX_MODE;
                     break;

                  //100BASE-TX full-duplex
                  case LAN9353_PSCSR_SPEED_100BTX_FD:
                     virtualInterface->linkSpeed = NIC_LINK_SPEED_100MBPS;
                     virtualInterface->duplexMode = NIC_FULL_DUPLEX_MODE;
                     break;
...
#if (ETH_PORT_TAGGING_SUPPORT == ENABLED)
   //Valid port?
   if(ancillary->port <= LAN9353_PORT2)
   {
      ...
      if(*offset >= sizeof(VlanTag))
      {
         ...
         *offset -= sizeof(VlanTag);
         ...
         if(ancillary->port == 0)
         {
            ...
            vlanTag->tci = htons(LAN9353_VID_ALR_LOOKUP);
         }
         else
         {
            //VID bits 0 and 1 specify the destination port
            vlanTag->tci = htons(LAN9353_VID_STP_OVERRIDE | ancillary->port);
         }
   ...
   size_t *length, NetRxAncillary *ancillary)
{
   ...
   if(ntohs(header->type) == ETH_TYPE_VLAN)
   {
      ...
      if(*length >= (sizeof(EthHeader) + sizeof(VlanTag)))
      {
         ...
         ancillary->port = ntohs(vlanTag->tci) & LAN9353_VID_SRC_PORT;
...
bool_t lan9353GetLinkState(NetInterface *interface, uint8_t port)
{
   ...
   if(port >= LAN9353_PORT1 && port <= LAN9353_PORT2)
   {
      ...
      status = lan9353ReadPhyReg(interface, port, LAN9353_BMSR);

      //Retrieve current link state
      linkState = (status & LAN9353_BMSR_LINK_STATUS) ? TRUE : FALSE;
   }
   else
   {
   ...
   SwitchPortState state)
{
   ...
   temp = lan9353ReadSwitchReg(interface, LAN9353_SWE_PORT_STATE);

   //Check port number
   if(port == LAN9353_PORT1)
   {
      //Clear current configuration
      temp &= ~LAN9353_SWE_PORT_STATE_PORT1;

      //Update port 1 state
      switch(state)
      {
      case SWITCH_PORT_STATE_BLOCKING:
         temp |= LAN9353_SWE_PORT_STATE_PORT1_LISTENING;
         break;
      case SWITCH_PORT_STATE_LEARNING:
         temp |= LAN9353_SWE_PORT_STATE_PORT1_LEARNING;
         break;
      case SWITCH_PORT_STATE_FORWARDING:
         temp |= LAN9353_SWE_PORT_STATE_PORT1_FORWARDING;
         break;
      default:
         temp |= LAN9353_SWE_PORT_STATE_PORT1_DISABLED;
         break;
      }
   }
   else if(port == LAN9353_PORT2)
   {
      //Clear current configuration
      temp &= ~LAN9353_SWE_PORT_STATE_PORT2;

      //Update port 2 state
      switch(state)
      {
      case SWITCH_PORT_STATE_BLOCKING:
         temp |= LAN9353_SWE_PORT_STATE_PORT2_LISTENING;
         break;
      case SWITCH_PORT_STATE_LEARNING:
         temp |= LAN9353_SWE_PORT_STATE_PORT2_LEARNING;
         break;
      case SWITCH_PORT_STATE_FORWARDING:
         temp |= LAN9353_SWE_PORT_STATE_PORT2_FORWARDING;
         break;
      default:
         temp |= LAN9353_SWE_PORT_STATE_PORT2_DISABLED;
         break;
      }
   }
   else
   ...
   lan9353WriteSwitchReg(interface, LAN9353_SWE_PORT_STATE, temp);
}
...
SwitchPortState lan9353GetPortState(NetInterface *interface, uint8_t port)
{
   ...
   {
      //Check port 1 state
      switch(temp & LAN9353_SWE_PORT_STATE_PORT1)
      {
      case LAN9353_SWE_PORT_STATE_PORT1_LISTENING:
         state = SWITCH_PORT_STATE_BLOCKING;
         break;
      case LAN9353_SWE_PORT_STATE_PORT1_LEARNING:
         state = SWITCH_PORT_STATE_LEARNING;
         break;
      case LAN9353_SWE_PORT_STATE_PORT1_FORWARDING:
   ...
   {
      //Check port 2 state
      switch(temp & LAN9353_SWE_PORT_STATE_PORT2)
      {
      case LAN9353_SWE_PORT_STATE_PORT2_LISTENING:
         state = SWITCH_PORT_STATE_BLOCKING;
         break;
      case LAN9353_SWE_PORT_STATE_PORT2_LEARNING:
         state = SWITCH_PORT_STATE_LEARNING;
         break;
      case LAN9353_SWE_PORT_STATE_PORT2_FORWARDING:
         state = SWITCH_PORT_STATE_FORWARDING;
         break;
      default:
         state = SWITCH_PORT_STATE_DISABLED;
...
void lan9353SetAgingTime(NetInterface *interface, uint32_t agingTime)
{
   ...
   temp = ((agingTime - 1) << 16) & LAN9353_SWE_ALR_CFG_AGING_TIME;

   //Write the value to the Switch Engine ALR Configuration register
   lan9353WriteSwitchReg(interface, LAN9353_SWE_ALR_CFG, temp |
...
void lan9353EnableIgmpSnooping(NetInterface *interface, bool_t enable)
{
   ...
   temp = lan9353ReadSwitchReg(interface, LAN9353_SWE_GLOBAL_INGRSS_CFG);

   //Enable or disable IGMP monitoring
   if(enable)
   {
      temp |= LAN9353_SWE_GLOBAL_INGRSS_CFG_IGMP_MONITORING_EN;
   }
   else
   {
      temp &= ~LAN9353_SWE_GLOBAL_INGRSS_CFG_IGMP_MONITORING_EN;
   ...
   temp = (temp & ~LAN9353_SWE_GLOBAL_INGRSS_CFG_MLD_IGMP_MONITOR_PORT) |
   ...
   lan9353WriteSwitchReg(interface, LAN9353_SWE_GLOBAL_INGRSS_CFG, temp);
}
...
void lan9353EnableMldSnooping(NetInterface *interface, bool_t enable)
{
   ...
   if(enable)
   {
      temp |= LAN9353_SWE_GLOBAL_INGRSS_CFG_MLD_MONITORING_EN;
   }
   else
   {
      temp &= ~LAN9353_SWE_GLOBAL_INGRSS_CFG_MLD_MONITORING_EN;
...
void lan9353EnableRsvdMcastTable(NetInterface *interface, bool_t enable)
{
   ...
   for(i = 0; i <= 15; i++)
   {
      ...
      if(enable)
      {
         lan9353AddStaticFdbEntry(interface, &entry);
      }
      else
      {
         lan9353DeleteStaticFdbEntry(interface, &entry);
      }
   }
}
   ...
   const SwitchFdbEntry *entry)
{
   ...
   {
      ports = LAN9353_PORT0_MASK;
   }
   else
   {
      ports = entry->destPorts & LAN9353_PORT_MASK;
   }
   ...
   if(ports != 0)
   {
      ...
      value = LAN9353_SWE_ALR_WR_DAT_1_VALID | LAN9353_SWE_ALR_WR_DAT_1_STATIC;
      ...
      if(entry->override)
      {
         value |= LAN9353_SWE_ALR_WR_DAT_1_AGE1_OVERRIDE;
      }

      //Set the ports associated with this MAC address
      switch(ports)
      {
      case LAN9353_PORT0_MASK:
         value |= LAN9353_SWE_ALR_WR_DAT_1_PORT_0;
         break;
      case LAN9353_PORT1_MASK:
         value |= LAN9353_SWE_ALR_WR_DAT_1_PORT_1;
         break;
      case LAN9353_PORT2_MASK:
         value |= LAN9353_SWE_ALR_WR_DAT_1_PORT_2;
         break;
      case LAN9353_PORT0_1_MASK:
         value |= LAN9353_SWE_ALR_WR_DAT_1_PORT_0_1;
         break;
      case LAN9353_PORT0_2_MASK:
         value |= LAN9353_SWE_ALR_WR_DAT_1_PORT_0_2;
         break;
      case LAN9353_PORT1_2_MASK:
         value |= LAN9353_SWE_ALR_WR_DAT_1_PORT_1_2;
         break;
      default:
         value |= LAN9353_SWE_ALR_WR_DAT_1_PORT_0_1_2;
         break;
      }
   }
   else
   ...
   lan9353WriteSwitchReg(interface, LAN9353_SWE_ALR_WR_DAT_0, value);

   //Write the SWE_ALR_CMD register
   lan9353WriteSwitchReg(interface, LAN9353_SWE_ALR_CMD,
   ...
   do
   {
      ...
      value = lan9353ReadSwitchReg(interface, LAN9353_SWE_ALR_CMD_STS);

      //Check the Make Pending bit
   } while((value & LAN9353_SWE_ALR_CMD_STS_OPERATION_PENDING) != 0);

   //Clear the SWE_ALR_CMD register
   lan9353WriteSwitchReg(interface, LAN9353_SWE_ALR_CMD, 0);
   ...
   SwitchFdbEntry *entry)
{
   ...
   while(1)
   {
      ...
      if(index == 0)
      {
         value = LAN9353_SWE_ALR_CMD_GET_FIRST_ENTRY;
      }
      else
      {
         value = LAN9353_SWE_ALR_CMD_GET_NEXT_ENTRY;
      }

      //Write the SWE_ALR_CMD register
      lan9353WriteSwitchReg(interface, LAN9353_SWE_ALR_CMD, value);
      //Clear the SWE_ALR_CMD register
      lan9353WriteSwitchReg(interface, LAN9353_SWE_ALR_CMD, 0);
      ...
      while(1)
      {
         ...
         value = lan9353ReadSwitchReg(interface, LAN9353_SWE_ALR_RD_DAT_1);

         //If the End of Table bit is set, then exit
         if((value & LAN9353_SWE_ALR_RD_DAT_1_END_OF_TABLE) != 0)
         {
            return ERROR_END_OF_TABLE;
         }

         //If the Valid bit is set, then the entry is valid
         if((value & LAN9353_SWE_ALR_RD_DAT_1_VALID) != 0)
         {
            break;
         }
      }
      ...
      //Static entry?
      if((value & LAN9353_SWE_ALR_RD_DAT_1_STATIC) != 0)
      {
         break;
      }
   ...
   switch(value & LAN9353_SWE_ALR_RD_DAT_1_PORT)
   {
   case LAN9353_SWE_ALR_RD_DAT_1_PORT_0:
      entry->destPorts = LAN9353_PORT0_MASK;
      break;
   case LAN9353_SWE_ALR_RD_DAT_1_PORT_1:
      entry->destPorts = LAN9353_PORT1_MASK;
      break;
   case LAN9353_SWE_ALR_RD_DAT_1_PORT_2:
      entry->destPorts = LAN9353_PORT2_MASK;
      break;
   case LAN9353_SWE_ALR_RD_DAT_1_PORT_0_1:
      entry->destPorts = LAN9353_PORT0_1_MASK;
      break;
   case LAN9353_SWE_ALR_RD_DAT_1_PORT_0_2:
      entry->destPorts = LAN9353_PORT0_2_MASK;
      break;
   case LAN9353_SWE_ALR_RD_DAT_1_PORT_1_2:
      entry->destPorts = LAN9353_PORT1_2_MASK;
      break;
   case LAN9353_SWE_ALR_RD_DAT_1_PORT_0_1_2:
      entry->destPorts = LAN9353_PORT0_1_2_MASK;
      break;
   default:
      entry->destPorts = 0;
      break;
   ...
   value = lan9353ReadSwitchReg(interface, LAN9353_SWE_ALR_RD_DAT_0);
...
void lan9353FlushStaticFdbTable(NetInterface *interface)
{
   ...
   //Loop through the ALR table
   for(i = 0; i < LAN9353_ALR_TABLE_SIZE; i++)
   {
      //Read current entry
      error = lan9353GetStaticFdbEntry(interface, i, &entry);
   ...
   SwitchFdbEntry *entry)
{
   ...
   {
   case LAN9353_SWE_ALR_RD_DAT_1_PORT_0:
      entry->srcPort = LAN9353_PORT0;
      break;
   case LAN9353_SWE_ALR_RD_DAT_1_PORT_1:
      entry->srcPort = LAN9353_PORT1;
      break;
   case LAN9353_SWE_ALR_RD_DAT_1_PORT_2:
      entry->srcPort = LAN9353_PORT2;
      break;
   default:
      entry->srcPort = 0;
      break;
...
void lan9353FlushDynamicFdbTable(NetInterface *interface, uint8_t port)
{
   ...
   {
      //Read current entry
      error = lan9353GetDynamicFdbEntry(interface, i, &entry);
...
void lan9353DumpPhyReg(NetInterface *interface, uint8_t port)
{
   ...
   for(i = 0; i < 32; i++)
   {
      ...
      TRACE_DEBUG("%02" PRIu8 ": 0x%04" PRIX16 "\r\n", i,
         lan9353ReadPhyReg(interface, port, i));
   }
   ...
   uint32_t data)
{
   ...
   lan9353WriteSysReg(interface, LAN9353_SWITCH_CSR_DATA, data);
   ...
   lan9353WriteSysReg(interface, LAN9353_SWITCH_CSR_CMD, value);
   ...
   do
   {
      ...
      value = lan9353ReadSysReg(interface, LAN9353_SWITCH_CSR_CMD);
...
uint32_t lan9353ReadSwitchReg(NetInterface *interface, uint16_t address)
{
   ...
   value = LAN9353_SWITCH_CSR_CMD_BUSY | LAN9353_SWITCH_CSR_CMD_READ |
      LAN9353_SWITCH_CSR_CMD_BE;
   ...
   return lan9353ReadSysReg(interface, LAN9353_SWITCH_CSR_DATA);
}
