...
/**
 * @brief LAN9355 Ethernet switch driver
 **/
...
/**
 * @brief LAN9355 Ethernet switch initialization
...
error_t lan9355Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing LAN9355...\r\n");
   ...
   do
   {
      ...
      value = lan9355ReadSysReg(interface, LAN9355_BYTE_TEST);

      //The returned data is invalid until the serial interface reset is
      //complete
   } while(value != LAN9355_BYTE_TEST_DEFAULT);
   ...
   do
   {
      ...
      value = lan9355ReadSysReg(interface, LAN9355_HW_CFG);

      //When set, the READY bit indicates that the reset has completed and
      //the device is ready to be accessed
   } while((value & LAN9355_HW_CFG_DEVICE_READY) == 0);

#if (ETH_PORT_TAGGING_SUPPORT == ENABLED)
   //Enable special VLAN tagging mode
   lan9355WriteSwitchReg(interface, LAN9355_SWE_INGRSS_PORT_TYP,
      LAN9355_SWE_INGRSS_PORT_TYP_PORT0);

   //Configure egress VLAN tagging rules
   lan9355WriteSwitchReg(interface, LAN9355_BM_EGRSS_PORT_TYPE,
      LAN9355_BM_EGRSS_PORT_TYPE_PORT0_TYPE_CPU);
#else
   //Disable special VLAN tagging mode
   lan9355WriteSwitchReg(interface, LAN9355_SWE_INGRSS_PORT_TYP, 0);

   //Revert to default configuration
   lan9355WriteSwitchReg(interface, LAN9355_BM_EGRSS_PORT_TYPE, 0);
#endif
   ...
   if(interface->port != 0)
   {
      ...
      lan9355WriteSwitchReg(interface, LAN9355_SWE_PORT_MIRROR,
         LAN9355_SWE_PORT_MIRROR_RX_MIRRORING_FILT_EN |
         LAN9355_SWE_PORT_MIRROR_SNIFFER_PORT_0 |
         LAN9355_SWE_PORT_MIRROR_MIRRORED_PORT_2 |
         LAN9355_SWE_PORT_MIRROR_MIRRORED_PORT_1 |
         LAN9355_SWE_PORT_MIRROR_RX_MIRRORING_EN);

      //Configure port state
      lan9355WriteSwitchReg(interface, LAN9355_SWE_PORT_STATE,
         LAN9355_SWE_PORT_STATE_PORT2_LISTENING |
         LAN9355_SWE_PORT_STATE_PORT1_LISTENING |
         LAN9355_SWE_PORT_STATE_PORT0_FORWARDING);
   }
   else
#endif
   {
      //Disable port mirroring
      lan9355WriteSwitchReg(interface, LAN9355_SWE_PORT_MIRROR, 0);

      //Configure port state
      lan9355WriteSwitchReg(interface, LAN9355_SWE_PORT_STATE,
         LAN9355_SWE_PORT_STATE_PORT2_FORWARDING |
         LAN9355_SWE_PORT_STATE_PORT1_FORWARDING |
         LAN9355_SWE_PORT_STATE_PORT0_FORWARDING);
   }

   //Configure port 0 receive parameters
   lan9355WriteSwitchReg(interface, LAN9355_MAC_RX_CFG(0),
      LAN9355_MAC_RX_CFG_REJECT_MAC_TYPES | LAN9355_MAC_RX_CFG_RX_EN);

   //Configure port 0 transmit parameters
   lan9355WriteSwitchReg(interface, LAN9355_MAC_TX_CFG(0),
      LAN9355_MAC_TX_CFG_IFG_CONFIG_DEFAULT | LAN9355_MAC_TX_CFG_TX_PAD_EN |
      LAN9355_MAC_TX_CFG_TX_EN);

   //Loop through the ports
   for(port = LAN9355_PORT1; port <= LAN9355_PORT2; port++)
   {
      //Debug message
      TRACE_DEBUG("Port %u:\r\n", port);
      //Dump PHY registers for debugging purpose
      lan9355DumpPhyReg(interface, port);
   }

   //Perform custom configuration
   lan9355InitHook(interface);
...
/**
 * @brief LAN9355 custom configuration
 * @param[in] interface Underlying network interface
 **/
...
void lan9355Tick(NetInterface *interface)
{
   ...
   if(interface->port != 0)
   {
      ...
      for(i = 0; i < NET_INTERFACE_COUNT; i++)
      {
            ...
            virtualInterface->parent == interface)
         {
            ...
            linkState = lan9355GetLinkState(interface, virtualInterface->port);
...
#endif
   {
      ...
      {
         //Retrieve current link state
         if(lan9355GetLinkState(interface, port))
         {
            linkState = TRUE;
         }
      }
   ...
   if(interface->port != 0)
   {
      ...
      for(i = 0; i < NET_INTERFACE_COUNT; i++)
      {
            ...
            virtualInterface->parent == interface)
         {
            ...
            //Valid port?
            if(port >= LAN9355_PORT1 && port <= LAN9355_PORT2)
            {
               //Retrieve current link state
               linkState = lan9355GetLinkState(interface, port);
               ...
               if(linkState && !virtualInterface->linkState)
               {
                  ...
                  status = lan9355ReadPhyReg(interface, port, LAN9355_PSCSR);

                  //Check current operation mode
                  switch(status & LAN9355_PSCSR_SPEED)
                  {
                  //10BASE-T half-duplex
                  case LAN9355_PSCSR_SPEED_10BT_HD:
                     virtualInterface->linkSpeed = NIC_LINK_SPEED_10MBPS;
                     virtualInterface->duplexMode = NIC_HALF_DUPLEX_MODE;
                     break;

                  //10BASE-T full-duplex
                  case LAN9355_PSCSR_SPEED_10BT_FD:
                     virtualInterface->linkSpeed = NIC_LINK_SPEED_10MBPS;
                     virtualInterface->duplexMode = NIC_FULL_DUPLEX_MODE;
                     break;

                  //100BASE-TX half-duplex
                  case LAN9355_PSCSR_SPEED_100BTX_HD:
                     virtualInterface->linkSpeed = NIC_LINK_SPEED_100MBPS;
                     virtualInterface->duplexMode = NIC_HALF_DUPLEX_MODE;
                     break;

                  //100BASE-TX full-duplex
                  case LAN9355_PSCSR_SPEED_100BTX_FD:
                     virtualInterface->linkSpeed = NIC_LINK_SPEED_100MBPS;
                     virtualInterface->duplexMode = NIC_FULL_DUPLEX_MODE;
                     break;
...
#if (ETH_PORT_TAGGING_SUPPORT == ENABLED)
   //Valid port?
   if(ancillary->port <= LAN9355_PORT2)
   {
      ...
      if(*offset >= sizeof(VlanTag))
      {
         ...
         *offset -= sizeof(VlanTag);
         ...
         if(ancillary->port == 0)
         {
            ...
            vlanTag->tci = htons(LAN9355_VID_ALR_LOOKUP);
         }
         else
         {
            //VID bits 0 and 1 specify the destination port
            vlanTag->tci = htons(LAN9355_VID_STP_OVERRIDE | ancillary->port);
         }
   ...
   size_t *length, NetRxAncillary *ancillary)
{
   ...
   if(ntohs(header->type) == ETH_TYPE_VLAN)
   {
      ...
      if(*length >= (sizeof(EthHeader) + sizeof(VlanTag)))
      {
         ...
         ancillary->port = ntohs(vlanTag->tci) & LAN9355_VID_SRC_PORT;
...
bool_t lan9355GetLinkState(NetInterface *interface, uint8_t port)
{
   ...
   if(port >= LAN9355_PORT1 && port <= LAN9355_PORT2)
   {
      ...
      status = lan9355ReadPhyReg(interface, port, LAN9355_BMSR);

      //Retrieve current link state
      linkState = (status & LAN9355_BMSR_LINK_STATUS) ? TRUE : FALSE;
   }
   else
   {
   ...
   SwitchPortState state)
{
   ...
   temp = lan9355ReadSwitchReg(interface, LAN9355_SWE_PORT_STATE);

   //Check port number
   if(port == LAN9355_PORT1)
   {
      //Clear current configuration
      temp &= ~LAN9355_SWE_PORT_STATE_PORT1;

      //Update port 1 state
      switch(state)
      {
      case SWITCH_PORT_STATE_BLOCKING:
         temp |= LAN9355_SWE_PORT_STATE_PORT1_LISTENING;
         break;
      case SWITCH_PORT_STATE_LEARNING:
         temp |= LAN9355_SWE_PORT_STATE_PORT1_LEARNING;
         break;
      case SWITCH_PORT_STATE_FORWARDING:
         temp |= LAN9355_SWE_PORT_STATE_PORT1_FORWARDING;
         break;
      default:
         temp |= LAN9355_SWE_PORT_STATE_PORT1_DISABLED;
         break;
      }
   }
   else if(port == LAN9355_PORT2)
   {
      //Clear current configuration
      temp &= ~LAN9355_SWE_PORT_STATE_PORT2;

      //Update port 2 state
      switch(state)
      {
      case SWITCH_PORT_STATE_BLOCKING:
         temp |= LAN9355_SWE_PORT_STATE_PORT2_LISTENING;
         break;
      case SWITCH_PORT_STATE_LEARNING:
         temp |= LAN9355_SWE_PORT_STATE_PORT2_LEARNING;
         break;
      case SWITCH_PORT_STATE_FORWARDING:
         temp |= LAN9355_SWE_PORT_STATE_PORT2_FORWARDING;
         break;
      default:
         temp |= LAN9355_SWE_PORT_STATE_PORT2_DISABLED;
         break;
      }
   }
   else
   ...
   lan9355WriteSwitchReg(interface, LAN9355_SWE_PORT_STATE, temp);
}
...
SwitchPortState lan9355GetPortState(NetInterface *interface, uint8_t port)
{
   ...
   {
      //Check port 1 state
      switch(temp & LAN9355_SWE_PORT_STATE_PORT1)
      {
      case LAN9355_SWE_PORT_STATE_PORT1_LISTENING:
         state = SWITCH_PORT_STATE_BLOCKING;
         break;
      case LAN9355_SWE_PORT_STATE_PORT1_LEARNING:
         state = SWITCH_PORT_STATE_LEARNING;
         break;
      case LAN9355_SWE_PORT_STATE_PORT1_FORWARDING:
   ...
   {
      //Check port 2 state
      switch(temp & LAN9355_SWE_PORT_STATE_PORT2)
      {
      case LAN9355_SWE_PORT_STATE_PORT2_LISTENING:
         state = SWITCH_PORT_STATE_BLOCKING;
         break;
      case LAN9355_SWE_PORT_STATE_PORT2_LEARNING:
         state = SWITCH_PORT_STATE_LEARNING;
         break;
      case LAN9355_SWE_PORT_STATE_PORT2_FORWARDING:
         state = SWITCH_PORT_STATE_FORWARDING;
         break;
      default:
         state = SWITCH_PORT_STATE_DISABLED;
...
void lan9355SetAgingTime(NetInterface *interface, uint32_t agingTime)
{
   ...
   temp = ((agingTime - 1) << 16) & LAN9355_SWE_ALR_CFG_AGING_TIME;

   //Write the value to the Switch Engine ALR Configuration register
   lan9355WriteSwitchReg(interface, LAN9355_SWE_ALR_CFG, temp |
      LAN9355_SWE_ALR_CFG_ALR_AGE_EN);
}
...
void lan9355EnableIgmpSnooping(NetInterface *interface, bool_t enable)
{
   ...
   temp = lan9355ReadSwitchReg(interface, LAN9355_SWE_GLOBAL_INGRSS_CFG);

   //Enable or disable IGMP monitoring
   if(enable)
   {
      temp |= LAN9355_SWE_GLOBAL_INGRSS_CFG_IGMP_MONITORING_EN;
   }
   else
   {
      temp &= ~LAN9355_SWE_GLOBAL_INGRSS_CFG_IGMP_MONITORING_EN;
   ...
   temp = (temp & ~LAN9355_SWE_GLOBAL_INGRSS_CFG_MLD_IGMP_MONITOR_PORT) |
      LAN9355_SWE_GLOBAL_INGRSS_CFG_MLD_IGMP_MONITOR_PORT_0;

   //Write the value back to Switch Engine Global Ingress Configuration register
   lan9355WriteSwitchReg(interface, LAN9355_SWE_GLOBAL_INGRSS_CFG, temp);
}
...
void lan9355EnableMldSnooping(NetInterface *interface, bool_t enable)
{
   ...
   if(enable)
   {
      temp |= LAN9355_SWE_GLOBAL_INGRSS_CFG_MLD_MONITORING_EN;
   }
   else
   {
      temp &= ~LAN9355_SWE_GLOBAL_INGRSS_CFG_MLD_MONITORING_EN;
...
void lan9355EnableRsvdMcastTable(NetInterface *interface, bool_t enable)
{
   ...
   for(i = 0; i <= 15; i++)
   {
      ...
      if(enable)
      {
         lan9355AddStaticFdbEntry(interface, &entry);
      }
      else
      {
         lan9355DeleteStaticFdbEntry(interface, &entry);
      }
   }
}
   ...
   const SwitchFdbEntry *entry)
{
   ...
   {
      ports = LAN9355_PORT0_MASK;
   }
   else
   {
      ports = entry->destPorts & LAN9355_PORT_MASK;
   }
   ...
   if(ports != 0)
   {
      ...
      value = LAN9355_SWE_ALR_WR_DAT_1_VALID | LAN9355_SWE_ALR_WR_DAT_1_STATIC;
      ...
      if(entry->override)
      {
         value |= LAN9355_SWE_ALR_WR_DAT_1_AGE1_OVERRIDE;
      }

      //Set the ports associated with this MAC address
      switch(ports)
      {
      case LAN9355_PORT0_MASK:
         value |= LAN9355_SWE_ALR_WR_DAT_1_PORT_0;
         break;
      case LAN9355_PORT1_MASK:
         value |= LAN9355_SWE_ALR_WR_DAT_1_PORT_1;
         break;
      case LAN9355_PORT2_MASK:
         value |= LAN9355_SWE_ALR_WR_DAT_1_PORT_2;
         break;
      case LAN9355_PORT0_1_MASK:
         value |= LAN9355_SWE_ALR_WR_DAT_1_PORT_0_1;
         break;
      case LAN9355_PORT0_2_MASK:
         value |= LAN9355_SWE_ALR_WR_DAT_1_PORT_0_2;
         break;
      case LAN9355_PORT1_2_MASK:
         value |= LAN9355_SWE_ALR_WR_DAT_1_PORT_1_2;
         break;
      default:
         value |= LAN9355_SWE_ALR_WR_DAT_1_PORT_0_1_2;
         break;
      }
   }
   else
   ...
   lan9355WriteSwitchReg(interface, LAN9355_SWE_ALR_WR_DAT_0, value);

   //Write the SWE_ALR_CMD register
   lan9355WriteSwitchReg(interface, LAN9355_SWE_ALR_CMD,
      LAN9355_SWE_ALR_CMD_MAKE_ENTRY);
   ...
   do
   {
      ...
      value = lan9355ReadSwitchReg(interface, LAN9355_SWE_ALR_CMD_STS);

      //Check the Make Pending bit
   } while((value & LAN9355_SWE_ALR_CMD_STS_OPERATION_PENDING) != 0);

   //Clear the SWE_ALR_CMD register
   lan9355WriteSwitchReg(interface, LAN9355_SWE_ALR_CMD, 0);
   ...
   SwitchFdbEntry *entry)
{
   ...
   {
      //Out of bound index?
      if(index >= LAN9355_ALR_TABLE_SIZE)
         return ERROR_END_OF_TABLE;

      //First entry?
      if(index == 0)
      {
         value = LAN9355_SWE_ALR_CMD_GET_FIRST_ENTRY;
      }
      else
      {
         value = LAN9355_SWE_ALR_CMD_GET_NEXT_ENTRY;
      }

      //Write the SWE_ALR_CMD register
      lan9355WriteSwitchReg(interface, LAN9355_SWE_ALR_CMD, value);
      //Clear the SWE_ALR_CMD register
      lan9355WriteSwitchReg(interface, LAN9355_SWE_ALR_CMD, 0);
      ...
      while(1)
      {
         ...
         value = lan9355ReadSwitchReg(interface, LAN9355_SWE_ALR_RD_DAT_1);

         //If the End of Table bit is set, then exit
         if((value & LAN9355_SWE_ALR_RD_DAT_1_END_OF_TABLE) != 0)
         {
            return ERROR_END_OF_TABLE;
         }

         //If the Valid bit is set, then the entry is valid
         if((value & LAN9355_SWE_ALR_RD_DAT_1_VALID) != 0)
         {
            break;
         }
      }
      ...
      //Static entry?
      if((value & LAN9355_SWE_ALR_RD_DAT_1_STATIC) != 0)
      {
         break;
      }
   ...
   switch(value & LAN9355_SWE_ALR_RD_DAT_1_PORT)
   {
   case LAN9355_SWE_ALR_RD_DAT_1_PORT_0:
      entry->destPorts = LAN9355_PORT0_MASK;
      break;
   case LAN9355_SWE_ALR_RD_DAT_1_PORT_1:
      entry->destPorts = LAN9355_PORT1_MASK;
      break;
   case LAN9355_SWE_ALR_RD_DAT_1_PORT_2:
      entry->destPorts = LAN9355_PORT2_MASK;
      break;
   case LAN9355_SWE_ALR_RD_DAT_1_PORT_0_1:
      entry->destPorts = LAN9355_PORT0_1_MASK;
      break;
   case LAN9355_SWE_ALR_RD_DAT_1_PORT_0_2:
      entry->destPorts = LAN9355_PORT0_2_MASK;
      break;
   case LAN9355_SWE_ALR_RD_DAT_1_PORT_1_2:
      entry->destPorts = LAN9355_PORT1_2_MASK;
      break;
   case LAN9355_SWE_ALR_RD_DAT_1_PORT_0_1_2:
      entry->destPorts = LAN9355_PORT0_1_2_MASK;
      break;
   default:
      entry->destPorts = 0;
      break;
   ...
   value = lan9355ReadSwitchReg(interface, LAN9355_SWE_ALR_RD_DAT_0);
...
void lan9355FlushStaticFdbTable(NetInterface *interface)
{
   ...
   //Loop through the ALR table
   for(i = 0; i < LAN9355_ALR_TABLE_SIZE; i++)
   {
      //Read current entry
      error = lan9355GetStaticFdbEntry(interface, i, &entry);
   ...
   SwitchFdbEntry *entry)
{
   ...
   {
   case LAN9355_SWE_ALR_RD_DAT_1_PORT_0:
      entry->srcPort = LAN9355_PORT0;
      break;
   case LAN9355_SWE_ALR_RD_DAT_1_PORT_1:
      entry->srcPort = LAN9355_PORT1;
      break;
   case LAN9355_SWE_ALR_RD_DAT_1_PORT_2:
      entry->srcPort = LAN9355_PORT2;
      break;
   default:
      entry->srcPort = 0;
      break;
...
void lan9355FlushDynamicFdbTable(NetInterface *interface, uint8_t port)
{
   ...
   {
      //Read current entry
      error = lan9355GetDynamicFdbEntry(interface, i, &entry);
...
void lan9355DumpPhyReg(NetInterface *interface, uint8_t port)
{
   ...
   for(i = 0; i < 32; i++)
   {
      ...
      TRACE_DEBUG("%02" PRIu8 ": 0x%04" PRIX16 "\r\n", i,
         lan9355ReadPhyReg(interface, port, i));
   }
   ...
   uint32_t data)
{
   ...
   lan9355WriteSysReg(interface, LAN9355_SWITCH_CSR_DATA, data);

   //Set up a write operation
   value = LAN9355_SWITCH_CSR_CMD_BUSY | LAN9355_SWITCH_CSR_CMD_WRITE |
      LAN9355_SWITCH_CSR_CMD_BE;

   //Set register address
   value |= address & LAN9355_SWITCH_CSR_CMD_ADDR;

   //The write cycle is initiated by performing a single write to the
   //SWITCH_CSR_CMD register
   lan9355WriteSysReg(interface, LAN9355_SWITCH_CSR_CMD, value);
   ...
   do
   {
      ...
      value = lan9355ReadSysReg(interface, LAN9355_SWITCH_CSR_CMD);

      //Poll CSR_BUSY bit
   } while((value & LAN9355_SWITCH_CSR_CMD_BUSY) != 0);
}
...
uint32_t lan9355ReadSwitchReg(NetInterface *interface, uint16_t address)
{
   ...
   value = LAN9355_SWITCH_CSR_CMD_BUSY | LAN9355_SWITCH_CSR_CMD_READ |
      LAN9355_SWITCH_CSR_CMD_BE;
   ...
   return lan9355ReadSysReg(interface, LAN9355_SWITCH_CSR_DATA);
}
