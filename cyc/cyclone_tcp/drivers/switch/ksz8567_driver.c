...
#include "drivers/switch/ksz8567_driver.h"
#include "debug.h"


/**
 * @brief KSZ8567 Ethernet switch driver
 **/
...
const SwitchDriver ksz8567SwitchDriver =
{
   ...
   ksz8567GetLinkState,
   ksz8567GetLinkSpeed,
   ksz8567GetDuplexMode,
   ksz8567SetPortState,
   ksz8567GetPortState,
   ksz8567SetAgingTime,
   ksz8567EnableIgmpSnooping,
   ksz8567EnableMldSnooping,
   ksz8567EnableRsvdMcastTable,
   ksz8567AddStaticFdbEntry,
   ksz8567DeleteStaticFdbEntry,
   ksz8567GetStaticFdbEntry,
   ksz8567FlushStaticFdbTable,
   ksz8567GetDynamicFdbEntry,
   ksz8567FlushDynamicFdbTable,
   ksz8567SetUnknownMcastFwdPorts
};


/**
 * @brief Tail tag rules (host to KSZ8567)
 **/

const uint16_t ksz8567IngressTailTag[6] =
{
   HTONS(KSZ8567_TAIL_TAG_NORMAL_ADDR_LOOKUP),
   HTONS(KSZ8567_TAIL_TAG_PORT_BLOCKING_OVERRIDE | KSZ8567_TAIL_TAG_DEST_PORT1),
   HTONS(KSZ8567_TAIL_TAG_PORT_BLOCKING_OVERRIDE | KSZ8567_TAIL_TAG_DEST_PORT2),
   HTONS(KSZ8567_TAIL_TAG_PORT_BLOCKING_OVERRIDE | KSZ8567_TAIL_TAG_DEST_PORT3),
   HTONS(KSZ8567_TAIL_TAG_PORT_BLOCKING_OVERRIDE | KSZ8567_TAIL_TAG_DEST_PORT4),
   HTONS(KSZ8567_TAIL_TAG_PORT_BLOCKING_OVERRIDE | KSZ8567_TAIL_TAG_DEST_PORT5)
};


/**
 * @brief KSZ8567 Ethernet switch initialization
...
error_t ksz8567Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing KSZ8567...\r\n");
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      do
      {
         ...
         temp = ksz8567ReadSwitchReg8(interface, KSZ8567_CHIP_ID1);

         //The returned data is invalid until the serial interface is ready
      } while(temp != KSZ8567_CHIP_ID1_DEFAULT);

#if (ETH_PORT_TAGGING_SUPPORT == ENABLED)
      //Enable tail tag feature
      temp = ksz8567ReadSwitchReg8(interface, KSZ8567_PORT6_OP_CTRL0);
      ...
      for(port = KSZ8567_PORT1; port <= KSZ8567_PORT5; port++)
      {
         ...
         if(interface->port != 0)
         {
            ...
            ksz8567SetPortState(interface, port, SWITCH_PORT_STATE_LISTENING);
         }
         else
#endif
         {
            //Enable transmission, reception and address learning
            ksz8567SetPortState(interface, port, SWITCH_PORT_STATE_FORWARDING);
         }
      }

      //Restore default age count
      ksz8567WriteSwitchReg8(interface, KSZ8567_SWITCH_LUE_CTRL0,
         KSZ8567_SWITCH_LUE_CTRL0_AGE_COUNT_DEFAULT |
         KSZ8567_SWITCH_LUE_CTRL0_HASH_OPTION_CRC);

      //Restore default age period
      ksz8567WriteSwitchReg8(interface, KSZ8567_SWITCH_LUE_CTRL3,
         KSZ8567_SWITCH_LUE_CTRL3_AGE_PERIOD_DEFAULT);

      //Add internal delay to ingress and egress RGMII clocks
      temp = ksz8567ReadSwitchReg8(interface, KSZ8567_PORT6_XMII_CTRL1);
      temp |= KSZ8567_PORTn_XMII_CTRL1_RGMII_ID_IG;
      temp |= KSZ8567_PORTn_XMII_CTRL1_RGMII_ID_EG;
      ksz8567WriteSwitchReg8(interface, KSZ8567_PORT6_XMII_CTRL1, temp);

      //Start switch operation
      ksz8567WriteSwitchReg8(interface, KSZ8567_SWITCH_OP,
         KSZ8567_SWITCH_OP_START_SWITCH);
   }
   ...
   for(port = KSZ8567_PORT1; port <= KSZ8567_PORT5; port++)
   {
      ...
      ksz8567WriteMmdReg(interface, port, KSZ8567_MMD_LED_MODE,
         KSZ8567_MMD_LED_MODE_LED_MODE_SINGLE |
         KSZ8567_MMD_LED_MODE_RESERVED_DEFAULT);

      //Implement workaround for single-LED mode
      ksz8567WritePhyReg(interface, port, 0x1E, 0xFA00);

      //Debug message
      TRACE_DEBUG("Port %u:\r\n", port);
      //Dump PHY registers for debugging purpose
      ksz8567DumpPhyReg(interface, port);
   }

   //Perform custom configuration
   ksz8567InitHook(interface);
...
/**
 * @brief KSZ8567 custom configuration
 * @param[in] interface Underlying network interface
 **/
...
void ksz8567Tick(NetInterface *interface)
{
   ...
   if(interface->port != 0)
   {
      ...
      for(i = 0; i < NET_INTERFACE_COUNT; i++)
      {
            ...
            virtualInterface->parent == interface)
         {
            ...
            linkState = ksz8567GetLinkState(interface, virtualInterface->port);
...
#endif
   {
      ...
      {
         //Retrieve current link state
         if(ksz8567GetLinkState(interface, port))
         {
            linkState = TRUE;
         }
      }
...
void ksz8567EventHandler(NetInterface *interface)
{
   ...
   if(interface->port != 0)
   {
      ...
      for(i = 0; i < NET_INTERFACE_COUNT; i++)
      {
            ...
            virtualInterface->parent == interface)
         {
            ...
            //Valid port?
            if(port >= KSZ8567_PORT1 && port <= KSZ8567_PORT5)
            {
               //Retrieve current link state
               linkState = ksz8567GetLinkState(interface, port);

               //Link up event?
               if(linkState && !virtualInterface->linkState)
               {
                  //Retrieve host interface speed
                  interface->linkSpeed = ksz8567GetLinkSpeed(interface,
                     KSZ8567_PORT6);

                  //Retrieve host interface duplex mode
                  interface->duplexMode = ksz8567GetDuplexMode(interface,
                     KSZ8567_PORT6);
                  ...
                  //Check current speed
                  virtualInterface->linkSpeed = ksz8567GetLinkSpeed(interface,
                     port);

                  //Check current duplex mode
                  virtualInterface->duplexMode = ksz8567GetDuplexMode(interface,
                     port);
...
#endif
   {
      ...
      {
         //Retrieve host interface speed
         interface->linkSpeed = ksz8567GetLinkSpeed(interface, KSZ8567_PORT6);
         //Retrieve host interface duplex mode
         interface->duplexMode = ksz8567GetDuplexMode(interface, KSZ8567_PORT6);
   ...
   size_t *length, NetRxAncillary *ancillary)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      if(*length >= (sizeof(EthHeader) + sizeof(uint8_t)))
      {
         ...
         ancillary->port = (*tailTag & KSZ8567_TAIL_TAG_SRC_PORT) + 1;
...
bool_t ksz8567GetLinkState(NetInterface *interface, uint8_t port)
{
   ...
   if(port >= KSZ8567_PORT1 && port <= KSZ8567_PORT5)
   {
      ...
      value = ksz8567ReadPhyReg(interface, port, KSZ8567_BMSR);
      value = ksz8567ReadPhyReg(interface, port, KSZ8567_BMSR);

      //Retrieve current link state
      linkState = (value & KSZ8567_BMSR_LINK_STATUS) ? TRUE : FALSE;
   }
   else
   {
...
uint32_t ksz8567GetLinkSpeed(NetInterface *interface, uint8_t port)
{
   ...
   if(port >= KSZ8567_PORT1 && port <= KSZ8567_PORT5)
   {
      ...
      value = ksz8567ReadPhyReg(interface, port, KSZ8567_PHYCON);

      //Retrieve current link speed
      if((value & KSZ8567_PHYCON_SPEED_100BTX) != 0)
      {
         //100BASE-TX
         linkSpeed = NIC_LINK_SPEED_100MBPS;
      }
      else if((value & KSZ8567_PHYCON_SPEED_10BT) != 0)
      {
         //10BASE-T
   ...
   else if(port == KSZ8567_PORT6)
   {
      //SPI slave mode?
      if(interface->spiDriver != NULL)
      {
         //Read port 6 XMII control 1 register
         value = ksz8567ReadSwitchReg8(interface, KSZ8567_PORT6_XMII_CTRL1);

         //Retrieve host interface type
         type = value & KSZ8567_PORTn_XMII_CTRL1_IF_TYPE;

         //Gigabit interface?
         if(type == KSZ8567_PORTn_XMII_CTRL1_IF_TYPE_RGMII &&
            (value & KSZ8567_PORTn_XMII_CTRL1_SPEED_1000) == 0)
         {
            //1000 Mb/s mode
         ...
         else
         {
            ...
            value = ksz8567ReadSwitchReg8(interface, KSZ8567_PORT6_XMII_CTRL0);

            //Retrieve host interface speed
            if((value & KSZ8567_PORTn_XMII_CTRL0_SPEED_10_100) != 0)
            {
               //100 Mb/s mode
               linkSpeed = NIC_LINK_SPEED_100MBPS;
            }
...
NicDuplexMode ksz8567GetDuplexMode(NetInterface *interface, uint8_t port)
{
   ...
   if(port >= KSZ8567_PORT1 && port <= KSZ8567_PORT5)
   {
      ...
      if((value & KSZ8567_PHYCON_DUPLEX_STATUS) != 0)
      {
   ...
   else if(port == KSZ8567_PORT6)
   {
      ...
      if(interface->spiDriver != NULL)
      {
         ...
         //Retrieve host interface duplex mode
         if((value & KSZ8567_PORTn_XMII_CTRL0_DUPLEX) != 0)
         {
            duplexMode = NIC_FULL_DUPLEX_MODE;
         }
         else
   ...
   SwitchPortState state)
{
   ...
   if(port >= KSZ8567_PORT1 && port <= KSZ8567_PORT5)
   {
      ...
      temp = ksz8567ReadSwitchReg8(interface, KSZ8567_PORTn_MSTP_STATE(port));
      ...
      case SWITCH_PORT_STATE_LISTENING:
         temp &= ~KSZ8567_PORTn_MSTP_STATE_TRANSMIT_EN;
         temp |= KSZ8567_PORTn_MSTP_STATE_RECEIVE_EN;
         temp |= KSZ8567_PORTn_MSTP_STATE_LEARNING_DIS;
         break;
      ...
      ksz8567WriteSwitchReg8(interface, KSZ8567_PORTn_MSTP_STATE(port), temp);
   }
}
...
SwitchPortState ksz8567GetPortState(NetInterface *interface, uint8_t port)
{
   ...
   if(port >= KSZ8567_PORT1 && port <= KSZ8567_PORT5)
   {
      ...
      //Check port state
      if((temp & KSZ8567_PORTn_MSTP_STATE_TRANSMIT_EN) == 0 &&
         (temp & KSZ8567_PORTn_MSTP_STATE_RECEIVE_EN) == 0 &&
         (temp & KSZ8567_PORTn_MSTP_STATE_LEARNING_DIS) != 0)
      {
         //Disabled state
         state = SWITCH_PORT_STATE_DISABLED;
      }
      else if((temp & KSZ8567_PORTn_MSTP_STATE_TRANSMIT_EN) == 0 &&
         (temp & KSZ8567_PORTn_MSTP_STATE_RECEIVE_EN) != 0 &&
         (temp & KSZ8567_PORTn_MSTP_STATE_LEARNING_DIS) != 0)
      {
         //Listening state
...
void ksz8567EnableIgmpSnooping(NetInterface *interface, bool_t enable)
{
   ...
   temp = ksz8567ReadSwitchReg8(interface,
      KSZ8567_GLOBAL_PORT_MIRROR_SNOOP_CTRL);

   //Enable or disable IGMP snooping
   if(enable)
   {
      temp |= KSZ8567_GLOBAL_PORT_MIRROR_SNOOP_CTRL_IGMP_SNOOP_EN;
   }
   else
   {
      temp &= ~KSZ8567_GLOBAL_PORT_MIRROR_SNOOP_CTRL_IGMP_SNOOP_EN;
   ...
   ksz8567WriteSwitchReg8(interface, KSZ8567_GLOBAL_PORT_MIRROR_SNOOP_CTRL,
      temp);
}
...
void ksz8567EnableMldSnooping(NetInterface *interface, bool_t enable)
{
   ...
   if(enable)
   {
      temp |= KSZ8567_GLOBAL_PORT_MIRROR_SNOOP_CTRL_MLD_SNOOP_EN;
   }
   else
   {
      temp &= ~KSZ8567_GLOBAL_PORT_MIRROR_SNOOP_CTRL_MLD_SNOOP_EN;
...
void ksz8567EnableRsvdMcastTable(NetInterface *interface, bool_t enable)
{
   ...
   temp = ksz8567ReadSwitchReg8(interface, KSZ8567_SWITCH_LUE_CTRL0);

   //Enable or disable the reserved multicast table
   if(enable)
   {
      temp |= KSZ8567_SWITCH_LUE_CTRL0_RESERVED_MCAST_LOOKUP_EN;
   }
   else
   {
      temp &= ~KSZ8567_SWITCH_LUE_CTRL0_RESERVED_MCAST_LOOKUP_EN;
   ...
   ksz8567WriteSwitchReg8(interface, KSZ8567_SWITCH_LUE_CTRL0, temp);
}
   ...
   const SwitchFdbEntry *entry)
{
   ...
   j = KSZ8567_STATIC_MAC_TABLE_SIZE;

   //Loop through the static MAC table
   for(i = 0; i < KSZ8567_STATIC_MAC_TABLE_SIZE; i++)
   {
      //Read current entry
      error = ksz8567GetStaticFdbEntry(interface, i, &currentEntry);
      ...
      {
         //Keep track of the first free entry
         if(j == KSZ8567_STATIC_MAC_TABLE_SIZE)
         {
            j = i;
         }
      }
   ...
   if(j < KSZ8567_STATIC_MAC_TABLE_SIZE)
   {
      ...
      ksz8567WriteSwitchReg32(interface, KSZ8567_STATIC_TABLE_ENTRY1,
         KSZ8567_STATIC_TABLE_ENTRY1_VALID);

      //Set the relevant forward ports
      if(entry->destPorts == SWITCH_CPU_PORT_MASK)
      {
         value = KSZ8567_PORT6_MASK;
      }
      else
      {
         value = entry->destPorts & KSZ8567_PORT_MASK;
      }

      //Enable overriding of port state
      if(entry->override)
      {
         value |= KSZ8567_STATIC_TABLE_ENTRY2_OVERRIDE;
      }

      //Write the Static Address Table Entry 2 register
      ksz8567WriteSwitchReg32(interface, KSZ8567_STATIC_TABLE_ENTRY2, value);

      //Copy MAC address (first 16 bits)
      value = (entry->macAddr.b[0] << 8) | entry->macAddr.b[1];

      //Write the Static Address Table Entry 3 register
      ksz8567WriteSwitchReg32(interface, KSZ8567_STATIC_TABLE_ENTRY3, value);
      ...
      ksz8567WriteSwitchReg32(interface, KSZ8567_STATIC_TABLE_ENTRY4, value);

      //Write the TABLE_INDEX field with the 4-bit index value
      value = (j << 16) & KSZ8567_STATIC_MCAST_TABLE_CTRL_TABLE_INDEX;
      //Set the TABLE_SELECT bit to 0 to select the static address table
      value &= ~KSZ8567_STATIC_MCAST_TABLE_CTRL_TABLE_SELECT;
      //Set the ACTION bit to 0 to indicate a write operation
      value &= ~KSZ8567_STATIC_MCAST_TABLE_CTRL_ACTION;
      //Set the START_FINISH bit to 1 to initiate the operation
      value |= KSZ8567_STATIC_MCAST_TABLE_CTRL_START_FINISH;

      //Start the write operation
      ksz8567WriteSwitchReg32(interface, KSZ8567_STATIC_MCAST_TABLE_CTRL,
         value);
      ...
      do
      {
         ...
         value = ksz8567ReadSwitchReg32(interface,
            KSZ8567_STATIC_MCAST_TABLE_CTRL);

         //Poll the START_FINISH bit
      } while((value & KSZ8567_STATIC_MCAST_TABLE_CTRL_START_FINISH) != 0);
   ...
   const SwitchFdbEntry *entry)
{
   ...
   //Loop through the static MAC table
   for(j = 0; j < KSZ8567_STATIC_MAC_TABLE_SIZE; j++)
   {
      //Read current entry
      error = ksz8567GetStaticFdbEntry(interface, j, &currentEntry);
   ...
   if(j < KSZ8567_STATIC_MAC_TABLE_SIZE)
   {
      ...
      ksz8567WriteSwitchReg32(interface, KSZ8567_STATIC_TABLE_ENTRY1, 0);
      ksz8567WriteSwitchReg32(interface, KSZ8567_STATIC_TABLE_ENTRY2, 0);
      ksz8567WriteSwitchReg32(interface, KSZ8567_STATIC_TABLE_ENTRY3, 0);
      ksz8567WriteSwitchReg32(interface, KSZ8567_STATIC_TABLE_ENTRY4, 0);
   ...
   SwitchFdbEntry *entry)
{
   ...
   if(index < KSZ8567_STATIC_MAC_TABLE_SIZE)
   {
      //Write the TABLE_INDEX field with the 4-bit index value
      value = (index << 16) & KSZ8567_STATIC_MCAST_TABLE_CTRL_TABLE_INDEX;
      //Set the TABLE_SELECT bit to 0 to select the static address table
      value &= ~KSZ8567_STATIC_MCAST_TABLE_CTRL_TABLE_SELECT;
      ...
      value = ksz8567ReadSwitchReg32(interface, KSZ8567_STATIC_TABLE_ENTRY1);

      //Valid entry?
      if((value & KSZ8567_STATIC_TABLE_ENTRY1_VALID) != 0)
      {
         //Read the Static Address Table Entry 2 register
         value = ksz8567ReadSwitchReg32(interface, KSZ8567_STATIC_TABLE_ENTRY2);

         //Retrieve the ports associated with this MAC address
         entry->srcPort = 0;
         entry->destPorts = value & KSZ8567_STATIC_TABLE_ENTRY2_PORT_FORWARD;

         //Check the value of the OVERRIDE bit
         if((value & KSZ8567_STATIC_TABLE_ENTRY2_OVERRIDE) != 0)
         {
            entry->override = TRUE;
         }
         else
         ...
         value = ksz8567ReadSwitchReg32(interface, KSZ8567_STATIC_TABLE_ENTRY3);
         ...
         value = ksz8567ReadSwitchReg32(interface, KSZ8567_STATIC_TABLE_ENTRY4);
...
void ksz8567FlushStaticFdbTable(NetInterface *interface)
{
   ...
   for(i = 0; i < KSZ8567_STATIC_MAC_TABLE_SIZE; i++)
   {
      ...
      //Write the TABLE_INDEX field with the 4-bit index value
      value = (i << 16) & KSZ8567_STATIC_MCAST_TABLE_CTRL_TABLE_INDEX;
      //Set the TABLE_SELECT bit to 0 to select the static address table
      value &= ~KSZ8567_STATIC_MCAST_TABLE_CTRL_TABLE_SELECT;
      //Set the ACTION bit to 0 to indicate a write operation
      value &= ~KSZ8567_STATIC_MCAST_TABLE_CTRL_ACTION;
   ...
   SwitchFdbEntry *entry)
{
   ...
   if(index == 0)
   {
      ...
      ksz8567WriteSwitchReg32(interface, KSZ8567_ALU_TABLE_CTRL, 0);

      //Start the search operation
      ksz8567WriteSwitchReg32(interface, KSZ8567_ALU_TABLE_CTRL,
         KSZ8567_ALU_TABLE_CTRL_START_FINISH |
         KSZ8567_ALU_TABLE_CTRL_ACTION_SEARCH);
   }
   ...
   do
   {
      ...
      value = ksz8567ReadSwitchReg32(interface, KSZ8567_ALU_TABLE_CTRL);

      //This bit goes high to indicate either a new valid entry is returned or
      //the search is complete
   } while((value & KSZ8567_ALU_TABLE_CTRL_VALID_ENTRY_OR_SEARCH_END) == 0);

   //Check whether the next valid entry is ready
   if((value & KSZ8567_ALU_TABLE_CTRL_VALID) != 0)
   {
      ...
      value = ksz8567ReadSwitchReg32(interface, KSZ8567_ALU_TABLE_ENTRY1);
      value = ksz8567ReadSwitchReg32(interface, KSZ8567_ALU_TABLE_ENTRY2);

      //Retrieve the port associated with this MAC address
      switch(value & KSZ8567_ALU_TABLE_ENTRY2_PORT_FORWARD)
      {
      case KSZ8567_ALU_TABLE_ENTRY2_PORT1_FORWARD:
         entry->srcPort = KSZ8567_PORT1;
         break;
      case KSZ8567_ALU_TABLE_ENTRY2_PORT2_FORWARD:
         entry->srcPort = KSZ8567_PORT2;
         break;
      case KSZ8567_ALU_TABLE_ENTRY2_PORT3_FORWARD:
         entry->srcPort = KSZ8567_PORT3;
         break;
      case KSZ8567_ALU_TABLE_ENTRY2_PORT4_FORWARD:
         entry->srcPort = KSZ8567_PORT4;
         break;
      case KSZ8567_ALU_TABLE_ENTRY2_PORT5_FORWARD:
         entry->srcPort = KSZ8567_PORT5;
         break;
      case KSZ8567_ALU_TABLE_ENTRY2_PORT6_FORWARD:
         entry->srcPort = KSZ8567_PORT6;
         break;
      case KSZ8567_ALU_TABLE_ENTRY2_PORT7_FORWARD:
         entry->srcPort = KSZ8567_PORT7;
         break;
      default:
         entry->srcPort = 0;
         break;
      ...
      value = ksz8567ReadSwitchReg32(interface, KSZ8567_ALU_TABLE_ENTRY3);
      ...
      value = ksz8567ReadSwitchReg32(interface, KSZ8567_ALU_TABLE_ENTRY4);
...
void ksz8567FlushDynamicFdbTable(NetInterface *interface, uint8_t port)
{
   ...
   temp = ksz8567ReadSwitchReg8(interface, KSZ8567_SWITCH_LUE_CTRL2);
   temp &= ~KSZ8567_SWITCH_LUE_CTRL2_FLUSH_OPTION;
   temp |= KSZ8567_SWITCH_LUE_CTRL2_FLUSH_OPTION_DYNAMIC;
   ksz8567WriteSwitchReg8(interface, KSZ8567_SWITCH_LUE_CTRL2, temp);

   //Valid port number?
   if(port >= KSZ8567_PORT1 && port <= KSZ8567_PORT7)
   {
      //Save the current state of the port
      state = ksz8567ReadSwitchReg8(interface, KSZ8567_PORTn_MSTP_STATE(port));

      //Turn off learning capability
      ksz8567WriteSwitchReg8(interface, KSZ8567_PORTn_MSTP_STATE(port),
         state | KSZ8567_PORTn_MSTP_STATE_LEARNING_DIS);

      //All the entries associated with a port that has its learning capability
      //being turned off will be flushed
      temp = ksz8567ReadSwitchReg8(interface, KSZ8567_SWITCH_LUE_CTRL1);
      temp |= KSZ8567_SWITCH_LUE_CTRL1_FLUSH_MSTP_ENTRIES;
      ksz8567WriteSwitchReg8(interface, KSZ8567_SWITCH_LUE_CTRL1, temp);

      //Restore the original state of the port
      ksz8567WriteSwitchReg8(interface, KSZ8567_PORTn_MSTP_STATE(port), state);
   }
   else
   {
      //Trigger a flush of the entire address lookup table
      temp = ksz8567ReadSwitchReg8(interface, KSZ8567_SWITCH_LUE_CTRL1);
      temp |= KSZ8567_SWITCH_LUE_CTRL1_FLUSH_ALU_TABLE;
      ksz8567WriteSwitchReg8(interface, KSZ8567_SWITCH_LUE_CTRL1, temp);
   }
}
   ...
   bool_t enable, uint32_t forwardPorts)
{
   ...
   temp = ksz8567ReadSwitchReg32(interface, KSZ8567_UNKONWN_MULTICAST_CTRL);

   //Clear port map
   temp &= ~KSZ8567_UNKONWN_MULTICAST_CTRL_FWD_MAP;
   ...
   if(enable)
   {
      ...
      {
         temp |= KSZ8567_UNKONWN_MULTICAST_CTRL_FWD_MAP_PORT6;
      }

      //Select the desired forward ports
      temp |= forwardPorts & KSZ8567_UNKONWN_MULTICAST_CTRL_FWD_MAP_ALL;
   }
   else
   {
      //Disable forwarding
   ...
   ksz8567WriteSwitchReg32(interface, KSZ8567_UNKONWN_MULTICAST_CTRL, temp);
}
   ...
   uint8_t address, uint16_t data)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      n = KSZ8567_PORTn_ETH_PHY_REG(port, address);
      //Write the 16-bit value
      ksz8567WriteSwitchReg16(interface, n, data);
   }
   else if(interface->smiDriver != NULL)
   {
   ...
   uint8_t address)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      data = ksz8567ReadSwitchReg16(interface, n);
   }
   else if(interface->smiDriver != NULL)
   {
...
void ksz8567DumpPhyReg(NetInterface *interface, uint8_t port)
{
   ...
   for(i = 0; i < 32; i++)
   {
      ...
      TRACE_DEBUG("%02" PRIu8 ": 0x%04" PRIX16 "\r\n", i,
         ksz8567ReadPhyReg(interface, port, i));
   }
   ...
   uint8_t devAddr, uint16_t regAddr, uint16_t data)
{
   ...
   ksz8567WritePhyReg(interface, port, KSZ8567_MMDACR,
      KSZ8567_MMDACR_FUNC_ADDR | (devAddr & KSZ8567_MMDACR_DEVAD));

   //Write MMD register address
   ksz8567WritePhyReg(interface, port, KSZ8567_MMDAADR, regAddr);

   //Select data operation
   ksz8567WritePhyReg(interface, port, KSZ8567_MMDACR,
      KSZ8567_MMDACR_FUNC_DATA_NO_POST_INC | (devAddr & KSZ8567_MMDACR_DEVAD));

   //Write the content of the MMD register
   ksz8567WritePhyReg(interface, port, KSZ8567_MMDAADR, data);
}
   ...
   uint8_t devAddr, uint16_t regAddr)
{
   ...
   return ksz8567ReadPhyReg(interface, port, KSZ8567_MMDAADR);
}
   ...
   uint8_t data)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      command = KSZ8567_SPI_CMD_WRITE;
      //Set register address
      command |= (address << 5) & KSZ8567_SPI_CMD_ADDR;
