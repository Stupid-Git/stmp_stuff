...
/**
 * @brief KSZ9893 Ethernet switch driver
 **/
...
const SwitchDriver ksz9893SwitchDriver =
{
   ...
   ksz9893GetLinkState,
   ksz9893GetLinkSpeed,
   ksz9893GetDuplexMode,
   ksz9893SetPortState,
   ksz9893GetPortState,
   ksz9893SetAgingTime,
   ksz9893EnableIgmpSnooping,
   ksz9893EnableMldSnooping,
   ksz9893EnableRsvdMcastTable,
   ksz9893AddStaticFdbEntry,
   ksz9893DeleteStaticFdbEntry,
   ksz9893GetStaticFdbEntry,
   ksz9893FlushStaticFdbTable,
   ksz9893GetDynamicFdbEntry,
   ksz9893FlushDynamicFdbTable,
   ksz9893SetUnknownMcastFwdPorts
};


/**
 * @brief Tail tag rules (host to KSZ9893)
 **/

const uint8_t ksz9893IngressTailTag[3] =
{
   KSZ9893_TAIL_TAG_NORMAL_ADDR_LOOKUP,
   KSZ9893_TAIL_TAG_PORT_BLOCKING_OVERRIDE | KSZ9893_TAIL_TAG_DEST_PORT1,
   KSZ9893_TAIL_TAG_PORT_BLOCKING_OVERRIDE | KSZ9893_TAIL_TAG_DEST_PORT2
};


/**
 * @brief KSZ9893 Ethernet switch initialization
...
error_t ksz9893Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing KSZ9893...\r\n");
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      do
      {
         ...
         temp = ksz9893ReadSwitchReg8(interface, KSZ9893_CHIP_ID1);

         //The returned data is invalid until the serial interface is ready
      } while(temp != KSZ9893_CHIP_ID1_DEFAULT);

      //Reset switch
      ksz9893WriteSwitchReg8(interface, KSZ9893_SWITCH_OP,
         KSZ9893_SWITCH_OP_SOFT_HARD_RESET);

      //Wait for the reset to complete
      do
      {
         //Read switch operation register
         temp = ksz9893ReadSwitchReg8(interface, KSZ9893_SWITCH_OP);

         //The reset bit is self-clearing
      } while((temp & KSZ9893_SWITCH_OP_SOFT_HARD_RESET) != 0);

#if (ETH_PORT_TAGGING_SUPPORT == ENABLED)
      //Enable tail tag feature
      temp = ksz9893ReadSwitchReg8(interface, KSZ9893_PORT3_OP_CTRL0);
      ...
      for(port = KSZ9893_PORT1; port <= KSZ9893_PORT2; port++)
      {
         ...
         if(interface->port != 0)
         {
            ...
            ksz9893SetPortState(interface, port, SWITCH_PORT_STATE_LISTENING);
         }
         else
#endif
         {
            //Enable transmission, reception and address learning
            ksz9893SetPortState(interface, port, SWITCH_PORT_STATE_FORWARDING);
         }
      }

      //Restore default age count
      ksz9893WriteSwitchReg8(interface, KSZ9893_SWITCH_LUE_CTRL0,
         KSZ9893_SWITCH_LUE_CTRL0_AGE_COUNT_DEFAULT |
         KSZ9893_SWITCH_LUE_CTRL0_HASH_OPTION_CRC);

      //Restore default age period
      ksz9893WriteSwitchReg8(interface, KSZ9893_SWITCH_LUE_CTRL3,
         KSZ9893_SWITCH_LUE_CTRL3_AGE_PERIOD_DEFAULT);

      //Add internal delay to ingress and egress RGMII clocks
      temp = ksz9893ReadSwitchReg8(interface, KSZ9893_PORT3_XMII_CTRL1);
      temp |= KSZ9893_PORTn_XMII_CTRL1_RGMII_ID_IG;
      temp |= KSZ9893_PORTn_XMII_CTRL1_RGMII_ID_EG;
      ksz9893WriteSwitchReg8(interface, KSZ9893_PORT3_XMII_CTRL1, temp);

      //Start switch operation
      ksz9893WriteSwitchReg8(interface, KSZ9893_SWITCH_OP,
         KSZ9893_SWITCH_OP_START_SWITCH);
   }
   ...
   for(port = KSZ9893_PORT1; port <= KSZ9893_PORT2; port++)
   {
      ...
      ksz9893DumpPhyReg(interface, port);
   }

   //Perform custom configuration
   ksz9893InitHook(interface);
...
/**
 * @brief KSZ9893 custom configuration
 * @param[in] interface Underlying network interface
 **/
...
void ksz9893Tick(NetInterface *interface)
{
   ...
   if(interface->port != 0)
   {
      ...
      for(i = 0; i < NET_INTERFACE_COUNT; i++)
      {
            ...
            virtualInterface->parent == interface)
         {
            ...
            linkState = ksz9893GetLinkState(interface, virtualInterface->port);
...
#endif
   {
      ...
      {
         //Retrieve current link state
         if(ksz9893GetLinkState(interface, port))
         {
            linkState = TRUE;
         }
      }
...
void ksz9893EventHandler(NetInterface *interface)
{
   ...
   if(interface->port != 0)
   {
      ...
      for(i = 0; i < NET_INTERFACE_COUNT; i++)
      {
            ...
            virtualInterface->parent == interface)
         {
            ...
            //Valid port?
            if(port >= KSZ9893_PORT1 && port <= KSZ9893_PORT2)
            {
               //Retrieve current link state
               linkState = ksz9893GetLinkState(interface, port);

               //Link up event?
               if(linkState && !virtualInterface->linkState)
               {
                  //Retrieve host interface speed
                  interface->linkSpeed = ksz9893GetLinkSpeed(interface,
                     KSZ9893_PORT3);

                  //Retrieve host interface duplex mode
                  interface->duplexMode = ksz9893GetDuplexMode(interface,
                     KSZ9893_PORT3);
                  ...
                  //Check current speed
                  virtualInterface->linkSpeed = ksz9893GetLinkSpeed(interface,
                     port);

                  //Check current duplex mode
                  virtualInterface->duplexMode = ksz9893GetDuplexMode(interface,
                     port);
...
#endif
   {
      ...
      {
         //Retrieve host interface speed
         interface->linkSpeed = ksz9893GetLinkSpeed(interface, KSZ9893_PORT3);
         //Retrieve host interface duplex mode
         interface->duplexMode = ksz9893GetDuplexMode(interface, KSZ9893_PORT3);
   ...
   size_t *length, NetRxAncillary *ancillary)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      if(*length >= (sizeof(EthHeader) + sizeof(uint8_t)))
      {
         ...
         ancillary->port = (*tailTag & KSZ9893_TAIL_TAG_SRC_PORT) + 1;
...
bool_t ksz9893GetLinkState(NetInterface *interface, uint8_t port)
{
   ...
   if(port >= KSZ9893_PORT1 && port <= KSZ9893_PORT2)
   {
      ...
      value = ksz9893ReadPhyReg(interface, port, KSZ9893_BMSR);
      value = ksz9893ReadPhyReg(interface, port, KSZ9893_BMSR);

      //Retrieve current link state
      linkState = (value & KSZ9893_BMSR_LINK_STATUS) ? TRUE : FALSE;
   }
   else
   {
...
uint32_t ksz9893GetLinkSpeed(NetInterface *interface, uint8_t port)
{
   ...
   if(port >= KSZ9893_PORT1 && port <= KSZ9893_PORT2)
   {
      ...
      value = ksz9893ReadPhyReg(interface, port, KSZ9893_PHYCON);

      //Retrieve current link speed
      if((value & KSZ9893_PHYCON_SPEED_1000BT) != 0)
      {
         //1000BASE-T
         linkSpeed = NIC_LINK_SPEED_1GBPS;
      }
      else if((value & KSZ9893_PHYCON_SPEED_100BTX) != 0)
      {
         //100BASE-TX
         linkSpeed = NIC_LINK_SPEED_100MBPS;
      }
      else if((value & KSZ9893_PHYCON_SPEED_10BT) != 0)
      {
         //10BASE-T
   ...
   else if(port == KSZ9893_PORT3)
   {
      //SPI slave mode?
      if(interface->spiDriver != NULL)
      {
         //Read port 3 XMII control 1 register
         value = ksz9893ReadSwitchReg8(interface, KSZ9893_PORT3_XMII_CTRL1);

         //Retrieve host interface type
         type = value & KSZ9893_PORTn_XMII_CTRL1_IF_TYPE;

         //Gigabit interface?
         if(type == KSZ9893_PORTn_XMII_CTRL1_IF_TYPE_RGMII &&
            (value & KSZ9893_PORTn_XMII_CTRL1_SPEED_1000) != 0)
         {
            //1000 Mb/s mode
            linkSpeed = NIC_LINK_SPEED_1GBPS;
         }
         else
         {
            ...
            value = ksz9893ReadSwitchReg8(interface, KSZ9893_PORT3_XMII_CTRL0);

            //Retrieve host interface speed
            if((value & KSZ9893_PORTn_XMII_CTRL0_SPEED_10_100) != 0)
            {
               //100 Mb/s mode
               linkSpeed = NIC_LINK_SPEED_100MBPS;
            }
...
NicDuplexMode ksz9893GetDuplexMode(NetInterface *interface, uint8_t port)
{
   ...
   if(port >= KSZ9893_PORT1 && port <= KSZ9893_PORT2)
   {
      ...
      if((value & KSZ9893_PHYCON_DUPLEX_STATUS) != 0)
      {
   ...
   else if(port == KSZ9893_PORT3)
   {
      ...
      if(interface->spiDriver != NULL)
      {
         ...
         //Retrieve host interface duplex mode
         if((value & KSZ9893_PORTn_XMII_CTRL0_DUPLEX) != 0)
         {
            duplexMode = NIC_FULL_DUPLEX_MODE;
         }
         else
   ...
   SwitchPortState state)
{
   ...
   if(port >= KSZ9893_PORT1 && port <= KSZ9893_PORT2)
   {
      ...
      temp = ksz9893ReadSwitchReg8(interface, KSZ9893_PORTn_MSTP_STATE(port));
      ...
      case SWITCH_PORT_STATE_LISTENING:
         temp &= ~KSZ9893_PORTn_MSTP_STATE_TRANSMIT_EN;
         temp |= KSZ9893_PORTn_MSTP_STATE_RECEIVE_EN;
         temp |= KSZ9893_PORTn_MSTP_STATE_LEARNING_DIS;
         break;
      ...
      ksz9893WriteSwitchReg8(interface, KSZ9893_PORTn_MSTP_STATE(port), temp);
   }
}
...
SwitchPortState ksz9893GetPortState(NetInterface *interface, uint8_t port)
{
   ...
   if(port >= KSZ9893_PORT1 && port <= KSZ9893_PORT2)
   {
      ...
      //Check port state
      if((temp & KSZ9893_PORTn_MSTP_STATE_TRANSMIT_EN) == 0 &&
         (temp & KSZ9893_PORTn_MSTP_STATE_RECEIVE_EN) == 0 &&
         (temp & KSZ9893_PORTn_MSTP_STATE_LEARNING_DIS) != 0)
      {
         //Disabled state
         state = SWITCH_PORT_STATE_DISABLED;
      }
      else if((temp & KSZ9893_PORTn_MSTP_STATE_TRANSMIT_EN) == 0 &&
         (temp & KSZ9893_PORTn_MSTP_STATE_RECEIVE_EN) != 0 &&
         (temp & KSZ9893_PORTn_MSTP_STATE_LEARNING_DIS) != 0)
      {
         //Listening state
...
void ksz9893EnableIgmpSnooping(NetInterface *interface, bool_t enable)
{
   ...
   temp = ksz9893ReadSwitchReg8(interface,
      KSZ9893_GLOBAL_PORT_MIRROR_SNOOP_CTRL);

   //Enable or disable IGMP snooping
   if(enable)
   {
      temp |= KSZ9893_GLOBAL_PORT_MIRROR_SNOOP_CTRL_IGMP_SNOOP_EN;
   }
   else
   {
      temp &= ~KSZ9893_GLOBAL_PORT_MIRROR_SNOOP_CTRL_IGMP_SNOOP_EN;
   ...
   ksz9893WriteSwitchReg8(interface, KSZ9893_GLOBAL_PORT_MIRROR_SNOOP_CTRL,
      temp);
}
...
void ksz9893EnableMldSnooping(NetInterface *interface, bool_t enable)
{
   ...
   if(enable)
   {
      temp |= KSZ9893_GLOBAL_PORT_MIRROR_SNOOP_CTRL_MLD_SNOOP_EN;
   }
   else
   {
      temp &= ~KSZ9893_GLOBAL_PORT_MIRROR_SNOOP_CTRL_MLD_SNOOP_EN;
...
void ksz9893EnableRsvdMcastTable(NetInterface *interface, bool_t enable)
{
   ...
   temp = ksz9893ReadSwitchReg8(interface, KSZ9893_SWITCH_LUE_CTRL0);

   //Enable or disable the reserved multicast table
   if(enable)
   {
      temp |= KSZ9893_SWITCH_LUE_CTRL0_RESERVED_MCAST_LOOKUP_EN;
   }
   else
   {
      temp &= ~KSZ9893_SWITCH_LUE_CTRL0_RESERVED_MCAST_LOOKUP_EN;
   ...
   ksz9893WriteSwitchReg8(interface, KSZ9893_SWITCH_LUE_CTRL0, temp);
}
   ...
   const SwitchFdbEntry *entry)
{
   ...
   j = KSZ9893_STATIC_MAC_TABLE_SIZE;

   //Loop through the static MAC table
   for(i = 0; i < KSZ9893_STATIC_MAC_TABLE_SIZE; i++)
   {
      //Read current entry
      error = ksz9893GetStaticFdbEntry(interface, i, &currentEntry);
      ...
      {
         //Keep track of the first free entry
         if(j == KSZ9893_STATIC_MAC_TABLE_SIZE)
         {
            j = i;
         }
      }
   ...
   if(j < KSZ9893_STATIC_MAC_TABLE_SIZE)
   {
      ...
      ksz9893WriteSwitchReg32(interface, KSZ9893_STATIC_TABLE_ENTRY1,
         KSZ9893_STATIC_TABLE_ENTRY1_VALID);

      //Set the relevant forward ports
      if(entry->destPorts == SWITCH_CPU_PORT_MASK)
      {
         value = KSZ9893_PORT3_MASK;
      }
      else
      {
         value = entry->destPorts & KSZ9893_PORT_MASK;
      }

      //Enable overriding of port state
      if(entry->override)
      {
         value |= KSZ9893_STATIC_TABLE_ENTRY2_OVERRIDE;
      }

      //Write the Static Address Table Entry 2 register
      ksz9893WriteSwitchReg32(interface, KSZ9893_STATIC_TABLE_ENTRY2, value);

      //Copy MAC address (first 16 bits)
      value = (entry->macAddr.b[0] << 8) | entry->macAddr.b[1];

      //Write the Static Address Table Entry 3 register
      ksz9893WriteSwitchReg32(interface, KSZ9893_STATIC_TABLE_ENTRY3, value);
   ...
   const SwitchFdbEntry *entry)
{
   ...
   //Loop through the static MAC table
   for(j = 0; j < KSZ9893_STATIC_MAC_TABLE_SIZE; j++)
   {
      //Read current entry
      error = ksz9893GetStaticFdbEntry(interface, j, &currentEntry);
   ...
   if(j < KSZ9893_STATIC_MAC_TABLE_SIZE)
   {
      ...
      ksz9893WriteSwitchReg32(interface, KSZ9893_STATIC_TABLE_ENTRY1, 0);
      ksz9893WriteSwitchReg32(interface, KSZ9893_STATIC_TABLE_ENTRY2, 0);
      ksz9893WriteSwitchReg32(interface, KSZ9893_STATIC_TABLE_ENTRY3, 0);
   ...
   SwitchFdbEntry *entry)
{
   ...
   if(index < KSZ9893_STATIC_MAC_TABLE_SIZE)
   {
      ...
      //Valid entry?
      if((value & KSZ9893_STATIC_TABLE_ENTRY1_VALID) != 0)
      {
         //Read the Static Address Table Entry 2 register
         value = ksz9893ReadSwitchReg32(interface, KSZ9893_STATIC_TABLE_ENTRY2);

         //Retrieve the ports associated with this MAC address
         entry->srcPort = 0;
         entry->destPorts = value & KSZ9893_STATIC_TABLE_ENTRY2_PORT_FORWARD;

         //Check the value of the OVERRIDE bit
         if((value & KSZ9893_STATIC_TABLE_ENTRY2_OVERRIDE) != 0)
         {
            entry->override = TRUE;
         }
         else
         ...
         value = ksz9893ReadSwitchReg32(interface, KSZ9893_STATIC_TABLE_ENTRY3);
         ...
         value = ksz9893ReadSwitchReg32(interface, KSZ9893_STATIC_TABLE_ENTRY4);
   ...
   SwitchFdbEntry *entry)
{
   ...
   if(index == 0)
   {
      ...
      ksz9893WriteSwitchReg32(interface, KSZ9893_ALU_TABLE_CTRL, 0);

      //Start the search operation
      ksz9893WriteSwitchReg32(interface, KSZ9893_ALU_TABLE_CTRL,
         KSZ9893_ALU_TABLE_CTRL_START_FINISH |
         KSZ9893_ALU_TABLE_CTRL_ACTION_SEARCH);
   }
   ...
   do
   {
      ...
      value = ksz9893ReadSwitchReg32(interface, KSZ9893_ALU_TABLE_CTRL);

      //This bit goes high to indicate either a new valid entry is returned or
      //the search is complete
   } while((value & KSZ9893_ALU_TABLE_CTRL_VALID_ENTRY_OR_SEARCH_END) == 0);

   //Check whether the next valid entry is ready
   if((value & KSZ9893_ALU_TABLE_CTRL_VALID) != 0)
   {
      ...
      value = ksz9893ReadSwitchReg32(interface, KSZ9893_ALU_TABLE_ENTRY1);
      value = ksz9893ReadSwitchReg32(interface, KSZ9893_ALU_TABLE_ENTRY2);

      //Retrieve the port associated with this MAC address
      switch(value & KSZ9893_ALU_TABLE_ENTRY2_PORT_FORWARD)
      {
      case KSZ9893_ALU_TABLE_ENTRY2_PORT1_FORWARD:
         entry->srcPort = KSZ9893_PORT1;
         break;
      case KSZ9893_ALU_TABLE_ENTRY2_PORT2_FORWARD:
         entry->srcPort = KSZ9893_PORT2;
         break;
      case KSZ9893_ALU_TABLE_ENTRY2_PORT3_FORWARD:
         entry->srcPort = KSZ9893_PORT3;
         break;
      default:
         entry->srcPort = 0;
         break;
      ...
      value = ksz9893ReadSwitchReg32(interface, KSZ9893_ALU_TABLE_ENTRY3);
      ...
      value = ksz9893ReadSwitchReg32(interface, KSZ9893_ALU_TABLE_ENTRY4);
...
void ksz9893FlushDynamicFdbTable(NetInterface *interface, uint8_t port)
{
   ...
   temp = ksz9893ReadSwitchReg8(interface, KSZ9893_SWITCH_LUE_CTRL2);
   temp &= ~KSZ9893_SWITCH_LUE_CTRL2_FLUSH_OPTION;
   temp |= KSZ9893_SWITCH_LUE_CTRL2_FLUSH_OPTION_DYNAMIC;
   ksz9893WriteSwitchReg8(interface, KSZ9893_SWITCH_LUE_CTRL2, temp);

   //Valid port number?
   if(port >= KSZ9893_PORT1 && port <= KSZ9893_PORT3)
   {
      //Save the current state of the port
      state = ksz9893ReadSwitchReg8(interface, KSZ9893_PORTn_MSTP_STATE(port));

      //Turn off learning capability
      ksz9893WriteSwitchReg8(interface, KSZ9893_PORTn_MSTP_STATE(port),
         state | KSZ9893_PORTn_MSTP_STATE_LEARNING_DIS);

      //All the entries associated with a port that has its learning capability
      //being turned off will be flushed
      temp = ksz9893ReadSwitchReg8(interface, KSZ9893_SWITCH_LUE_CTRL1);
      temp |= KSZ9893_SWITCH_LUE_CTRL1_FLUSH_MSTP_ENTRIES;
      ksz9893WriteSwitchReg8(interface, KSZ9893_SWITCH_LUE_CTRL1, temp);

      //Restore the original state of the port
      ksz9893WriteSwitchReg8(interface, KSZ9893_PORTn_MSTP_STATE(port), state);
   }
   else
   {
      //Trigger a flush of the entire address lookup table
      temp = ksz9893ReadSwitchReg8(interface, KSZ9893_SWITCH_LUE_CTRL1);
      temp |= KSZ9893_SWITCH_LUE_CTRL1_FLUSH_ALU_TABLE;
      ksz9893WriteSwitchReg8(interface, KSZ9893_SWITCH_LUE_CTRL1, temp);
   }
}
   ...
   bool_t enable, uint32_t forwardPorts)
{
   ...
   temp = ksz9893ReadSwitchReg32(interface, KSZ9893_UNKONWN_MULTICAST_CTRL);

   //Clear port map
   temp &= ~KSZ9893_UNKONWN_MULTICAST_CTRL_FWD_MAP;
   ...
   if(enable)
   {
      ...
      {
         temp |= KSZ9893_UNKONWN_MULTICAST_CTRL_FWD_MAP_PORT3;
      }

      //Select the desired forward ports
      temp |= forwardPorts & KSZ9893_UNKONWN_MULTICAST_CTRL_FWD_MAP_ALL;
   }
   else
   {
      //Disable forwarding
   ...
   ksz9893WriteSwitchReg32(interface, KSZ9893_UNKONWN_MULTICAST_CTRL, temp);
}
   ...
   uint8_t address, uint16_t data)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      n = KSZ9893_PORTn_ETH_PHY_REG(port, address);
      //Write the 16-bit value
      ksz9893WriteSwitchReg16(interface, n, data);
   }
   else if(interface->smiDriver != NULL)
   {
   ...
   uint8_t address)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      data = ksz9893ReadSwitchReg16(interface, n);
   }
   else if(interface->smiDriver != NULL)
   {
...
void ksz9893DumpPhyReg(NetInterface *interface, uint8_t port)
{
   ...
   for(i = 0; i < 32; i++)
   {
      ...
      TRACE_DEBUG("%02" PRIu8 ": 0x%04" PRIX16 "\r\n", i,
         ksz9893ReadPhyReg(interface, port, i));
   }
   ...
   uint8_t devAddr, uint16_t regAddr, uint16_t data)
{
   ...
   ksz9893WritePhyReg(interface, port, KSZ9893_MMDACR,
      KSZ9893_MMDACR_FUNC_ADDR | (devAddr & KSZ9893_MMDACR_DEVAD));

   //Write MMD register address
   ksz9893WritePhyReg(interface, port, KSZ9893_MMDAADR, regAddr);

   //Select data operation
   ksz9893WritePhyReg(interface, port, KSZ9893_MMDACR,
      KSZ9893_MMDACR_FUNC_DATA_NO_POST_INC | (devAddr & KSZ9893_MMDACR_DEVAD));

   //Write the content of the MMD register
   ksz9893WritePhyReg(interface, port, KSZ9893_MMDAADR, data);
}
   ...
   uint8_t devAddr, uint16_t regAddr)
{
   ...
   return ksz9893ReadPhyReg(interface, port, KSZ9893_MMDAADR);
}
   ...
   uint8_t data)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      command = KSZ9893_SPI_CMD_WRITE;
      //Set register address
      command |= (address << 5) & KSZ9893_SPI_CMD_ADDR;
