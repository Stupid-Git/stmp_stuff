...
#include "drivers/switch/ksz9896_driver.h"
#include "debug.h"


/**
 * @brief KSZ9896 Ethernet switch driver
 **/
...
/**
 * @brief Tail tag rules (host to KSZ9896)
 **/

const uint16_t ksz9896IngressTailTag[6] =
{
   HTONS(KSZ9896_TAIL_TAG_NORMAL_ADDR_LOOKUP),
   HTONS(KSZ9896_TAIL_TAG_PORT_BLOCKING_OVERRIDE | KSZ9896_TAIL_TAG_DEST_PORT1),
   HTONS(KSZ9896_TAIL_TAG_PORT_BLOCKING_OVERRIDE | KSZ9896_TAIL_TAG_DEST_PORT2),
   HTONS(KSZ9896_TAIL_TAG_PORT_BLOCKING_OVERRIDE | KSZ9896_TAIL_TAG_DEST_PORT3),
   HTONS(KSZ9896_TAIL_TAG_PORT_BLOCKING_OVERRIDE | KSZ9896_TAIL_TAG_DEST_PORT4),
   HTONS(KSZ9896_TAIL_TAG_PORT_BLOCKING_OVERRIDE | KSZ9896_TAIL_TAG_DEST_PORT5)
};


/**
 * @brief KSZ9896 Ethernet switch initialization
...
error_t ksz9896Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing KSZ9896...\r\n");
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      do
      {
         ...
         temp = ksz9896ReadSwitchReg8(interface, KSZ9896_CHIP_ID1);

         //The returned data is invalid until the serial interface is ready
      } while(temp != KSZ9896_CHIP_ID1_DEFAULT);

#if (ETH_PORT_TAGGING_SUPPORT == ENABLED)
      //Enable tail tag feature
      temp = ksz9896ReadSwitchReg8(interface, KSZ9896_PORT6_OP_CTRL0);
      temp |= KSZ9896_PORTn_OP_CTRL0_TAIL_TAG_EN;
      ksz9896WriteSwitchReg8(interface, KSZ9896_PORT6_OP_CTRL0, temp);

      //Disable frame length check (silicon errata workaround 12)
      temp = ksz9896ReadSwitchReg8(interface, KSZ9896_SWITCH_MAC_CTRL0);
      temp &= ~KSZ9896_SWITCH_MAC_CTRL0_FRAME_LEN_CHECK_EN;
      ksz9896WriteSwitchReg8(interface, KSZ9896_SWITCH_MAC_CTRL0, temp);
#else
      ...
      for(port = KSZ9896_PORT1; port <= KSZ9896_PORT5; port++)
      {
#if (ETH_PORT_TAGGING_SUPPORT == ENABLED)
         ...
         if(interface->port != 0)
         {
            ...
            ksz9896SetPortState(interface, port, SWITCH_PORT_STATE_LISTENING);
         }
         else
#endif
         {
            //Enable transmission, reception and address learning
            ksz9896SetPortState(interface, port, SWITCH_PORT_STATE_FORWARDING);
         }
      }

      //Restore default age count
      ksz9896WriteSwitchReg8(interface, KSZ9896_SWITCH_LUE_CTRL0,
         KSZ9896_SWITCH_LUE_CTRL0_AGE_COUNT_DEFAULT |
         KSZ9896_SWITCH_LUE_CTRL0_HASH_OPTION_CRC);

      //Restore default age period
      ksz9896WriteSwitchReg8(interface, KSZ9896_SWITCH_LUE_CTRL3,
         KSZ9896_SWITCH_LUE_CTRL3_AGE_PERIOD_DEFAULT);

      //Add internal delay to ingress and egress RGMII clocks
      temp = ksz9896ReadSwitchReg8(interface, KSZ9896_PORT6_XMII_CTRL1);
      temp |= KSZ9896_PORT6_XMII_CTRL1_RGMII_ID_IG;
      temp |= KSZ9896_PORT6_XMII_CTRL1_RGMII_ID_EG;
      ksz9896WriteSwitchReg8(interface, KSZ9896_PORT6_XMII_CTRL1, temp);

      //Start switch operation
      ksz9896WriteSwitchReg8(interface, KSZ9896_SWITCH_OP,
   ...
   for(port = KSZ9896_PORT1; port <= KSZ9896_PORT5; port++)
   {
      ...
      ksz9896WriteMmdReg(interface, port, 0x01, 0x6F, 0xDD0B);
      ksz9896WriteMmdReg(interface, port, 0x01, 0x8F, 0x6032);
      ksz9896WriteMmdReg(interface, port, 0x01, 0x9D, 0x248C);
      ksz9896WriteMmdReg(interface, port, 0x01, 0x75, 0x0060);
      ksz9896WriteMmdReg(interface, port, 0x01, 0xD3, 0x7777);
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x06, 0x3008);
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x08, 0x2001);

      //Improve transmit waveform amplitude (silicon errata workaround 2)
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x04, 0x00D0);

      //EEE must be manually disabled (silicon errata workaround 3)
      ksz9896WriteMmdReg(interface, port, KSZ9896_MMD_EEE_ADV, 0);

      //Adjust power supply settings (silicon errata workaround 6)
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x13, 0x6EFF);
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x14, 0xE6FF);
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x15, 0x6EFF);
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x16, 0xE6FF);
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x17, 0x00FF);
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x18, 0x43FF);
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x19, 0xC3FF);
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x1A, 0x6FFF);
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x1B, 0x07FF);
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x1C, 0x0FFF);
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x1D, 0xE7FF);
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x1E, 0xEFFF);
      ksz9896WriteMmdReg(interface, port, 0x1C, 0x20, 0xEEEE);

      //Select tri-color dual-LED mode (silicon errata workaround 14)
      ksz9896WriteMmdReg(interface, port, KSZ9896_MMD_LED_MODE,
      ...
      ksz9896DumpPhyReg(interface, port);
   }

   //Perform custom configuration
   ksz9896InitHook(interface);
...
/**
 * @brief KSZ9896 custom configuration
 * @param[in] interface Underlying network interface
 **/
...
void ksz9896Tick(NetInterface *interface)
{
   ...
   if(interface->port != 0)
   {
      ...
      for(i = 0; i < NET_INTERFACE_COUNT; i++)
      {
            ...
            virtualInterface->parent == interface)
         {
            ...
            linkState = ksz9896GetLinkState(interface, virtualInterface->port);
...
#endif
   {
      ...
      {
         //Retrieve current link state
         if(ksz9896GetLinkState(interface, port))
         {
            linkState = TRUE;
         }
      }
...
void ksz9896EventHandler(NetInterface *interface)
{
   ...
   if(interface->port != 0)
   {
      ...
      for(i = 0; i < NET_INTERFACE_COUNT; i++)
      {
            ...
            virtualInterface->parent == interface)
         {
            ...
            //Valid port?
            if(port >= KSZ9896_PORT1 && port <= KSZ9896_PORT5)
            {
               //Retrieve current link state
               linkState = ksz9896GetLinkState(interface, port);

               //Link up event?
               if(linkState && !virtualInterface->linkState)
               {
                  //Retrieve host interface speed
                  interface->linkSpeed = ksz9896GetLinkSpeed(interface,
                     KSZ9896_PORT6);

                  //Retrieve host interface duplex mode
                  interface->duplexMode = ksz9896GetDuplexMode(interface,
                     KSZ9896_PORT6);
                  ...
                  //Check current speed
                  virtualInterface->linkSpeed = ksz9896GetLinkSpeed(interface,
                     port);

                  //Check current duplex mode
                  virtualInterface->duplexMode = ksz9896GetDuplexMode(interface,
                     port);
...
#endif
   {
      ...
      {
         //Retrieve host interface speed
         interface->linkSpeed = ksz9896GetLinkSpeed(interface, KSZ9896_PORT6);
         //Retrieve host interface duplex mode
         interface->duplexMode = ksz9896GetDuplexMode(interface, KSZ9896_PORT6);
   ...
   size_t *length, NetRxAncillary *ancillary)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      if(*length >= (sizeof(EthHeader) + sizeof(uint8_t)))
      {
         ...
         ancillary->port = (*tailTag & KSZ9896_TAIL_TAG_SRC_PORT) + 1;
...
bool_t ksz9896GetLinkState(NetInterface *interface, uint8_t port)
{
   ...
   if(port >= KSZ9896_PORT1 && port <= KSZ9896_PORT5)
   {
      ...
      value = ksz9896ReadPhyReg(interface, port, KSZ9896_BMSR);
      value = ksz9896ReadPhyReg(interface, port, KSZ9896_BMSR);

      //Retrieve current link state
      linkState = (value & KSZ9896_BMSR_LINK_STATUS) ? TRUE : FALSE;
   }
   else
   {
...
uint32_t ksz9896GetLinkSpeed(NetInterface *interface, uint8_t port)
{
   ...
   if(port >= KSZ9896_PORT1 && port <= KSZ9896_PORT5)
   {
      ...
      value = ksz9896ReadPhyReg(interface, port, KSZ9896_PHYCON);

      //Retrieve current link speed
      if((value & KSZ9896_PHYCON_SPEED_1000BT) != 0)
      {
         //1000BASE-T
         linkSpeed = NIC_LINK_SPEED_1GBPS;
      }
      else if((value & KSZ9896_PHYCON_SPEED_100BTX) != 0)
      {
         //100BASE-TX
         linkSpeed = NIC_LINK_SPEED_100MBPS;
      }
      else if((value & KSZ9896_PHYCON_SPEED_10BT) != 0)
      {
         //10BASE-T
   ...
   else if(port == KSZ9896_PORT6)
   {
      //SPI slave mode?
      if(interface->spiDriver != NULL)
      {
         //Read port 6 XMII control 1 register
         value = ksz9896ReadSwitchReg8(interface, KSZ9896_PORT6_XMII_CTRL1);

         //Retrieve host interface type
         type = value & KSZ9896_PORT6_XMII_CTRL1_IF_TYPE;

         //Gigabit interface?
         if(type == KSZ9896_PORT6_XMII_CTRL1_IF_TYPE_RGMII &&
            (value & KSZ9896_PORT6_XMII_CTRL1_SPEED_1000) == 0)
         {
            //1000 Mb/s mode
            linkSpeed = NIC_LINK_SPEED_1GBPS;
         }
         else
         {
            ...
            value = ksz9896ReadSwitchReg8(interface, KSZ9896_PORT6_XMII_CTRL0);

            //Retrieve host interface speed
            if((value & KSZ9896_PORT6_XMII_CTRL0_SPEED_10_100) != 0)
            {
               //100 Mb/s mode
               linkSpeed = NIC_LINK_SPEED_100MBPS;
            }
...
NicDuplexMode ksz9896GetDuplexMode(NetInterface *interface, uint8_t port)
{
   ...
   if(port >= KSZ9896_PORT1 && port <= KSZ9896_PORT5)
   {
      ...
      if((value & KSZ9896_PHYCON_DUPLEX_STATUS) != 0)
      {
   ...
   else if(port == KSZ9896_PORT6)
   {
      ...
      if(interface->spiDriver != NULL)
      {
         ...
         //Retrieve host interface duplex mode
         if((value & KSZ9896_PORT6_XMII_CTRL0_DUPLEX) != 0)
         {
            duplexMode = NIC_FULL_DUPLEX_MODE;
         }
         else
   ...
   SwitchPortState state)
{
   ...
   if(port >= KSZ9896_PORT1 && port <= KSZ9896_PORT5)
   {
      ...
      temp = ksz9896ReadSwitchReg8(interface, KSZ9896_PORTn_MSTP_STATE(port));
      ...
      case SWITCH_PORT_STATE_LISTENING:
         temp &= ~KSZ9896_PORTn_MSTP_STATE_TRANSMIT_EN;
         temp |= KSZ9896_PORTn_MSTP_STATE_RECEIVE_EN;
         temp |= KSZ9896_PORTn_MSTP_STATE_LEARNING_DIS;
         break;
      ...
      ksz9896WriteSwitchReg8(interface, KSZ9896_PORTn_MSTP_STATE(port), temp);
   }
}
...
SwitchPortState ksz9896GetPortState(NetInterface *interface, uint8_t port)
{
   ...
   if(port >= KSZ9896_PORT1 && port <= KSZ9896_PORT5)
   {
      ...
      //Check port state
      if((temp & KSZ9896_PORTn_MSTP_STATE_TRANSMIT_EN) == 0 &&
         (temp & KSZ9896_PORTn_MSTP_STATE_RECEIVE_EN) == 0 &&
         (temp & KSZ9896_PORTn_MSTP_STATE_LEARNING_DIS) != 0)
      {
         //Disabled state
         state = SWITCH_PORT_STATE_DISABLED;
      }
      else if((temp & KSZ9896_PORTn_MSTP_STATE_TRANSMIT_EN) == 0 &&
         (temp & KSZ9896_PORTn_MSTP_STATE_RECEIVE_EN) != 0 &&
         (temp & KSZ9896_PORTn_MSTP_STATE_LEARNING_DIS) != 0)
      {
         //Listening state
...
void ksz9896EnableIgmpSnooping(NetInterface *interface, bool_t enable)
{
   ...
   temp = ksz9896ReadSwitchReg8(interface,
      KSZ9896_GLOBAL_PORT_MIRROR_SNOOP_CTRL);

   //Enable or disable IGMP snooping
   if(enable)
   {
      temp |= KSZ9896_GLOBAL_PORT_MIRROR_SNOOP_CTRL_IGMP_SNOOP_EN;
   }
   else
   {
      temp &= ~KSZ9896_GLOBAL_PORT_MIRROR_SNOOP_CTRL_IGMP_SNOOP_EN;
   ...
   ksz9896WriteSwitchReg8(interface, KSZ9896_GLOBAL_PORT_MIRROR_SNOOP_CTRL,
      temp);
}
...
void ksz9896EnableMldSnooping(NetInterface *interface, bool_t enable)
{
   ...
   if(enable)
   {
      temp |= KSZ9896_GLOBAL_PORT_MIRROR_SNOOP_CTRL_MLD_SNOOP_EN;
   }
   else
   {
      temp &= ~KSZ9896_GLOBAL_PORT_MIRROR_SNOOP_CTRL_MLD_SNOOP_EN;
...
void ksz9896EnableRsvdMcastTable(NetInterface *interface, bool_t enable)
{
   ...
   temp = ksz9896ReadSwitchReg8(interface, KSZ9896_SWITCH_LUE_CTRL0);

   //Enable or disable the reserved multicast table
   if(enable)
   {
      temp |= KSZ9896_SWITCH_LUE_CTRL0_RESERVED_MCAST_LOOKUP_EN;
   }
   else
   {
      temp &= ~KSZ9896_SWITCH_LUE_CTRL0_RESERVED_MCAST_LOOKUP_EN;
   ...
   ksz9896WriteSwitchReg8(interface, KSZ9896_SWITCH_LUE_CTRL0, temp);
}
   ...
   const SwitchFdbEntry *entry)
{
   ...
   j = KSZ9896_STATIC_MAC_TABLE_SIZE;

   //Loop through the static MAC table
   for(i = 0; i < KSZ9896_STATIC_MAC_TABLE_SIZE; i++)
   {
      //Read current entry
      error = ksz9896GetStaticFdbEntry(interface, i, &currentEntry);
      ...
      {
         //Keep track of the first free entry
         if(j == KSZ9896_STATIC_MAC_TABLE_SIZE)
         {
            j = i;
         }
      }
   ...
   if(j < KSZ9896_STATIC_MAC_TABLE_SIZE)
   {
      ...
      ksz9896WriteSwitchReg32(interface, KSZ9896_STATIC_TABLE_ENTRY1,
         KSZ9896_STATIC_TABLE_ENTRY1_VALID);

      //Set the relevant forward ports
      if(entry->destPorts == SWITCH_CPU_PORT_MASK)
      {
         value = KSZ9896_PORT6_MASK;
      }
      else
      {
         value = entry->destPorts & KSZ9896_PORT_MASK;
      }

      //Enable overriding of port state
      if(entry->override)
      {
         value |= KSZ9896_STATIC_TABLE_ENTRY2_OVERRIDE;
      }

      //Write the Static Address Table Entry 2 register
      ksz9896WriteSwitchReg32(interface, KSZ9896_STATIC_TABLE_ENTRY2, value);

      //Copy MAC address (first 16 bits)
      value = (entry->macAddr.b[0] << 8) | entry->macAddr.b[1];

      //Write the Static Address Table Entry 3 register
      ksz9896WriteSwitchReg32(interface, KSZ9896_STATIC_TABLE_ENTRY3, value);
      ...
      ksz9896WriteSwitchReg32(interface, KSZ9896_STATIC_TABLE_ENTRY4, value);

      //Write the TABLE_INDEX field with the 4-bit index value
      value = (j << 16) & KSZ9896_STATIC_MCAST_TABLE_CTRL_TABLE_INDEX;
      //Set the TABLE_SELECT bit to 0 to select the static address table
      value &= ~KSZ9896_STATIC_MCAST_TABLE_CTRL_TABLE_SELECT;
      //Set the ACTION bit to 0 to indicate a write operation
      value &= ~KSZ9896_STATIC_MCAST_TABLE_CTRL_ACTION;
      //Set the START_FINISH bit to 1 to initiate the operation
      value |= KSZ9896_STATIC_MCAST_TABLE_CTRL_START_FINISH;

      //Start the write operation
      ksz9896WriteSwitchReg32(interface, KSZ9896_STATIC_MCAST_TABLE_CTRL,
         value);
      ...
      do
      {
         ...
         value = ksz9896ReadSwitchReg32(interface,
            KSZ9896_STATIC_MCAST_TABLE_CTRL);

         //Poll the START_FINISH bit
      } while((value & KSZ9896_STATIC_MCAST_TABLE_CTRL_START_FINISH) != 0);
   ...
   const SwitchFdbEntry *entry)
{
   ...
   //Loop through the static MAC table
   for(j = 0; j < KSZ9896_STATIC_MAC_TABLE_SIZE; j++)
   {
      //Read current entry
      error = ksz9896GetStaticFdbEntry(interface, j, &currentEntry);
   ...
   if(j < KSZ9896_STATIC_MAC_TABLE_SIZE)
   {
      ...
      ksz9896WriteSwitchReg32(interface, KSZ9896_STATIC_TABLE_ENTRY1, 0);
      ksz9896WriteSwitchReg32(interface, KSZ9896_STATIC_TABLE_ENTRY2, 0);
      ksz9896WriteSwitchReg32(interface, KSZ9896_STATIC_TABLE_ENTRY3, 0);
      ksz9896WriteSwitchReg32(interface, KSZ9896_STATIC_TABLE_ENTRY4, 0);
   ...
   SwitchFdbEntry *entry)
{
   ...
   if(index < KSZ9896_STATIC_MAC_TABLE_SIZE)
   {
      //Write the TABLE_INDEX field with the 4-bit index value
      value = (index << 16) & KSZ9896_STATIC_MCAST_TABLE_CTRL_TABLE_INDEX;
      //Set the TABLE_SELECT bit to 0 to select the static address table
      value &= ~KSZ9896_STATIC_MCAST_TABLE_CTRL_TABLE_SELECT;
      ...
      value = ksz9896ReadSwitchReg32(interface, KSZ9896_STATIC_TABLE_ENTRY1);

      //Valid entry?
      if((value & KSZ9896_STATIC_TABLE_ENTRY1_VALID) != 0)
      {
         //Read the Static Address Table Entry 2 register
         value = ksz9896ReadSwitchReg32(interface, KSZ9896_STATIC_TABLE_ENTRY2);

         //Retrieve the ports associated with this MAC address
         entry->srcPort = 0;
         entry->destPorts = value & KSZ9896_STATIC_TABLE_ENTRY2_PORT_FORWARD;

         //Check the value of the OVERRIDE bit
         if((value & KSZ9896_STATIC_TABLE_ENTRY2_OVERRIDE) != 0)
         {
            entry->override = TRUE;
         }
         else
         ...
         value = ksz9896ReadSwitchReg32(interface, KSZ9896_STATIC_TABLE_ENTRY3);
         ...
         value = ksz9896ReadSwitchReg32(interface, KSZ9896_STATIC_TABLE_ENTRY4);
...
void ksz9896FlushStaticFdbTable(NetInterface *interface)
{
   ...
   for(i = 0; i < KSZ9896_STATIC_MAC_TABLE_SIZE; i++)
   {
      ...
      //Write the TABLE_INDEX field with the 4-bit index value
      value = (i << 16) & KSZ9896_STATIC_MCAST_TABLE_CTRL_TABLE_INDEX;
      //Set the TABLE_SELECT bit to 0 to select the static address table
      value &= ~KSZ9896_STATIC_MCAST_TABLE_CTRL_TABLE_SELECT;
      //Set the ACTION bit to 0 to indicate a write operation
      value &= ~KSZ9896_STATIC_MCAST_TABLE_CTRL_ACTION;
   ...
   SwitchFdbEntry *entry)
{
   ...
   if(index == 0)
   {
      ...
      ksz9896WriteSwitchReg32(interface, KSZ9896_ALU_TABLE_CTRL, 0);

      //Start the search operation
      ksz9896WriteSwitchReg32(interface, KSZ9896_ALU_TABLE_CTRL,
   ...
   do
   {
      ...
      value = ksz9896ReadSwitchReg32(interface, KSZ9896_ALU_TABLE_CTRL);

      //This bit goes high to indicate either a new valid entry is returned or
      //the search is complete
   } while((value & KSZ9896_ALU_TABLE_CTRL_VALID_ENTRY_OR_SEARCH_END) == 0);

   //Check whether the next valid entry is ready
   if((value & KSZ9896_ALU_TABLE_CTRL_VALID) != 0)
   {
      ...
      value = ksz9896ReadSwitchReg32(interface, KSZ9896_ALU_TABLE_ENTRY1);
      value = ksz9896ReadSwitchReg32(interface, KSZ9896_ALU_TABLE_ENTRY2);

      //Retrieve the port associated with this MAC address
      switch(value & KSZ9896_ALU_TABLE_ENTRY2_PORT_FORWARD)
      {
      case KSZ9896_ALU_TABLE_ENTRY2_PORT1_FORWARD:
         entry->srcPort = KSZ9896_PORT1;
         break;
      case KSZ9896_ALU_TABLE_ENTRY2_PORT2_FORWARD:
         entry->srcPort = KSZ9896_PORT2;
         break;
      case KSZ9896_ALU_TABLE_ENTRY2_PORT3_FORWARD:
         entry->srcPort = KSZ9896_PORT3;
         break;
      case KSZ9896_ALU_TABLE_ENTRY2_PORT4_FORWARD:
         entry->srcPort = KSZ9896_PORT4;
         break;
      case KSZ9896_ALU_TABLE_ENTRY2_PORT5_FORWARD:
         entry->srcPort = KSZ9896_PORT5;
         break;
      case KSZ9896_ALU_TABLE_ENTRY2_PORT6_FORWARD:
         entry->srcPort = KSZ9896_PORT6;
         break;
      default:
         entry->srcPort = 0;
         break;
      ...
      value = ksz9896ReadSwitchReg32(interface, KSZ9896_ALU_TABLE_ENTRY3);
      ...
      value = ksz9896ReadSwitchReg32(interface, KSZ9896_ALU_TABLE_ENTRY4);
...
void ksz9896FlushDynamicFdbTable(NetInterface *interface, uint8_t port)
{
   ...
   temp = ksz9896ReadSwitchReg8(interface, KSZ9896_SWITCH_LUE_CTRL2);
   temp &= ~KSZ9896_SWITCH_LUE_CTRL2_FLUSH_OPTION;
   temp |= KSZ9896_SWITCH_LUE_CTRL2_FLUSH_OPTION_DYNAMIC;
   ksz9896WriteSwitchReg8(interface, KSZ9896_SWITCH_LUE_CTRL2, temp);

   //Valid port number?
   if(port >= KSZ9896_PORT1 && port <= KSZ9896_PORT6)
   {
      //Save the current state of the port
      state = ksz9896ReadSwitchReg8(interface, KSZ9896_PORTn_MSTP_STATE(port));

      //Turn off learning capability
      ksz9896WriteSwitchReg8(interface, KSZ9896_PORTn_MSTP_STATE(port),
         state | KSZ9896_PORTn_MSTP_STATE_LEARNING_DIS);

      //All the entries associated with a port that has its learning capability
      //being turned off will be flushed
      temp = ksz9896ReadSwitchReg8(interface, KSZ9896_SWITCH_LUE_CTRL1);
      temp |= KSZ9896_SWITCH_LUE_CTRL1_FLUSH_MSTP_ENTRIES;
      ksz9896WriteSwitchReg8(interface, KSZ9896_SWITCH_LUE_CTRL1, temp);

      //Restore the original state of the port
      ksz9896WriteSwitchReg8(interface, KSZ9896_PORTn_MSTP_STATE(port), state);
   }
   else
   {
      //Trigger a flush of the entire address lookup table
      temp = ksz9896ReadSwitchReg8(interface, KSZ9896_SWITCH_LUE_CTRL1);
      temp |= KSZ9896_SWITCH_LUE_CTRL1_FLUSH_ALU_TABLE;
      ksz9896WriteSwitchReg8(interface, KSZ9896_SWITCH_LUE_CTRL1, temp);
   }
}
   ...
   bool_t enable, uint32_t forwardPorts)
{
   ...
   temp = ksz9896ReadSwitchReg32(interface, KSZ9896_UNKONWN_MULTICAST_CTRL);

   //Clear port map
   temp &= ~KSZ9896_UNKONWN_MULTICAST_CTRL_FWD_MAP;

   //Enable or disable forwarding of unknown multicast packets
   if(enable)
   {
      //Enable forwarding
      temp |= KSZ9896_UNKONWN_MULTICAST_CTRL_FWD;

      //Check whether unknown multicast packets should be forwarded to the CPU port
      if((forwardPorts & SWITCH_CPU_PORT_MASK) != 0)
      {
         temp |= KSZ9896_UNKONWN_MULTICAST_CTRL_FWD_MAP_PORT6;
      }

      //Select the desired forward ports
      temp |= forwardPorts & KSZ9896_UNKONWN_MULTICAST_CTRL_FWD_MAP_ALL;
   }
   else
   {
      //Disable forwarding
   ...
   ksz9896WriteSwitchReg32(interface, KSZ9896_UNKONWN_MULTICAST_CTRL, temp);
}
   ...
   uint8_t address, uint16_t data)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      n = KSZ9896_PORTn_ETH_PHY_REG(port, address);
      //Write the 16-bit value
      ksz9896WriteSwitchReg16(interface, n, data);
   }
   else if(interface->smiDriver != NULL)
   {
   ...
   uint8_t address)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      data = ksz9896ReadSwitchReg16(interface, n);
   }
   else if(interface->smiDriver != NULL)
   {
...
void ksz9896DumpPhyReg(NetInterface *interface, uint8_t port)
{
   ...
   for(i = 0; i < 32; i++)
   {
      ...
      TRACE_DEBUG("%02" PRIu8 ": 0x%04" PRIX16 "\r\n", i,
         ksz9896ReadPhyReg(interface, port, i));
   }
   ...
   uint8_t devAddr, uint16_t regAddr, uint16_t data)
{
   ...
   ksz9896WritePhyReg(interface, port, KSZ9896_MMDACR,
      KSZ9896_MMDACR_FUNC_ADDR | (devAddr & KSZ9896_MMDACR_DEVAD));

   //Write MMD register address
   ksz9896WritePhyReg(interface, port, KSZ9896_MMDAADR, regAddr);

   //Select data operation
   ksz9896WritePhyReg(interface, port, KSZ9896_MMDACR,
      KSZ9896_MMDACR_FUNC_DATA_NO_POST_INC | (devAddr & KSZ9896_MMDACR_DEVAD));

   //Write the content of the MMD register
   ksz9896WritePhyReg(interface, port, KSZ9896_MMDAADR, data);
}
   ...
   uint8_t devAddr, uint16_t regAddr)
{
   ...
   return ksz9896ReadPhyReg(interface, port, KSZ9896_MMDAADR);
}
   ...
   uint8_t data)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      command = KSZ9896_SPI_CMD_WRITE;
      //Set register address
      command |= (address << 5) & KSZ9896_SPI_CMD_ADDR;
