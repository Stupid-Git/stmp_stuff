...
//Transmit buffer
static uint8_t txBuffer[WINC3400_TX_BUFFER_SIZE];
//Receive buffer
static uint8_t rxBuffer[WINC3400_RX_BUFFER_SIZE];


/**
 * @brief WINC3400 driver
 **/

const NicDriver winc3400Driver =
{
   NIC_TYPE_ETHERNET,
   ETH_MTU,
   winc3400Init,
   ...
   winc3400EnableIrq,
   winc3400DisableIrq,
   winc3400EventHandler,
   winc3400SendPacket,
   winc3400UpdateMacAddrFilter,
   NULL,
   NULL,
   NULL,
   TRUE,
...
error_t winc3400Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing WINC3400...\r\n");
   ...
   do
   {
      ...
      param.pfAppWifiCb = winc3400AppWifiEvent;
      param.pfAppMonCb = NULL;
      param.strEthInitParam.pfAppWifiCb = NULL;
      param.strEthInitParam.pfAppEthCb = winc3400AppEthEvent;

      //Set receive buffer
      param.strEthInitParam.au8ethRcvBuf = rxBuffer;
      param.strEthInitParam.u16ethRcvBufSize = WINC3400_RX_BUFFER_SIZE;

      //Enable Ethernet mode
      param.strEthInitParam.u8EthernetEnable = M2M_WIFI_MODE_ETHERNET;

      //Initialize WINC3400 controller
...
/**
 * @brief WINC3400 interrupt service routine
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > WINC3400_TX_BUFFER_SIZE)
   {
...
error_t winc3400UpdateMacAddrFilter(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Updating WINC3400 multicast filter...\r\n");
...
void winc3400AppWifiEvent(uint8_t msgType, void *msg)
{
   ...
   //Debug message
   TRACE_INFO("WINC3400 Wi-Fi event callback\r\n");
...
void winc3400AppEthEvent(uint8_t msgType, void *msg, void *ctrlBuf)
{
   ...
   //Debug message
   TRACE_DEBUG("WINC3400 RX event callback\r\n");
