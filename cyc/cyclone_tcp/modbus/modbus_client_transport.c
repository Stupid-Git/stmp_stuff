/**
 * @file modbus_client_transport.c
...
error_t modbusClientOpenConnection(ModbusClientContext *context)
{
   ...
   if(context->tlsInitCallback != NULL)
   {
      ...
      error = tlsSetBufferSize(context->tlsContext,
         MODBUS_CLIENT_TLS_TX_BUFFER_SIZE, MODBUS_CLIENT_TLS_RX_BUFFER_SIZE);
      //Any error to report?
      if(error)
         return error;
 ...
 * @param[in] context Pointer to the Modbus/TCP client context
 * @param[in] serverIpAddr IP address of the Modbus/TCP server to connect to
