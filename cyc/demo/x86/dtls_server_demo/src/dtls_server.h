...
typedef error_t (*DtlsServerDatagramCallback)(DtlsClientSession *session,
   const struct sockaddr_in *remoteIpAddr, const uint8_t *data, size_t length);
...
struct _DtlsServerContext
{
   ...
   uint_t counter;
   intptr_t socket;                                          ///<Underlying UDP socket
   struct sockaddr_in clientIpAddr;                          ///<Client's IP address
   TlsCache *dtlsCache;                                      ///<DTLS session cache
#if (TLS_TICKET_SUPPORT == ENABLED)
   TlsTicketContext tlsTicketContext;                        ///<DTLS ticket encryption context
#endif
