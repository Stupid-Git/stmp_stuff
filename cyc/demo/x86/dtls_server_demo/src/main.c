   ...
   TlsContext *dtlsContext)
{
   ...
   error = tlsEnableSecureRenegotiation(dtlsContext, TRUE);
   //Any error to report?
   if(error)
      return error;

   //Load server's ECDSA certificate
   error = tlsLoadCertificate(dtlsContext, 0, serverEcdsaCert,
      strlen(serverEcdsaCert), serverEcdsaPrivateKey,
      strlen(serverEcdsaPrivateKey), NULL);
   //Any error to report?
   if(error)
      return error;
