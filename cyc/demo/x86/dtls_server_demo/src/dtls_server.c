   ...
   const DtlsServerSettings *settings)
{
   ...
   ret = bind(context->socket, (struct sockaddr *) &serverAddr, sizeof(serverAddr));
   ...
   if(ret < 0)
      ...
      timeout.tv_sec = DTLS_SERVER_TICK_INTERVAL / 1000;
      timeout.tv_usec = (DTLS_SERVER_TICK_INTERVAL % 1000) * 1000;
#endif
   ...
   {
      //Debug message
      TRACE_ERROR("Error: Failed to initialize DTLS cache\r\n");
   }
#endif
   ...
   {
      //Debug message
      TRACE_ERROR("Error: Failed to initialize DTLS ticket context!\r\n");
   }
#endif
...
error_t dtlsServerStart(DtlsServerContext *context)
{
   ...
   taskId = osCreateTask("DTLS Server", (OsTaskCode) dtlsServerTask, context,
      NULL);
...
void dtlsServerTask(DtlsServerContext *context)
{
   ...
   while(1)
   {
      ...
      clientIpAddrLen = sizeof(struct sockaddr_in);

      //Wait for an incoming datagram
      n = recvfrom(context->socket, context->buffer, DTLS_SERVER_BUFFER_SIZE, 0,
         (struct sockaddr *) &context->clientIpAddr, &clientIpAddrLen);
...
#else
               if(context->clientIpAddr.sin_family == session->clientIpAddr.sin_family &&
                  context->clientIpAddr.sin_addr.s_addr == session->clientIpAddr.sin_addr.s_addr &&
                  context->clientIpAddr.sin_port == session->clientIpAddr.sin_port)
#endif
               {
                  //Save current time
                  ...
                  {
                     //Debug message
                     TRACE_DEBUG("Datagram received from client %s port %u (%" PRIuSIZE " bytes)...\r\n",
...
error_t dtlsServerAcceptSession(DtlsServerContext *context,
   DtlsClientSession *session, const struct sockaddr_in *remoteIpAddr)
{
   error_t error;
   TlsState state;
   ...
   if(session->dtlsContext != NULL)
   {
      ...
      do
      {
         ...
         error = tlsSetTicketCallbacks(session->dtlsContext, tlsEncryptTicket,
            tlsDecryptTicket, &session->context->tlsTicketContext);
         //Any error to report?
         if(error)
            break;
#endif
   ...
   const void *data, size_t length)
{
   ...
   //Debug message
   TRACE_INFO("Sending datagram to client %s port %u (%" PRIuSIZE " bytes)...\r\n",
      inet_ntoa(session->clientIpAddr.sin_addr),
      ntohs(session->clientIpAddr.sin_port), length);
   ...
   size_t length, size_t *written, uint_t flags)
{
   ...
   session = (DtlsClientSession *) handle;
   //Point to the DTLS server context
   context = session->context;

   //Send datagram
   n = sendto(context->socket, data, length, 0,
      (struct sockaddr *) &session->clientIpAddr, sizeof(struct sockaddr_in));
